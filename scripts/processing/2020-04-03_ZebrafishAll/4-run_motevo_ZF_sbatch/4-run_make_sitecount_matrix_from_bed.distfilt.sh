...
echo "Sleeping hour"
sleep 3600
echo "Done sleeping, submitting jobs"
...
for dist in $dists; do
    for jmark in $jmarks; do
        echo $jmark
        inf="/hpc/hub_oudenaarden/jyeung/data/scChiC/tfbs_output_cluster_${jsuffix}/${jmark}/motevo_outputs/bed/merged_bed_closestbed_long/motevo_merged.closest.dist.long.distfilt_${dist}.bed"
        [[ ! -e $inf ]] && echo "$inf not found, exiting" && exit 1

        outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/mara_analysis_${jsuffix}/${jmark}/mara_input/sitecount_mats"
        [[ ! -d $outdir ]] && mkdir -p $outdir

        outf="${outdir}/hiddenDomains_motevo_merged.closest.long.distfilt_${dist}.scale_${jscale}.center_${jcenter}.byrow_${byrow}.${jmark}.txt"
        ...
        if [ $byrow -eq 0 ]
        then
            # echo "Rscript $rs $inf $outf --scale $jscale --center $jcenter&"
            ...
            # echo "Rscript $rs $inf $outf --scale $jscale --center $jcenter --byrow&"
