...
for ctype in $ctypes; do
    inmain="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/comparisons_with_pseudobulk/${ctype}_binsize-${bsize}_comparison_${suffix}"
    outdir=$inmain
    for jmark in $marks; do
        indat="$inmain/${jmark}_${ctype}_${markref}_vs_${jmark}_comparison_binsize-${bsize}.npz"
        [[ ! -e $indat ]] && echo "$indat not found, exiting" && exit 1
        for jtype in $jtypes; do
            for cor in $cors; do
                # echo $jtype $cor
                outf="$outdir/${jmark}_${ctype}_${markref}_vs_${jmark}_comparison.${jtype}.${cor}.pdf"
