...
for indir in `ls -d $inmain/B6-13W1-BM-H3K*-merged`; do
    ...
    outname=$dname.filtered.RZcounts_splitRS_bugfixed.csv
    outf=$tagdir/$outname
    [[ -e $outf ]] && echo "$outf found, continuing" && continue
    echo "python $ps $inbam -joinedFeatureTags -sampleTags SM -featureTags RZ,RS -o $outf"
    . /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3;python $ps $inbam -joinedFeatureTags RZ,RS -sampleTags SM -o $outf
    if (( $(($((++n)) % $maxjobs)) == 0 )) ; then
