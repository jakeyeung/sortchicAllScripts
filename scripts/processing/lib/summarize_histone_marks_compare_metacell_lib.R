...
GetGOData <- function(out.tb.lst, ontology, jdecreasing=TRUE, order.by="Hyper_Fold_Enrichment", top){
...
outdir <- args[[2]]
mcdir <- args[[3]]
jdist <- StrToInteger(args[[4]])
jmean <- StrToInteger(args[[5]])
jmin <- StrToInteger(args[[6]])
jmax <- StrToInteger(args[[7]])
jtops <- args[[8]]
jthres <- args[[9]]  # for GREAT
binarize <- args[[10]]
...
# for (jchip in jchips){
parallel::mclapply(as.list(jchips), function(jchip){
  ...
  jdir <- file.path(indir, paste0('ldaAnalysisHiddenDomains_', jdist, '/lda_outputs.meanfilt_', jmean, '.cellmin_', jmin, '.cellmax_', jmax, '.binarize.', binarize))
  ...
  inf.GREAT <- file.path(jdir, "downstream", paste0(infbase, ".GREAT.", jthres, ".Robj"))
  # inf.mc <- file.path(paste0("~/Dropbox/scCHiC_figs/FIG4_BM/", jchip, ".datadir_mc_f.Rda"))
  inf.mc <- file.path(mcdir, paste0(jchip, ".datadir_mc_f.Rda"))
  ...
  if (!exists("regions.annotated")){
    print("regions.annotated not found, making it from annotatePeak()")
    regions.annotated <- as.data.frame(annotatePeak(regions.range, 
                                                    TxDb=TxDb.Mmusculus.UCSC.mm10.knownGene, 
                                                    annoDb='org.Mm.eg.db'))
    regions.annotated$region_coord <- rownames(regions.range)
  }
  # otherwise regions.annotated already loaded as object 
  ...
  regions.intersect <- subsetByOverlaps(GRanges(regions.annotated), Qgr)  # used to be regions.ranges but sometimes we dont have that 
  ...
  GOdata.all <- bind_rows(lapply(jontos, function(onto) GetGOData(out.tb.lst, onto, jdecreasing, order.by, top)))
