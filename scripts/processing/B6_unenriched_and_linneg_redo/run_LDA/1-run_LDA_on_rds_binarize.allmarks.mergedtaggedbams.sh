...
prefix="B6BM_All_allmarks_mergedtagged"

indir="/hpc/hub_oudenaarden/jyeung/data/scChiC/from_macbook/count_mat_binfilt_cellfilt_for_LDA_PZ-Bl6BM_FromMergedTaggedBams/quality_control_PZ_Bl6-BM-Linneg-Stemcells_AllMerged"
...
for inf in `ls -d $indir/*.rds`; do
    ...
    echo "cd $workdir; . /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs $inf $outdir --topics $topics --projname $bname --binarize" | qsub -l h_rt=${jtime} -l h_vmem=${jmem} -o ${BNAME}.out -e ${BNAME}.err -pe threaded $ncores -m beas -M j.yeung@hubrecht.eu -N $bname
