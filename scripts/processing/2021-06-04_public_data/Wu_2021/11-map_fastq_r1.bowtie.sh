...
indxbase="/hpc/hub_oudenaarden/group_references/ensembl/97/homo_sapiens/primary_assembly_NOMASK_ERCC92"
...
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/Wu_et_al_2021/SRA_data/prefetch_outputs/SRR12638101/bams_r1_notrim_bowtie2"
...
for f1 in `ls -d $inmain/*fastq.gz`; do
    ...
    # [[ ! -d $outdir ]] && mkdir $outdir
    f1="$inmain/${bname}.demux.fastq.gz"
    ...
    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3; bowtie2 -x $indxbase -U $f1 --threads $ncores --local --very-sensitive-local --no-unal --no-discordant -I 10 -X 700 | samtools view -Sb - > $outf"
