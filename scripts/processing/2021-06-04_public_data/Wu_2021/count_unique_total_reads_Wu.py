...
def main():
    ...
    with pysam.AlignmentFile(args.infile, "rb") as bamfile:
        for entry in bamfile:
            ...
            total += 1
            # SRR12638101.1.ACCTCAGCCTC_CCTAGTAGCCGGGA 
            bc_umi = entry.query_name.split(".")[2]
            bc = bc_umi.split("_")[0]
            umi = bc_umi.split("_")[1]
            assert len(bc) == bclength
            assert len(umi) == umilength
            ...
            if bc not in outdic:
                outdic[bc] = {}
            if umi not in outdic[bc]:
                outdic[bc][umi] = {}
            if coord not in outdic[bc][umi]:
                outdic[bc][umi][coord] = 0
            outdic[bc][umi][coord] += 1
    ...
    with gzip.open(args.outfile, "wt") as outf:
        ...
        for bc, bcdic in outdic.items():
            for umi, umidic in bcdic.items():
                for coord, dupcount in umidic.items():
                    # print(coord)
                    chromo = coord.split(".")[0]
                    start = coord.split(".")[1]
                    end = coord.split(".")[2]
                    outrow = [chromo, start, end, dupcount, bc, umi]
