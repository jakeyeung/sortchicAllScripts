...
outdir="${inmain}/counts_in_peaks_vs_nonpeaks_vs_blacklist.faster.clean3.blfix.ChIP_seq_peaks"
...
for jmark in $jmarks; do
    ...
    inbed="/hpc/hub_oudenaarden/jyeung/data/public_data/ENCODE/bedfiles/ENCODEpeaks.${jmark}.nochr.cleanchr.qval_3.bed"

    bname="K562_AllMerged_${jmark}.merged.sorted.tagged.bam"
    inbam=${inmain}/${bname}
    [[ ! -e $inbam ]] && echo "inbam $inbam not found, exiting" && exit 1

        outf1=$outdir/${bname}.cuts_in_peaks.ChIP_seq_peaks.csv
        [[ -e $outf1 ]] && echo "outf1 $outf1 found, continuing" && continue
        outf2=$outdir/${bname}.cuts_in_genome.ChIP_seq_peaks.csv
        [[ -e $outf2 ]] && echo "outf2 $outf2 found, continuing" && continue

        BNAME1=$outdir/${bname}.cuts_in_peaks.ChIP_seq_peaks.qsub
        DBASE1=$(dirname "${BNAME1}")
        [[ ! -d $DBASE1 ]] && echo "dbase1 $DBASE2 not found, exiting" && exit 1

        BNAME2=$outdir/${bname}.cuts_in_genome.ChIP_seq_peaks.qsub
