...
inmain="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/K562_Kaya-Okur/H3K27me3/dedup_beds"

# outmain="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/K562_tech_rep_merged"
outmain="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/K562_Kaya-Okur/H3K27me3"
[[ ! -d $outmain ]] && echo "outmain $outmain not found, exiting" && exit 1
outdir="${outmain}/Kaya-Okur_vs_ENCODE_peaks_filtered_dedup"
[[ ! -d $outdir ]] && mkdir $outdir

for beda in `ls -d $inmain/*.bed`; do
    bedamain=$(basename $beda)
    bedamain=${bedamain%.*}
    outbed=${outdir}/${bedamain}.ENCODE_filt.dedup.bed
    BNAME=${outdir}/${bedamain}.dedup.sbatchout
    DBASE=$(dirname "${BNAME}")
