...
for jmark in $jmarks; do
    inlda="${ldadir}/lda_outputs.count_mat_from_sitecount_mat.${jmark}.filtNAcells_allbins.from_same_annot_file.old_cells_only.2020-12-27.K-30.binarize.FALSE/ldaOut.count_mat_from_sitecount_mat.${jmark}.filtNAcells_allbins.from_same_annot_file.old_cells_only.2020-12-27.K-30.Robj"
    [[ ! -e $inlda ]] && echo "$inlda not found, exiting" && exit 1

    inmat="${countdir}/count_mat_from_sitecount_mat.${jmark}.filtNAcells_allbins.from_same_annot_file.new_cells_only.2020-12-27.rds"
    [[ ! -e $inmat ]] && echo "$inmat not found, exiting" && exit 1

    outbase="BM_project_new_onto_old.${jmark}"
    ...
    echo $cmd
    sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME}_%j.log --ntasks=1 --nodes=1 --job-name=project_on_new${jmark} --wrap "$cmd"
done
