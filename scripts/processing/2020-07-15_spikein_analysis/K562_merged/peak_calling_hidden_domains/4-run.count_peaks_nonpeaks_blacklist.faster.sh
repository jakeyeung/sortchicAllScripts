...
ps1="/home/hub_oudenaarden/jyeung/projects/scChiC/scripts/processing/2020-07-15_spikein_analysis/K562_merged/peak_calling_hidden_domains/count_peaks_blacklist.faster.py"
...
outdir="${inmain}/counts_in_peaks_vs_nonpeaks_vs_blacklist.faster"
...
for jmark in $jmarks; do
    ...
    [[ ! -e $inbam ]] && echo "inbam $inbam not found, exiting" && exit 1
        ...
        cmd1=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate scmo4_NewCountFilters; python $ps1 $inbam $outf1 --filterXA -minMQ $mapq --dedup --r1only -blacklist $bl --proper_pairs_only --no_softclips -max_base_edits 2 --no_indels -bedfile $inbed"
        cmd2=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate scmo4_NewCountFilters; python $ps2 $inbam $outf2 --filterXA -minMQ $mapq --dedup --r1only -blacklist $bl --proper_pairs_only --no_softclips -max_base_edits 2 --no_indels"
        echo $cmd1
        echo $cmd2
        # exit 0
        sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME1}_%j.log --ntasks=1 --nodes=1 --job-name=MakeCounts_${bname} --wrap "$cmd1"
        sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME1}_%j.log --ntasks=1 --nodes=1 --job-name=MakeCounts_${bname} --wrap "$cmd2"
