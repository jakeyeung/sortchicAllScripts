...
def main():
    ...
    parser.add_argument('-blacklist', metavar='bedfile of blacklist',
                        help='Blacklist')
    parser.add_argument('-chromo', metavar='Chromoname',
                        help='Chromosome name to summarize eg chr1 or 1')
    ...
    with pysam.AlignmentFile(args.infile, "rb") as samfile:
            ...
            # for i, read in enumerate(samfile.fetch(args.chromo, 42000, 45000)):
                if i % 100000 == 0:
                    print("Nbr reads processed: ", i)
                samp = read.get_tag('SM')
                if samp not in count_dic:
                    count_dic[samp] = {'peak' : 0, 'nonpeak' : 0, 'blacklist' : 0, 'skipped' : 0}

                if not read_should_be_counted(read, args):
                    count_dic[samp]['skipped'] += 1 
                    continue
                # Read is in blacklist
                if debug:
                    print("Checking blacklist")
                if read_in_dic(read, blacklist_dic, debug = debug):
                    count_dic[samp]['blacklist'] += 1
                    continue

                # check if in peaks
                if debug:
                    print("Checking peak")
                if read_in_dic(read, peaks_dic, debug = debug):
                    # print("Is peak read")
                    count_dic[samp]['peak'] += 1
                else:
                    # print("Is not peak read")
                    count_dic[samp]['nonpeak'] += 1
                if debug:
                    input("Waiting... peak assigned to nonblacklist")

    # write count_dic as table
    with open(args.outfile, "w") as outf:
        jwriter = csv.writer(outf, delimiter = "\t")
        # write header
        jheader = ['samp', 'cuts_in_peak', 'cuts_not_in_peaks', 'cuts_in_blacklist', 'chromo', 'bad_reads']
        for samp, count_subdic in count_dic.items():
            outrow = [samp, count_subdic['peak'], count_subdic['nonpeak'], count_subdic['blacklist'], args.chromo, count_subdic['skipped']]
