...
indir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/K562_tech_rep_merged/bams_G1filt_split_by_G1filt/r1only"
# outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/K562_ENCODE_rerun_from_Buys"
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/K562_tech_rep_merged/bigwigs_G1filt_split_by_G1filt.for_chipseq_comparison.mapq_${mapq}.r1only"
...
bs="/home/hub_oudenaarden/jyeung/projects/scchic-functions/scripts/processing_scripts/bam_to_bigwig_mm10_with_blacklist.mapq.sh"
bsize=1000

for mark in $marks; do
    # fbase="${mark}_ChIP_dedup_index"
    fbase="K562_AllMerged_${mark}.merged.sorted.tagged.G1filt.r1only.sorted"
    fname=${fbase}.bam
    inbam=${indir}/${fname}
    outbw=${outdir}/${fbase}.bsize_${bsize}.mapq_${mapq}.r1only.bw

    BNAME=${outdir}/${fbase}.sbatch
    DBASE=$(dirname "${BNAME}")
    [[ ! -d $DBASE ]] && echo "$DBASE not found, exiting" && exit 1

    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3; bash $bs $inbam $outbw ${bsize} $bl ${mapq}"
