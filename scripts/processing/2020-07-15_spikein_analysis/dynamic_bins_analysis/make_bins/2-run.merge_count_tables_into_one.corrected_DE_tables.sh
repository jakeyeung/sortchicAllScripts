...
indir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/count_tables_all_four_marks_dynamic_bins.corrected_DE_tables"
...
for mark in $marks; do
    ...
    bedfnames="dynamic_bins.50kb.corrected_DE_tables.${mark}.2021-04-07.txt"
    for bedname in $bedfnames; do
        echo $bedname

        infs=${indir}/*${mark}*.corrected_DE_tables.txt
        echo $infs
        outf="${outdir}/count_tables_merged.${mark}.${bedname}.corrected_DE_tables.rds"
