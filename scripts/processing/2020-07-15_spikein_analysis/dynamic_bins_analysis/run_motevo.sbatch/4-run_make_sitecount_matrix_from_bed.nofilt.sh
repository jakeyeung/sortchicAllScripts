...
for jmark in $jmarks; do
    ...
    outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/mara_analysis_${jsuffix2}/${jmark}/mara_input/sitecount_mats"
    [[ ! -d $outdir ]] && mkdir -p $outdir

    outf="${outdir}/dynamic_bins_50kb__motevo_merged.closest.long.scale_${jscale}.center_${jcenter}.byrow_${byrow}.${jmark}.lessmem.txt"
    ...
    then
        ...
        sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME}_%j.log --ntasks=1 --nodes=1 --job-name=MakeNmatNotByRow --wrap "$cmd"
    else
        ...
        sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME}_%j.log --ntasks=1 --nodes=1 --job-name=MakeNmatByRow --wrap "$cmd"
    fi
done
wait
echo "Done script"
