...
outdir="${prefix}/jyeung/data/scChiC/raw_demultiplexed/LDA_outputs_all_spikeins/glmpcaPois_mouse_spikein_VAN5046_VAN5230_BM_varfilt"
...
varfilt=1.5
for inf in `ls -d $indir/*varfilt_${varfilt}.rds`; do
    bname=$(basename $inf)
    bname=${bname%.*}

    BNAME=${outdir}/${bname}.sbatchout.${jpenalty}.${maxiter}.${minibatch}.${optimizer}.tol_${tol}.devfilt_${devfilt}.varfilt_${varfilt}
    DBASE=$(dirname "${BNAME}")
    [[ ! -d $DBASE ]] && echo "$DBASE not found, exiting" && exit 1

    outf=${outdir}/${bname}.glmpcaout.penalty_${jpenalty}.maxiter_${maxiter}.${minibatch}.${optimizer}.tol_${tol}.devfilt_${devfilt}.varfilt_${varfilt}.by_plate.RData
