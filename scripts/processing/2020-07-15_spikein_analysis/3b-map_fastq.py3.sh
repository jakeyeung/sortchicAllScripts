...
for indir in `ls -d $inmain/PZ*`; do
    ...
    f1=$indir/trimmed.R1.py3.fastq.gz
    f2=$indir/trimmed.R2.py3.fastq.gz
    ...
    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3; $bwabin mem -t $ncores $indxbase $f1 $f2 | samtools view -Sb - > $outf"
