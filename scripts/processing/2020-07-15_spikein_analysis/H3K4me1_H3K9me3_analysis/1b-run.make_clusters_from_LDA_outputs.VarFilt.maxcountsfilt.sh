...
jname="mouse_spikein_BMround2all.dbl_common_rows"
...
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/H3K4me1_H3K9me3_analysis/clustering_outputs.${jname}.UseClusterColname"
[[ ! -d $outdir ]] && mkdir $outdir

# jmarks="K27m3 K9m3 K9m3K27m3"
# jmarks="K27m3 K9m3 K27m3xK9m3"
jmarks="H3K4me1 H3K9me3 H3K4me1xH3K9me3"
jsuffix="match_dbl"

for jmark in $jmarks; do
  # dname="lda_outputs.count_mat.${jmark}.KeepTopBins_500.${jsuffix}.K-30.binarize.FALSE"
  dname="lda_outputs.count_mat.${jmark}.${jsuffix}.K-30.binarize.FALSE"
  # fname="ldaOut.count_mat.${jmark}.KeepTopBins_500.${jsuffix}.K-30.Robj"
  ...
  # echo ". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -infile $inf -outprefix $outpref -mark $jmark -topn 150 --WriteTopicLoadings -inftss $inftss -species mouse" | qsub -l h_rt=${jtime} -l h_vmem=${jmem} -o ${BNAME}.out -e ${BNAME}.err -pe threaded 1 -m beas -M j.yeung@hubrecht.eu -N cluster_${jmark}
  cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -infile $inf -outprefix $outpref -mark $jmark -topn 150 --WriteTopicLoadings -inftss $inftss -species mouse"
  sbatch --time=$jtime --mem-per-cpu=$jmem --output=${BNAME}_%j.log --ntasks=1 --nodes=1 --job-name=MakeClusters_${jmark} --wrap "$cmd"
  # echo ". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -infile $inf -outprefix $outpref -mark $jmark -topn 150 --WriteTopicLoadings -inftss $inftss -species mouse"
done
