...
for indir in `ls -d $inmain/PZ*`; do
    ...
    inbam=$indir/bwaMapped.scmo3.again.bam
    BNAME=$indir/$bname.sort_index_tag.qsub
    DBASE=$(dirname "${BNAME}")
    [[ ! -d $DBASE ]] && echo "$DBASE not found, exiting" && exit 1
    # sortedbam=$indir/bwaMapped.sorted.bam
    sortedbam=$indir/bwaMapped.scmo3.again_contigfixed.sorted.bam
    # outbamtagged=$outdir/${bname}.tagged.bam
    outbamtagged=$outdir/${bname}.scmo3.again_contigfixed.tagged.bam
