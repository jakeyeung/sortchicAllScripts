...
for inf in `ls -d $indir/*topn_${topn}.rds`; do
    ...
    BNAME=${outdir}/${bname}.sbatchout.${jpenalty}.by_plate.topn_${topn}
    DBASE=$(dirname "${BNAME}")
    [[ ! -d $DBASE ]] && echo "$DBASE not found, exiting" && exit 1

    outf=${outdir}/${bname}.glmpcaout.penalty_${jpenalty}.by_plate.RData
    [[ -e $outf ]] && echo "$outf found, continuing" && continue
    # . /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -infile $inf -outfile $outf -inspike $outspike -K 30 -penalty $jpenalty --by_plate
    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate R3.6; Rscript $rs -infile $inf -outfile $outf -inspike $outspike -K 30 -penalty $jpenalty --by_plate"
