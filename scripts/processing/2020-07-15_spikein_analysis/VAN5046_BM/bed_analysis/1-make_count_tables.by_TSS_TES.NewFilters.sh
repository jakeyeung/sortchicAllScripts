...
bedfile="/hpc/hub_oudenaarden/jyeung/data/databases/refseq/MmRefseqTssToTes.chromorenamed.merged.rearranged.nochromo.sorted.bed"
...
for inbam in `ls -d $inmain/*.tagged.bam`; do
    ...
    bname=${bname%.*}
    outf=$outdir/${bname}.countTable.TSS_to_TES.csv
