...
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/glmpca_outputs/same_annot_file_rerun"
...
for binskeep in $binskeepvec; do
    for niter in $nitervec; do
        ...
        for jmark in $jmarks; do
            ...
            outbase=${outdir}/glmpca.${jmark}.bincutoff_${bincutoff}.binskeep_${binskeep}.byplate.szname_${szname}.niter_${niter}.reorder_rownames.dupfilt
            # check output doesnt exist
            outcheck="${outbase}.RData"
            [[ -e $outcheck ]] && echo "$outcheck found, continuing" && continue
            BNAME=${outdir}/glmpca.${jmark}.bincutoff_${bincutoff}.binskeep_${binskeep}.byplate.szname_${szname}.niter_${niter}.reorder_rownames.dupfilt
            DBASE=$(dirname "${BNAME}")
