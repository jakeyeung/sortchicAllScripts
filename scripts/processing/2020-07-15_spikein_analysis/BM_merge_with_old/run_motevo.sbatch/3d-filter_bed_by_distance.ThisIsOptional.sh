...
for thres in $threses; do
    for mark in $marks; do
        indir="/hpc/hub_oudenaarden/jyeung/data/scChiC/tfbs_output_cluster_ZF-AllMerged2_Peaks_1000/${mark}/motevo_outputs/bed/merged_bed_closestbed_long"
        ...
        awk -v jthres=$thres '$7 > jthres || $7 < -jthres' $inf > $outf&  # filters out the promoter area 
        if (( $(($((++n)) % $maxjobs)) == 0 )) ; then
