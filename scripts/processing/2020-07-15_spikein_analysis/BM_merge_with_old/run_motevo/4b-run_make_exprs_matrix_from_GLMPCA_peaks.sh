...
indir="/hpc/hub_oudenaarden/jyeung/data/scChiC/glmpca_outputs"
...
for jmark in $jmarks; do
    ...
    bname="BM_${jmark}.BM_AllMerged3.VarCorrection.Poisson.GLMPCA.withchromo"

    outdir="${outmain}/count_mats_peaks_from_GLMPCA"
    [[ ! -d $outdir ]] && mkdir $outdir
    outf="${outdir}"/${bname}.cleanuprows.txt
    outpdf="${outdir}"/${bname}.cleanuprows.pdf
