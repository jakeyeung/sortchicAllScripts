...
# bedfile="/hpc/hub_oudenaarden/jyeung/data/databases/refseq/MmRefseqTss.chromorenamed.50000.again.nochromo.bed"
# bedfile="/hpc/hub_oudenaarden/jyeung/data/databases/refseq/MmRefseqTss.chromorenamed.${binsize}.again.nochromo.sorted.bed"

# outdir="${inmain}/count_tables_from_TSS/binsize_${binsize}"
outdir="${inmain}/count_tables_from_bins/binsize_${binsize}"
[[ ! -d $outdir ]] && mkdir $outdir
...
for mark in ${marks}; do
    for inbam in `ls -d ${bamdir}/BM_round1_round2_merged_${mark}_*.bam`; do
        ...
        cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate scmo4_NewCountFilters; bamToCountTable.py --filterXA -minMQ $mapq $inbam -bin $binsize -sliding $stepsize -o $outftab -sampleTags SM -joinedFeatureTags reference_name  -binTag DS --dedup -blacklist $bl --r1only --proper_pairs_only --no_softclips -max_base_edits 2 --no_indels"
