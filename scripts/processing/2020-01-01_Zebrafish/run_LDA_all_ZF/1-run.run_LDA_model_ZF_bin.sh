...
for jmark in $marks; do
    for inf in `ls -d $indir/ZF-*${jmark}*.${jdate}.RData`; do
        ...
        echo "cd $workdir; /hpc/hub_oudenaarden/jyeung/software/anaconda3/envs/R3.6/bin/Rscript $rs $inf $outdir $K $topics $tunemodels $meanmax $cellmin $cellmax $binarize $bname" | qsub -l h_rt=${jtime} -l h_vmem=${jmem} -o ${BNAME}.out -e ${BNAME}.err -pe threaded $ncores -m beas -M j.yeung@hubrecht.eu -N bin.${binarize}.$bname
    done
done
