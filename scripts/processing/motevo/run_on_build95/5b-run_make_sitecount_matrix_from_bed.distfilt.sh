...
suffix="build95"  # suffix for sitecount matrix
suffixCounts="build95.cells_from_bin_analysis"  # MARA analysis now includes exprs matrix with more clels
...
for thres in $threses; do
    ...
    for jmark in $jmarks; do
        echo $jmark
        inf="/hpc/hub_oudenaarden/jyeung/data/scChiC/tfbs_output_singlegene_${suffix}/${jmark}/motevo_outputs/bed/merged_bed_closestbed_long/motevo_merged.closest.dist.$fstr.long.bed"
        [[ ! -e $inf ]] && echo "$inf not found, exiting" && exit 1

        outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/mara_analysis_${suffixCounts}/${jmark}/mara_input/sitecount_mats"
