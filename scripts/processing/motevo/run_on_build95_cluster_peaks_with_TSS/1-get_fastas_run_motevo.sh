...
prefix="cluster"
tssdist=20000
suffix="build95_WithDedupGeneTSS_${tssdist}"
scratchmain="/hpc/hub_oudenaarden/jyeung/data/scChiC/tfbs_output_singlegene_${prefix}_${suffix}/${jmark}"

[[ -d $scratchmain ]] && echo "$scratchmain found, exiting" && exit 1
[[ ! -d $scratchmain ]] && mkdir -p $scratchmain

## BEGIN GET FASTA ## 
windowsf="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_demultiplexed/merged_cluster_bam_hiddenDomains_output_build95/merged_across_clusters_${jmark}/merged_${jmark}.1000.cutoff_analysis.blacklistfilt.WithDedupGeneTSS_${tssdist}.annot.bed"
