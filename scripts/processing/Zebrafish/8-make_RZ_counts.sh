...
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data/oud3910_HD_0/RZcounts"
...
for indir in `ls -d $inmain/PZ*`; do
    ...
    # echo ". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3; bamToCountTable.py $inbam -sampleTags SM -featureTags RZ -o $outf --dedup -minMQ 40" | qsub -l h_rt=${jtime} -l h_vmem=${jmem} -o ${BNAME}.out -e ${BNAME}.err -pe threaded 1 -m beas -M j.yeung@hubrecht.eu -N $bname.RZcounts
done
