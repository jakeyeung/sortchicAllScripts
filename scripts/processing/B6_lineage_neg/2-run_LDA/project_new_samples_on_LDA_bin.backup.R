...
parser$add_argument('infile', metavar='INFILE RDATA',
                                            help='Input .RData containing LDA object and count mat')
...
load(args$infile, v=T)  # out.objs$out.lda, # out.objs$count.mat
assertthat::assert_that(class(out.objs$out.lda) == "LDA_Gibbs")
assertthat::assert_that(class(out.objs$count.mat) == "dgCMatrix")

system.time(
  out.lda.predict <- posterior(out.objs$out.lda, t(as.matrix(out.objs$count.mat)))
)
