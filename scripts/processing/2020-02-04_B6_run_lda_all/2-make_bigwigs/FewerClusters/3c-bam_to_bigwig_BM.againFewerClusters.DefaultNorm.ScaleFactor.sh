...
# for bsize in $bsizes; do
    outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data/bigwigs_with_R1only/ZellerRawData_B6_All_MergedByMarks_final.bigwigs_by_cluster.MAPQ_${mapq}.bsize_${bsize}.2020-06-17.offset.r1only${suffix}${suffix2}.${normby}.Verbose"
    ...
    for inbam in `ls -d $indir/*.bam`; do
        ...
        echo "$nf / ($totalcuts)"
        fullsf=`awk -v nf=$nf -v totalcuts=$totalcuts 'BEGIN { print ( nf / totalcuts ) }'`
        echo $fullsf
