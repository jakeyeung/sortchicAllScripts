...
suffix=".Downsamp"
# suffix=""
bs="/home/hub_oudenaarden/jyeung/projects/scchic-functions/scripts/processing_scripts/bam_to_bigwig_mm10_with_blacklist.offset.DefaultNorm.sh"
...
for bsize in $bsizes; do
    outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/raw_data/bigwigs_with_R1only/ZellerRawData_B6_All_MergedByMarks_final.bigwigs_by_cluster.MAPQ_${mapq}.bsize_${bsize}.2020-06-17.offset.r1only${suffix}.DefaultNorm.Verbose"
    [[ ! -d $outdir ]] && mkdir $outdir
    for inbam in `ls -d $indir/*.bam`; do
        echo $inbam
        bname=$(basename $inbam)
        bname=${bname%.*}  # strip extension 
        outbw=${outdir}/${bname}.${bsize}.DefaultNorm.bw
