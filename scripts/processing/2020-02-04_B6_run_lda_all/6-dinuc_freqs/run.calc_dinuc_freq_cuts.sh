...
outdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/dinuc_freq/again"
...
for inf in `ls -d $inmain/*.bam`; do
    ...
    cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate scmo4_NewCountFilters; python $ps -infile $inf -outfile $outf -refpath $refpath"
    sbatch --time=$jtime --mem-per-cpu=$jmem --output=${bname}_%j.log --ntasks=1 --nodes=1 --job-name=$bname --wrap "$cmd"
    # python $ps -infile $inf -outfile $outf -refpath $refpath
    # . /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate scmo4_NewCountFilters; python $ps -infile $inf -outfile $outf -refpath $refpath
    # exit 0
done
