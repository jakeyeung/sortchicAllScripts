...
for jmark in $jmarks; do
    ...
    outdir="${basedir}/bigwig_outputs/merged_bams.deeptools_outputs.promenh.MAPQ_${mapq}.dist_${dist}.genomewide.${jmark}.${bsize}"
    [[ ! -d $outdir ]] && mkdir $outdir

    refdir="/hpc/hub_oudenaarden/jyeung/data/scChiC/public_data/ENCODE"
    for ref in $(ls -d $refdir/*winsize_0.2020-03-29.bed); do
        echo $ref
        jbase=$(basename $ref)
        jbase=${jbase%.*}
