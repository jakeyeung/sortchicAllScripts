...
# dirsuffix=".Downsamp"
# dirsuffix=""
# jmeth="NormTotalCutsPerCell"
jmeth="NormTotalCutsOnly"
# jmeth="NormHeteroCutsOnly"
dirsuffix=".${jmeth}.DefaultNorm"
normsuffix=".DefaultNorm.${jmeth}"
...
refs="/hpc/hub_oudenaarden/jyeung/data/WKM_BM_merged/from_rstudioserver/offsets_hetero_and_totalcuts/MouseBM_HeteroTotalCounts_50kb_bins.2020-06-20.chrfilt.bed"
...
for jmark in $jmarks; do
    for dist in $dists; do
        ...
        infsinput="${indir}/${jmark}-BM_AllMerged.HSPCs.sorted.cleaned.${bsizeout}${normsuffix}.bw ${indir}/${jmark}-BM_AllMerged.Bcells.sorted.cleaned.${bsizeout}${normsuffix}.bw ${indir}/${jmark}-BM_AllMerged.Granulocytes.sorted.cleaned.${bsizeout}${normsuffix}.bw ${indir}/${jmark}-BM_AllMerged.Erythroblasts.sorted.cleaned.${bsizeout}${normsuffix}.bw"
        ...
        cmd=". /hpc/hub_oudenaarden/jyeung/software/anaconda3/etc/profile.d/conda.sh; conda activate py3; computeMatrix scale-regions -S $infsinput -R $refs -bs ${bsizeout} -b ${dist} -a ${dist} -out $outf --skipZeros --missingDataAsZero -p $ncores --sortRegions keep --samplesLabel HSPCs Bcells Granu Eryth"
