...
reticulate::source_python("/Users/yeung/projects/scchic/scripts/python_functions/parse_dictionary_text.py.py")
...
for (jclust in jclusts){
  ...
  m.pca <- ggplot(pca.dat, aes(x = PC1, y = PC2, label = pseudobulk)) + geom_point() + geom_text_repel() + 
    ...
    ggtitle(paste("Marker from scRNAseq:", jclust), paste("Mark:", jmark, "Direction", jdir)) 
  print(m.pca)
