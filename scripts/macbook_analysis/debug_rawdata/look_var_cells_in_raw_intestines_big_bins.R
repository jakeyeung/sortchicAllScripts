# Jake Yeung
# Date of Creation: 2020-01-18
# File: ~/projects/scchic/scripts/macbook_analysis/debug_rawdata/look_var_cells_in_raw_intestines.R
# Intestines
...
jmark <- "k4me1"
inf <- paste0("/Users/yeung/data/scchic/from_cluster/LDA_outputs_intestines/lda_outputs.mat.Scraped.AllMerged.", jmark, ".TAcutoff_0.5.countscutoff_500_1000.highcutoff_100000.winsize_50000_25000.2020-01-11.K-30.binarize.FALSE/ldaOut.mat.Scraped.AllMerged.", jmark, ".TAcutoff_0.5.countscutoff_500_1000.highcutoff_100000.winsize_50000_25000.2020-01-11.K-30.Robj")
load(inf, v=T)
...
# Calcualte intrachrom var imputed ----------------------------------------
...
# inf.raw <- "/Users/yeung/data/scchic/from_cluster/raw_counts_big_bins/HVG_Scraped_Bl6_k4me1_2019-12-20.2000000.2000000.countTable.csv"
inf.raw <- "/Users/yeung/data/scchic/from_cluster/raw_counts_big_bins/HVG_Scraped_Bl6_k4me1_2019-12-20.10000000.10000000.countTable.csv"
dat.raw <- ReadMatSlideWinFormat(inf.raw, as.sparse = TRUE)

dat.raw.filt <- sweep(dat.raw, MARGIN = 2, STATS = colSums(dat.raw), FUN = "/")
terms.keep <- !grepl("^chrX:|^chrY:", rownames(dat.raw.filt))
dat.raw.filt <- dat.raw.filt[terms.keep, ]

dat.var.raw <- data.frame(cell = colnames(dat.raw.filt), 
                          # frac.umi.var = apply(log2(as.matrix(dat.raw.filt) * 10^6 + 1), 2, var),
                          frac.umi.var = apply(log2(as.matrix(dat.raw.filt)), 2, var),
                          stringsAsFactors = FALSE) %>%
  left_join(., dat.var)

ggplot(dat.var.raw, aes(x = frac.umi.var, y = cell.var.within.sum.norm)) + geom_point() + 
  scale_x_log10 () + scale_y_log10()

# check interchromo var
jchromos <- paste("chr", c(seq(19), "X", "Y"), sep = "")
jsum <- SumAcrossChromos(dat.raw.filt, jchromos, colfunction = mean)
jsum.var <- jsum %>%
...
jmerge <- left_join(jsum.var, dat.var.raw %>% dplyr::select(cell, frac.umi.var))

ggplot(jmerge, aes(x = ncuts.var.chromo, frac.umi.var)) + geom_point()  + 
  scale_x_log10() + scale_y_log10()
...
ggplot(jsum.var, aes(x = ncuts.var.chromo, color = cell.var.within.sum.norm, y = cell.var.within.sum.norm)) + geom_point() + 
  scale_x_log10() + scale_y_log10()  + scale_color_viridis_c(direction = -1)
...
ggplot(jsum.var, aes(x = ncuts.var.chromo, y = ncuts)) + geom_point() + 
  scale_color_viridis_c(direction = -1)
# 
# jsum.mat <- reshape2::dcast(data = jsum, formula = cell ~ chromo, value.var = "ncuts") %>%
#   as.data.frame()
# rownames(jsum.mat) <- jsum.mat$cell
# jsum.mat$cell <- NULL
#   
# pca.out <- prcomp(jsum.mat, center = TRUE, scale. = TRUE)
# 
...
# 
# ggplot(dat.pca, aes(x = PC1, y = cell.var.within.sum.norm)) + geom_point()
# ggplot(dat.pca, aes(x = PC1, y = PC2, color= cell.var.within.sum.norm)) + geom_point() + scale_color_viridis_c(direction = -1)
# ggplot(dat.pca, aes(x = PC2, y = cell.var.within.sum.norm)) + geom_point()
# ggplot(dat.pca, aes(x = PC1, y = cell.var.across)) + geom_point()
# 
# ggplot(dat.loadings, aes(x = PC1, y = PC2, label = chromo)) + geom_point() + geom_text_repel()
# 
# # visualize low cell var across
# # jcell.lowvar <- (dat.meta %>% arrange(cell.var.across))$cell[[1]]
# jcell.lowvar <- (jsum.var %>% arrange(desc(ncuts.var.chromo)))$cell[[1]]
# jcell.highvar <- (dat.meta %>% arrange(desc(cell.var.across)))$cell[[1]]
# 
# ggplot(subset(jsum, cell == jcell.lowvar), aes(x = chromo, y = ncuts)) + geom_bar(stat = "identity") + 
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# ggplot(subset(jsum, cell == jcell.highvar), aes(x = chromo, y = ncuts)) + geom_bar(stat = "identity") + 
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# 
# dat.var.raw <- left_join(dat.var.raw, subset(dat.meta, 
#                                              select = c(cell, cell.var.across)) %>% 
#                            dplyr::rename(cell.var.across.imputed = cell.var.across))
# 
# ggplot(dat.var.raw, aes(x = cell.var.across, y = cell.var.across.imputed)) + geom_point() + 
#   scale_x_log10() + scale_y_log10()
# 
# 
# 
