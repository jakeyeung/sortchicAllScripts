...
# File: ~/projects/scchic/scripts/revision_scripts/revisions2/10-LDA_downstream_HSPCs_only_further.R
# 
...
lda.lst <- lapply(jmarks, function(jmark){
  print(jmark)
  inf <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_progs_only_further/lda_outputs.countmat_TSS_new_progenitors_only.nosl5.", jmark, ".2022-08-02/ldaOut.countmat_TSS_new_progenitors_only.nosl5.", jmark, ".2022-08-02.Robj")
  load(inf, v=T)
  return(out.lda)
})
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/downstream_LDA_progs_only_further"
dir.create(outdir)
...
for (jsuffix in jmarks){
  print(jsuffix)
  outpdf <- file.path(outdir, paste0("LDA_downstream_celltypes_Giladi_further.", jsuffix, ".", Sys.Date(), ".pdf"))
...
for (markref in jmarks){
  outpdf <- file.path(outdir, paste0("marker_genes_C1_C2_C3_further.", markref, ".", Sys.Date(), ".pdf"))
  pdf(outpdf, useDingbats = FALSE)
  for (jname in jnames){
