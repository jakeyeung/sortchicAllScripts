...
seurat.lst <- lapply(jmarks, function(jmark){
  ...
  # print(DefaultAssay(object = chic))
  # chic <- NormalizeData(chic)
  # chic <- ScaleData(chic, do.scale = TRUE)
...
marks.combined.lst <- lapply(jmarks[2:3], function(jmark){
  print(jmark)
  marks.combined <- RunCCA(seurat.lst[[jmarkref]], seurat.lst[[jmark]], num.cc = 30, assay1 = jmarkref, assay2 = jmark)
  outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/integration_data/CCA_outputs"
  save(marks.combined, file = file.path(outdir, paste0("CCA_", jmarkref, "_and_", jmark, ".RData")))
...
dat.loadings <- as.data.frame(marks.combined@reductions$cca@feature.loadings)
dat.loadings$gene <- rownames(dat.loadings)
dat.loadings <- dat.loadings %>%
  dplyr::select(c(CC_1, CC_2, CC_3, gene))

print(head(dat.loadings %>% arrange(CC_1)))
print(head(dat.loadings %>% arrange(desc(CC_1))))
...
# save(dat.umap.cca, file = file.path(outdir, paste0("UMAP_of_CCA_", jmarkref, "_and_", jmarkcheck, ".RData")))
...
pdf(file.path(outdir, paste0("plots_CCA_", jmarkref, "_and_", jmarkcheck, ".pdf")), useDingbats = FALSE)
