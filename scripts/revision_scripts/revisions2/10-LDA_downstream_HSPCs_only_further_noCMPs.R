...
# File: ~/projects/scchic/scripts/revision_scripts/revisions2/10-LDA_downstream_HSPCs_only_further_noCMPs.R
# 
...
lda.lst <- lapply(jmarks, function(jmark){
  print(jmark)
  inf <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_progs_only_further/lda_outputs.countmat_TSS_new_progenitors_only.nosl5.nocmp", jmark, ".2022-08-02/ldaOut.countmat_TSS_new_progenitors_only.nosl5.nocmp", jmark, ".2022-08-02.Robj")
  load(inf, v=T)
  return(out.lda)
})
...
m.ctype.lst <- lapply(jmarks, function(jmark){
  ggplot(dat.meta.lst[[jmark]], aes(x = umap1, y = umap2, color = ctype)) + 
    geom_point() + 
    theme_bw() + 
    ggtitle(jmark) + 
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/downstream_LDA_progs_only_further_no_CMPs"
dir.create(outdir)
...
for (jsuffix in jmarks){
  print(jsuffix)
  outpdf <- file.path(outdir, paste0("LDA_downstream_celltypes_Giladi_further_noCMPs.", jsuffix, ".", Sys.Date(), ".pdf"))
...
for (jmark in jmarks){
  print(jmark)
  outftmp <- file.path(outdir, paste0("dat_umap_progs_only_noCMPs.", jmark, ".", Sys.Date(), ".txt"))
...
for (markref in jmarks){
  outpdf <- file.path(outdir, paste0("marker_genes_C1_C2_C3_further_noCMPs.", markref, ".", Sys.Date(), ".pdf"))
  pdf(outpdf, useDingbats = FALSE)
  for (jname in jnames){
