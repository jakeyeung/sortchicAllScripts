...
clst2color.old["Monocytes"] <- "#d69941"
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots"
...
for (jmark in jmarks){
  ...
  outpdf <- file.path(outdir, paste0("celltyping_umaps_primetime_keepTcells.", jmark, ".", Sys.Date(), ".pdf"))
  ...
  fwrite(dat.annot.lst[[jmark]], file = file.path(outdir, paste0("metadata_with_colors_keepTcells.", jmark, ".", Sys.Date(), ".txt")))
  ...
  for (jctype in jctypes){
    ...
    m <- ggplot(jdat %>% arrange(is.ctype), aes(x = umap1, y = umap2, color = is.ctype)) + 
      geom_point() + 
      theme_bw() + 
      ggtitle(paste(jmark, jctype)) + 
      theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) 
