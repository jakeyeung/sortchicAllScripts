...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/9-pseudotime_for_different_celltypes_k4me3_cleaned.R
# 
...
dat.traj.lst <- lapply(ctypes.names, function(jctype){
  ...
  if (jdim2 == "dim2"){
    ...
    dat.glmpca.keep <- dat.glmpca.keep %>%
      rowwise() %>%
      mutate(is.ctype = ifelse(dim2 < 0.5 * dim1 - 3, FALSE, is.ctype))
  }
