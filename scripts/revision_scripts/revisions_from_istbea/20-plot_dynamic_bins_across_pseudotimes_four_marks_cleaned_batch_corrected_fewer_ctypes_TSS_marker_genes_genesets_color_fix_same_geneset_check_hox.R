...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots/umap_and_trajs/k27me3_batch_corrected_fewer_ctypes_TSS_genesets.check_hox"
dir.create(outdir)
...
parallel::mclapply(jmarks, function(jmark){
  ...
  for (jctype in jctypes){
    ...
    rows.keep.i <- which(grepl(pattern = genegrep, rownames(dat.impute.lst[[jmark]])))
    ...
    rows.keep.split <- paste(sapply(rows.keep, function(x) strsplit(x, "\\.")[[1]][[4]]), collapse = ",")
    
      jtitle <- paste(jmark, jctype, rows.keep.split)
