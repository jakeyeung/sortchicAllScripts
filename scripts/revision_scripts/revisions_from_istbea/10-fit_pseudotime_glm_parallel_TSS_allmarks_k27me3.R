...
for (jmark in jmarks){
  ...
  # indir.mat <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/count_mat_TSS"
  # inf.mat <- file.path(indir.mat, paste0("count_mat_TSS_combined.", jmark, ".2022-02-07.rds"))
  inf.mat <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/count_mat_TSS_rep2rep3only/count_mat_TSS_rep2rep3only.", jmark, ".2022-02-08.rds")
  count.mat <- readRDS(inf.mat)
  ...
  traj.dir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/rename"
  inf.traj.lst <- file.path(traj.dir, paste0("trajs_outputs.", jmark, ".rds"))
  ...
  outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/ptimefits_TSS"
  ...
  fit.out.lst.lst <- parallel::mclapply(jctypes, function(jctype){
    ...
    count.mat.filt <- count.mat[, cells.keep.i]
    
    # bad <- which(!cells.keep %in% colnames(count.mat.filt))
