...
dat.meta <- fread("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_update_ctypes_from_LDA_k4me3_cleaned_k27me3_eryths2/metadata_reannotate_from_LLmat_fix_ctypes_by_batch_dynamicbins.k27me3.2022-04-16.txt")
...
inf.lda <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_k27me3_clean_eryths/lda_outputs.count_mat_merged_with_old_dynbins.k27me3.2022-04-15/ldaOut.count_mat_merged_with_old_dynbins.", jmark, ".2022-04-15.Robj")
load(inf.lda, v=T)


# nns <- c(12, 18, 22, 28, 30, 33)
# mds <- c(0.01, 0.9)
# nns <- c(35, 40, 45, 50, 55)
# nns <- c(50, 55, 60, 65, 70)
# nns <- c(57, 60, 63)
# mds <- c(0.01, 0.5, 0.9)

nns <- c(35, 40, 45, 50, 55, 60, 65)
mds <- c(0.5)
spreads <- c(4, 6, 8)

# combos <- levels(interaction(nns, mds, sep = "_")); names(combos) <- combos
combos <- levels(interaction(nns, mds, spreads, sep = "_")); names(combos) <- combos
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/umaps_adj_params/spreads"
dir.create(outdir)

pdf(file.path(outdir, paste0("nns_mds_outputs.k27me3_ctype_from_LL.", Sys.Date(), ".pdf")))
...
m.lst.lst <- parallel::mclapply(combos, function(combo){
  ...
  jspread <- as.numeric(strsplit(combo, split = "_")[[1]][[3]])
  
  jsettings.tmp <- jsettings
  
  jsettings.tmp[["n_neighbors"]] <- nn
  jsettings.tmp[["min_dist"]] <- md
  jsettings.tmp[["spread"]] <- jspread
  
  
  outf.tmp <- file.path(outdir, paste0("nn_", nn, ".md_", md, ".spread_", jspread, ".", Sys.Date(), ".rds"))
  ...
  jtitle <- paste(jmark, "NN", nn, "MD", md, "spread", jspread)
  jdat <- dat.umap.long %>%
    rowwise() %>%
    mutate(ctype.eryths = ifelse(ctype.from.LL %in% c("Eryths", "MEP"), ctype.from.LL, "zNot"))
  m1 <- ggplot(jdat, aes(x = umap1, y = umap2, color = colcode)) + 
    geom_point() + 
    ggtitle(jtitle) + 
  ...
  m2 <- ggplot(jdat, aes(x = umap1, y = umap2, color = colcode)) + 
    geom_point() + 
    ggtitle(jtitle) + 
    facet_wrap(~batch) + 
    # scale_color_manual(values = cbPalette, na.value = "grey85") + 
