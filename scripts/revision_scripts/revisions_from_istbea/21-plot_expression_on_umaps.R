...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/21-plot_expression_on_umaps.R
# 
...
dat.meta.eryths <- subset(dat.meta, umap1 > -15 & umap2 > 8 & umap1 < 18) %>%
  filter(!ctype %in% c("NKs", "Tcells", "ST", "pDCs", "Bcells", "HSPCs")) %>%
  filter(umap2 > 15 | umap1 > 0)  %>%
  filter(umap2 < umap1 + 40)
...
outdirpdf <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/celltyping/repressive_cleaned_keep_eryths"
outpdf <- file.path(outdirpdf, paste0("check_marker_genes_on_UMAP.", Sys.Date(), ".pdf"))
...
bad.cells <- subset(dat.meta.eryths.label, is.bad)$cell
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/raw_data/count_tables/filtered_count_tables_for_LDA_no_frip_filter_on_eryths_clean/BM_k27me3"
dir.create(outdir)

fwrite(dat.meta.eryths.label, file.path(outdir, paste0("metadata_eryth_traj_only.k27me3.", Sys.Date(), ".txt")))
fwrite(dat.meta, file.path(outdir, paste0("metadata.k27me3.", Sys.Date(), ".txt")))

indir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/raw_data/count_tables/filtered_count_tables_for_LDA_no_frip_filter_on_eryths/BM_k27me3"
...
outf.allcells.allbins <- file.path(outdir, "count_mat_merged_with_old_allbins.k27me3.2022-04-12.rds")
outf.newcells.allbins <- file.path(outdir, "count_mat_new_only_allbins.k27me3.2022-04-12.rds")
outf.allcells.dynbins <- file.path(outdir, "count_mat_merged_with_old_dynbins.k27me3.2022-04-12.rds")
outf.newcells.dynbins <- file.path(outdir, "count_mat_new_only_dynbins.k27me3.2022-04-12.rds")
outf.allcells.tss <- file.path(outdir, "count_mat_merged_with_old_TSS.k27me3.2022-04-12.rds")
outf.newcells.tss <- file.path(outdir, "count_mat_new_only_TSS.k27me3.2022-04-12.rds")
