...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/2-check_LDA_output_dynamic_bins_repressive_cleaned_k27me3_eryth_no_frip.R
# 
...
# jsuffix2 <- "new_only_dynbins"

outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/metadata/repressive_cleaned_keep_eryths"
dir.create(outdir)
...
for (jmark in jmarks){
  dat.meta.var.lst <- parallel::mclapply(jsuffix2.vec, function(jsuffix2){
    ...
    outf.pdf <- file.path(outdir, paste0("LDA_outputs", jsuffix2, ".",  jsuffixbins, ".", jmark, ".", Sys.Date(), ".pdf"))
    indir <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_", jmark, "_keep_eryths")
    dname <- paste0("count_mat_", jsuffix2, ".", jmark, ".2022-04-12")
