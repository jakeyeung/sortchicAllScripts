...
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/11-pseudotime_downstream_k27me3_k9me3_together.R
# 
...
for (jctype in jctypes){
  ...
  inf.traj.k9 <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/trajs_outputs.k9me3.2022-02-06.rds")
  ...
  dat.meta.full.lst <- lapply(jmarks, function(jmark){
    fread(file.path(indir.meta, paste0("metadata.", jmark, ".2022-03-01.txt")))
  })
  dat.meta.lst <- lapply(jmarks, function(jmark){
    fread(file.path(indir.meta, paste0("metadata.", jmark, ".2022-03-01.txt"))) %>%
      dplyr::select(cell, colcode, batch, Batch)
  })
  ...
  outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/ptimefits_hits_k27_k9_together"
  outpdf <- file.path(outdir, paste0("traj_batchcorrect_by_ctype_", jctype, ".neg_slope_hits.", Sys.Date(), ".pdf"))
