...
for (jctype in jctypes){
  ...
  # inf.glmpca <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_cluster/glmpca_split_by_trajs_tss/glmpca.", jctype, ".", jmark, ".bincutoff_0.binskeep_0.platename_batch.szname_none.niter_1000.RData")
  inf.glmpca <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_cluster/glmpca_outputs_split_by_trajs_", jsuffix, "/glmpca.", jctype, ".", jmark, ".bincutoff_0.binskeep_0.platename_plate.szname_none.niter_1000.RData")
  load(inf.glmpca, v=T)
  
  dat.glm <- data.frame(cell = rownames(glm.out$factors), glm.out$factors, stringsAsFactors = FALSE) %>%
    left_join(., dat.meta.filt)
  
  # ggplot(dat.glm, aes(x = dim1, y = dim2, color = colcode)) + 
  #   geom_point() + 
  #   theme_bw() + 
  #   scale_color_identity() + 
  #   ggtitle(paste(jmark, jctype)) + 
