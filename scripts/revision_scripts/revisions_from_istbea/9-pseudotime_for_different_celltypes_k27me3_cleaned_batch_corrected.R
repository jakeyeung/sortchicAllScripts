...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/cleaned2_batch_corrected_eryth_fix"
dir.create(outdir)
outpdf <- file.path(outdir, paste0("trajs_plots_batch_corrected.", jmark, ".", Sys.Date(), ".pdf"))
outrds <- file.path(outdir, paste0("trajs_outputs_batch_corrected.", jmark, ".", Sys.Date(), ".rds"))
...
dat.traj.lst <- lapply(ctypes.names, function(jctype){
  ...
  if (jctype == "Eryths"){
    ...
    dat.glmpca.keep <- dat.glmpca.keep %>%
      rowwise() %>%
      mutate(is.ctype = ifelse(dim2 > 0.01, FALSE, is.ctype))
  }
  ...
  dat.ptime.filt <- data.frame(cell = dat.filt$cell, ptime = dat.filt$dim1, stringsAsFactors = FALSE) %>%
    ungroup() %>%
    mutate(ptime = (ptime - min(ptime)), 
           ptime = ptime / max(ptime))
