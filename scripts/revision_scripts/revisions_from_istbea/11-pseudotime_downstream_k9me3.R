...
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/11-pseudotime_downstream_k4me3.R
# 
...
inf.traj <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/trajs_outputs.", jmark, ".2022-02-06.rds")
...
for (jctype in ctypes.names){
  ...
  inf.fits <- file.path(indir, paste0("ptime_glm_fits_parallel.", jctype, ".", jmark, ".2022-02-06.rds"))
  ...
  dat.traj[[jctype]]$colcode <- sapply(dat.traj[[jctype]]$ctype.from.LL, function(x) AssignHash(x = x, jhash = ctype2col.hash, null.fill = NULL0))
