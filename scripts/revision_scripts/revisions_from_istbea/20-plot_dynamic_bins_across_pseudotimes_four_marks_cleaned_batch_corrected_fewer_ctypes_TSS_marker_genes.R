...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/20-plot_dynamic_bins_across_pseudotimes_four_marks_cleaned_batch_corrected_fewer_ctypes_TSS_marker_genes.R
# 
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots/umap_and_trajs/k27me3_batch_corrected_fewer_ctypes_TSS"
dir.create(outdir)
...
parallel::mclapply(jmarks, function(jmark){
  ...
  for (jctype in jctypes){
    ...
    for (jrname in rows.keep){
      
      jtitle <- paste(jmark, jctype, jrname)
      
      dat.signal <- data.frame(cell = colnames(dat.impute.lst[[jmark]]), 
                               signal = dat.impute.lst[[jmark]][jrname, ], 
