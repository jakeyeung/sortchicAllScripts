...
inf.lda <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/LDA_outputs_first_submission/ldaAnalysisBins_mouse_spikein_BMround2all_MergeWithOld.clusterfilt.2020-11-04/lda_outputs.count_mat_old_merged_with_new.", jmarkold, ".remove_bad_clusters.2020-11-04.K-30.binarize.FALSE/ldaOut.count_mat_old_merged_with_new.", jmarkold, ".remove_bad_clusters.2020-11-04.K-30.Robj")
...
outf.allmerged <- file.path(outdir, paste0("count_mat_merged_with_old_allbins.", jmark, ".", Sys.Date(), ".rds"))
outf.allmerged.fewer <- file.path(outdir, paste0("count_mat_merged_with_old_fewer2_allbins.", jmark, ".", Sys.Date(), ".rds"))
outf.newonly <- file.path(outdir, paste0("count_mat_new_only_allbins.", jmark, ".", Sys.Date(), ".rds"))

outf.allmerged.dynbins <- file.path(outdir, paste0("count_mat_merged_with_old_dynbins.", jmark, ".", Sys.Date(), ".rds"))
outf.allmerged.fewer.dynbins <- file.path(outdir, paste0("count_mat_merged_with_old_fewer2_dynbins.", jmark, ".", Sys.Date(), ".rds"))
...
dat.rz.annot <- dat.rz.annot %>%
  rowwise() %>%
  mutate(is.good = (frac.count.in.peak >= frip.cutoff & total.count >= counts.cutoff & TA.frac >= ta.cutoff) | (ctype == "Eryths" & total.count >= 500) )
...
count.mat.merged.with.old.fewer.allbins <- cbind(count.mat.orig[rows.common, cnames.orig.fewer], mat.merged[rows.common, ])
count.mat.merged.with.old.allbins <- cbind(count.mat.orig[rows.common, ], mat.merged[rows.common, ])
mat.merged.allbins <- mat.merged[rows.common, ]

count.mat.merged.with.old.fewer.dynbins <- count.mat.merged.with.old.fewer.allbins[rows.dynbins, ]
count.mat.merged.with.old.dynbins <- count.mat.merged.with.old.allbins[rows.dynbins, ]
mat.merged.dynbins <- mat.merged.allbins[rows.dynbins, ]

print(dim(count.mat.merged.with.old.allbins))
print(dim(count.mat.merged.with.old.dynbins))
...
saveRDS(count.mat.merged.with.old.fewer.allbins, outf.allmerged.fewer)
saveRDS(count.mat.merged.with.old.allbins, outf.allmerged)
saveRDS(mat.merged.allbins, file = outf.newonly)

saveRDS(count.mat.merged.with.old.fewer.dynbins, outf.allmerged.fewer.dynbins)
saveRDS(count.mat.merged.with.old.dynbins, outf.allmerged.dynbins)
saveRDS(mat.merged.dynbins, file = outf.newonly.dynbins)
