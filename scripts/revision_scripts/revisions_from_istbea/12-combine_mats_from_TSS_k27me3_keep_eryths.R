...
for (jmark in jmarks){
  ...
  mat.with.good.cells.newonly <- readRDS("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/raw_data/count_tables/filtered_count_tables_for_LDA_no_frip_filter_on_eryths/BM_k27me3/count_mat_new_only_dynbins.k27me3.2022-04-12.rds")
  cells.keep.newonly <- colnames(mat.with.good.cells.newonly)
  ...
  count.mat.merged.filt.newonly <- count.mat.merged.filt[, cells.keep.newonly]
  
  print(dim(count.mat.merged.filt))
  print(dim(count.mat.merged.filt.newonly))
  print(length(cells.keep))
  
  # save outputs
  # outrds <- file.path(outdir, paste0("count_mat_TSS_rep2rep3only.", jmark, ".", Sys.Date(), ".rds"))
  outrds <- file.path(outdir, paste0("count_mat_merged_with_old_TSS.", jmark, ".2022-04-12.rds"))
  outrds.newonly <- file.path(outdir, paste0("count_mat_new_only_TSS.", jmark, ".2022-04-12.rds"))
  saveRDS(object = count.mat.merged.filt, file = outrds)
  saveRDS(object = count.mat.merged.filt.newonly, file = outrds.newonly)
