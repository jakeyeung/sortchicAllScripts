...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/20-plot_dynamic_bins_across_pseudotimes_four_marks_cleaned_batch_corrected_fewer_ctypes_fit_cubic_spline.R
# 
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots/umap_and_trajs/k27me3_batch_corrected_fewer_ctypes"
dir.create(outdir)
for (jmark in jmarks){
  print(jmark)
  ...
  for (jctype in jctypes){
    ...
    pdfout <- file.path(outdir, paste0("umap_on_trajs_fix_color_bins_common_root_no_baso_fit_gam_explcit.", jmark, ".", jctype, ".", Sys.Date(), ".pdf"))
    ...
    # # progs <- c("HSCs")
    # dat.hscs <- subset(dat.trajs.long, ctype.from.LL %in% progs)
    # dat.nonhscs <- subset(dat.trajs.long, !ctype.from.LL %in% progs)
    ...
    # fit cubic spline 
    dat.trajs.long.split <- split(dat.commonroot, dat.commonroot$traj)
    jfits.split <- lapply(dat.trajs.long.split, function(jsub){
      jsub$traj <- as.factor(jsub$traj)
      jfit.sub <- mgcv::gam(formula = signal ~ s(ptime, k = 4, bs = "cs", by = traj), gamma = 10, method = "REML", data = jsub)
      jpred <- predict(jfit.sub, se.fit = TRUE)
      jsub$pred <- jpred$fit
      jsub$pred.se <- jpred$se.fit
      # derivatives(jfit.sub)
      return(list(jsub = jsub, fit = jfit.sub))
    }) 
    dat.commonroot.fit <- lapply(jfits.split, function(jout){
      jout$jsub
    }) %>%
      bind_rows()
    
    m.traj.fit.smooth <- ggplot() + 
      geom_point(mapping = aes(x = ptime, y = signal, alpha = jalpha, group = traj, color = colcode), data = dat.trajs.long) + 
      geom_line(mapping = aes(x = ptime, y = pred, group = traj, color = colcodetraj), 
                  data = dat.commonroot.fit, size = 1) +
      scale_color_identity( labels = dat.meta.colors$ctype.from.LL, breaks = dat.meta.colors$colcode,
                            guide = "legend") + 
      theme_bw() + 
      ggtitle(paste(jmark, jctype, "fromGAM")) + 
      theme(aspect.ratio=jratio, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom")
    print(m.traj.fit.smooth)
    
    m.traj.fit.smooth.orig <- ggplot(mapping = aes(x = ptime, y = signal, alpha = jalpha, group = traj)) + 
      ...
      geom_smooth(mapping = aes(color = colcodetraj), 
                  method = "gam", se = TRUE, formula = y ~ s(x, bs = "cs", k = 4),
                  data = dat.commonroot) +
      scale_color_identity( labels = dat.meta.colors$ctype.from.LL, breaks = dat.meta.colors$colcode,
                            guide = "legend") + 
      theme_bw() + 
      ggtitle(paste(jmark, jctype, "from geom_smooth")) + 
      theme(aspect.ratio=jratio, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom")
    print(m.traj.fit.smooth.orig)
    
    
    m.traj.fit <- ggplot(dat.trajs.long, aes(x = ptime, y = signal, alpha = jalpha, group = traj)) + 
      geom_point(mapping = aes(color = colcode)) + 
      # stat_smooth() + 
      # geom_smooth(method = "lm", se = FALSE, formula = y ~ 1 + x + x ^ 2) +
      geom_smooth(mapping = aes(color = colcodetraj), method = "gam", se = FALSE) +
      scale_color_identity( labels = dat.meta.colors$ctype.from.LL, breaks = dat.meta.colors$colcode,
                            guide = "legend") + 
      theme_bw() + 
      ggtitle(paste(jmark, jctype)) + 
    ...
    # Show distribution across celltypes for ctype ----------------------------
    dat.trajs.long.ctype <- subset(dat.trajs.long, traj == jctype) %>%
      filter(ctype.from.LL != "Basophils")
    
    m.dist <- ggplot(dat.trajs.long.ctype, aes(x = ptime, fill = colcode)) + 
      geom_density(alpha = 0.5) + 
      scale_fill_identity( labels = dat.meta.colors$ctype.from.LL, breaks = dat.meta.colors$colcode,
                            guide = "legend") + 
      ggtitle(paste(jmark, jctype)) + 
      theme_bw() + 
