...
outpdf <- file.path(outdir, paste0("imputation_sca1_kit_lin.", Sys.Date(), ".pdf"))
outrdata <- file.path(outdir, paste0("imputation_sca1_kit_lin.", Sys.Date(), ".RData"))
...
dat.sub.impute.knn.lst <- lapply(jmarks, function(jmark){
  ...
  jdat <- jdat %>%
    ...
    # mutate(sca1_knn_impute = ifelse(beyond.cutoff, NA, sca1_knn_impute)) %>%
...
ggplot(dat.sub.impute.knn.lst[[jmark]] %>% arrange(desc(is.na(lin_impute))) %>%
         filter(batch == "New"), 
       aes(x = umap1, y = umap2, color = lin_knn_impute)) + 
  geom_point() + 
  facet_wrap(~experi) + 
  scale_color_viridis_c(na.value = "grey85") + 
  theme_bw() + 
...
jmark <- "k4me1"
mat.input <- dat.sub.impute.knn.lst %>%
  bind_rows() %>%
  # dplyr::select(cell, c(ckit_zscore, sca1_knn_impute, lin_zscore)) %>%
  # dplyr::rename(sca1_zscore = sca1_knn_impute) %>%
  dplyr::select(cell, c(ckit_zscore, sca1_zscore, lin_zscore)) %>%
  as.data.frame()

plot(mat.input$lin_zscore, mat.input$sca1_zscore)
plot(mat.input$lin_zscore, mat.input$ckit_zscore)

plot(mat.input$ckit_zscore, mat.input$sca1_zscore)
plot(mat.input$ckit_zscore, mat.input$lin_zscore)

# min_sca1 <- min(mat.input$sca1_zscore, na.rm = TRUE)
# mat.input <- mat.input %>%
#   rowwise() %>%
#   mutate(sca1_zscore = ifelse(sca1_zscore == min_sca1, NA, sca1_zscore)) %>%
#   as.data.frame()
...
em.out <- norm2::emNorm(mat.input, prior="ridge", prior.df=0.5)
mcmc.out <- norm2::mcmcNorm(mat.input, starting.values = em.out$param)
summary(em.out)
summary(mcmc.out)

impute.out <- as.data.frame(impNorm(em.out))
impute.out.mcmc <- as.data.frame(impNorm(mcmc.out))

plot(impute.out.mcmc$lin_zscore, impute.out.mcmc$sca1_zscore)
plot(impute.out.mcmc$ckit_zscore, impute.out.mcmc$sca1_zscore)
plot(impute.out.mcmc$lin_zscore, impute.out.mcmc$ckit_zscore)

colnames(impute.out) <- paste(colnames(impute.out), "impute", sep = "_")
colnames(impute.out.mcmc) <- paste(colnames(impute.out.mcmc), "impute", sep = "_")
