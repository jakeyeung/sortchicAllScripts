# Jake Yeung
# Date of Creation: 2022-02-03
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/10-fit_pseudotime_glm.R
# 
...
FitGlmTrajs <- function(jrow, cnames, dat.annots, totalcounts.cells){
  ...
  glm.null <- glm(counts ~ 1 + offset(log(totalcounts)), data = dat, family = "poisson")
  glm.complex <- glm(counts ~ 1 + ptime + offset(log(totalcounts)), data = dat, family = "poisson")
  ...
  return(list(glm.null = glm.null, glm.complex = glm.complex, dat.coefs.complex = dat.coefs.complex))
}
...
inf.traj.lst <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/trajs_outputs.", jmark, ".2022-02-03.rds")
...
jctypes <- names(traj.lst)
ncores <- 4

# jctype <- "Granulocytes"
# for (jctype in jctypes){
lapply(jctypes, function(jctype){
  print(jctype)
  outf <- file.path(outdir, paste0("ptime_glm_fits.", jctype, ".", jmark, ".", Sys.Date(), ".rds"))
  ...
  system.time(
    ...
    fits.out.lst <- lapply(jgenes.vec, function(jgene){
      ...
      fit.out <- FitGlmTrajs(jrow = jrow, cnames = cnames, dat.annots = dat.annots, totalcounts.cells = totalcounts.cells)
      return(fit.out)
    # }, mc.cores = ncores)
    })
  )
