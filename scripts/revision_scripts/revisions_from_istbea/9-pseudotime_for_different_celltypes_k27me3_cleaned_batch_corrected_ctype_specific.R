...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/9-pseudotime_for_different_celltypes_k27me3_cleaned_batch_corrected.R
# 
...
inf.matadj <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/batch_effect_corrections/mat_wide_k27me3_batch_corrected.2022-04-19.rds"
mat <- readRDS(inf.matadj)
...
dat.meta.colors <- subset(dat.meta.LL, select = c(ctype.from.LL, colcode))
...
inf.pca <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/batch_effect_corrections/pca_output_mat_adj_irlba_scaled2.", jmark, ".2022-04-20.rds")
...
cnames <- paste0("dim", seq(length(dat.pca$d)))
colnames(dat.pca$u) <- cnames
colnames(dat.pca$v) <- cnames

dat.glmpca <- data.frame(cell = rownames(dat.pca$u), dat.pca$u, stringsAsFactors = FALSE) %>%
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/cleaned2_batch_corrected_byctype"
dir.create(outdir)
outpdf <- file.path(outdir, paste0("trajs_plots_batch_corrected_by_ctype.", jmark, ".", Sys.Date(), ".pdf"))
outrds <- file.path(outdir, paste0("trajs_outputs_batch_corrected_by_ctype.", jmark, ".", Sys.Date(), ".rds"))
...
dat.traj.lst <- lapply(ctypes.names, function(jctype){
  ...
  cells.keep <- subset(dat.glmpca.keep, is.ctype)$cell
  mat.filt <- mat[, cells.keep]
  mat.filt <- t(scale(t(mat.filt), center = TRUE, scale = TRUE))
  mat.pca <- irlba(A = t(mat.filt), nv = 3, scale = FALSE, center = FALSE)
  rownames(mat.pca$u) <- colnames(mat.filt)
  rownames(mat.pca$v) <- rownames(mat.filt)
  cnames <- paste0("dim", seq(length(mat.pca$d)))
  colnames(mat.pca$u) <- cnames
  colnames(mat.pca$v) <- cnames
  
  dat.pca.annot <- data.frame(cell = rownames(mat.pca$u), mat.pca$u, stringsAsFactors = FALSE) %>%
    left_join(., dat.meta.LL)
  
  # dat.glmpca.keep <- dat.glmpca.keep %>%
  #   arrange(is.ctype)
  
  m <- ggplot(dat.pca.annot, aes_string(x = jdim1, y = jdim2, color = "colcode")) + 
    geom_point() + 
    theme_bw() + 
  ...
  jcells <- subset(dat.pca.annot, ctype.from.LL == "HSCs")$cell
  ...
  # plot again
  dat.pca.annot.annot <- dat.pca.annot %>%
    left_join(., dat.ptime.filt)
  
  m <- ggplot(dat.pca.annot.annot, aes_string(x = jdim1, y = jdim2, color = "ptime")) + 
    geom_point() + 
