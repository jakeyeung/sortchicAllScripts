# Jake Yeung
# Date of Creation: 2022-02-09
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/11-pseudotime_downstream_TSS_k27me3.R
# description
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/ptimefits_plots_top_hits_TSS"
dir.create(outdir)
...
for (jmark in jmarks){
  ...
  inf.lda <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_bugfixed_TSS/ldaAnalysis_fripfilt_TSS/ldaOut.count_mat_TSS_rep2rep3only.k27me3.2022-02-08.Robj")
  load(inf.lda, v=T)
  ...
  for (jctype in ctypes.names){
    ...
    topbins <- dat.fits.long$rname[1:topn]
    names(topbins) <- topbins
    
    topbins.genes <- dat.fits.long$gene[1:topn]
