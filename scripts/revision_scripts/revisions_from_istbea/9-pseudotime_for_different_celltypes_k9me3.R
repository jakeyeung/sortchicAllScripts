# Jake Yeung
# Date of Creation: 2022-02-06
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/9-pseudotime_for_different_celltypes_k9me3.R
# description
...
# ctypes.to.annotate <- c("Bcells", "Eryths", "GMP", "Granulocytes", "HSCs", "LT", "MEP", "MPPs", "pDCs", "ST", "Monocytes")
...
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs"
...
dat.traj.lst <- lapply(ctypes.names, function(jctype){
  ...
  #     rowwise() %>%
  #     mutate(is.ctype = ctype %in% ctypes.vec)
  #   dat.glmpca.keep.filt <- dat.glmpca.keep %>%
  #     ungroup() %>%
  #     mutate(ctype.ordered = factor(ctype, levels = ctypes.vec))
  ...
  # for (jdim1tmp in seq(10)){
  #   jdim2tmp <- jdim1tmp + 1
  #   jdim1tmp.str <- paste0("dim", jdim1tmp)
  #   jdim2tmp.str <- paste0("dim", jdim2tmp)
  #   m <- ggplot(dat.glmpca.keep.filt %>% arrange(!is.na(ctype.ordered)), aes_string(x = jdim1tmp.str, y = jdim2tmp.str, color = "ctype.ordered")) + 
  #     geom_point() + 
