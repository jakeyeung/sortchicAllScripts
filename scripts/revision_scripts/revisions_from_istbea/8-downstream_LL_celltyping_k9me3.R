...
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/8-downstream_LL_celltyping_k9me3.R
# 
...
jsuffix <- "dynamicbins"
# indir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_newonly"
indir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping"
...
for (jmark in jmarks){
  ...
  inf <- file.path(indir, paste0("LLmat_dynamicbins_", jmark, ".2022-02-11.rds"))
  ...
  m1 <- ggplot(dat.meta.reannotate %>% filter(batch == "New"), aes(x = umap1, y = umap2, color = ctype)) +
    geom_point() + 
    facet_wrap(~ctype) + 
    theme_bw() + 
