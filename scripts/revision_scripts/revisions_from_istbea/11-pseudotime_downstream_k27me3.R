# Jake Yeung
# Date of Creation: 2022-03-03
# File: ~/projects/scchic/scripts/scripts_analysis/revisions_from_istbea/11-pseudotime_downstream_k27me3.R
# 
...
inf.traj <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/trajs/trajs_outputs.", jmark, ".2022-02-05.rds")
...
dat.meta.full <- fread(file.path("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots/metadata_cleaned_up_update_k27me3_umap", paste0("metadata.", jmark, ".2022-03-01.txt")))
ctype2col.hash <- hash::hash(dat.meta.full$ctype.from.LL, dat.meta.full$colcode)
...
for (jctype in ctypes.names){
  ...
  outpdf.tmp <- file.path(outdir, paste0("plot_top_ptime_hits.", jctype, ".", jmark, ".", Sys.Date(), ".neg_slope.pdf")) 
  outf.tmp <- file.path(outdir, paste0("fits_with_annotations.", jctype, ".", jmark, ".", Sys.Date(), ".neg_slope.txt"))
  
  inf.fits <- file.path(indir, paste0("ptime_glm_fits_parallel.", jctype, ".", jmark, ".2022-02-05.rds"))
  ...
  topbins <- (dat.fits.long %>% arrange(estimate, pval10))$gene[1:topn]
  ...
  dat.traj[[jctype]]$colcode <- sapply(dat.traj[[jctype]]$ctype.from.LL, function(x) AssignHash(x = x, jhash = ctype2col.hash, null.fill = x))
  ...
  m <- ggplot(dat.traj[[jctype]] %>% mutate(colcode = ifelse(!is.na(ptime), colcode, "grey85")), aes_string(x = jdim1, y = jdim2, color = "colcode")) + 
    geom_point() + 
