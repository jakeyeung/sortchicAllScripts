...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/7-annotate_celltypes_from_reference_dynamic_bins_update_ctypes_from_LDA_all_marks_oldannots_k27me3_nobasos.R
# 
...
jmarks <- c("k27me3", "k4me1", "k4me3", "k9me3"); names(jmarks) <- jmarks
jmark <- jmarks[[1]]
...
jsuffix <- "dynamicbins"
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_update_ctypes_from_LDA_oldannot_nobasos"
dir.create(outdir)
# for (jmark in jmarks){

ctypes.to.annotate <- c("Bcells", "CMP", "DCs", "Eryths", "Granulocytes", "HSCs", "MEP", "Monocytes", "MPPs", "NKs", "pDCs")
names(ctypes.to.annotate) <- ctypes.to.annotate

LL.all.lst <- mclapply(jmarks, function(jmark){
  print(jmark)
  
  outrds <- file.path(outdir, paste0("LLmat_by_batch_nobasos_", jsuffix, "_", jmark, ".", Sys.Date(), ".rds"))
