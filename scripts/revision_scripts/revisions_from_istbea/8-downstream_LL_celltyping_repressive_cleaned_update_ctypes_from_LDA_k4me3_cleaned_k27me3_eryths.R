...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/8-downstream_LL_celltyping_repressive_cleaned_update_ctypes_from_LDA_k4me3_cleaned_k27me3_eryths.R
# 
...
jsuffix <- "dynamicbins"
indir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_update_ctypes_from_LDA_k4me3_cleaned_k27me3_eryths"
outdir <- indir

# jmark <- jmarks[[1]]

prog.ctypes <- c("CMP", "MEP", "HSCs", "MPPs", "ST", "LT"); names(prog.ctypes) <- prog.ctypes
...
for (jmark in jmarks){
  ...
  outmeta <- file.path(outdir, paste0("metadata_reannotate_from_LLmat_fix_ctypes_by_batch_", jsuffix, ".", jmark, ".", Sys.Date(), ".txt"))
  outpdf <- file.path(outdir, paste0("plots_reannotate_from_LLmat_fix_ctypes_by_batch_", jsuffix, ".", jmark, ".", Sys.Date(), ".pdf"))
  pdf(outpdf, useDingbats = FALSE)
  
  
  if (jmark != "k27me3"){
    inf <- file.path(indir, paste0("LLmat_by_batch_dynamicbins_", jmark, ".2022-04-13.rds"))
  } else {
    inf <- file.path(indir, paste0("LLmat_by_batch_dynamicbins_", jmark, ".2022-04-15.rds"))
  }
  ...
  if (jmark == "k4me1"){
    ...
    # inf.meta <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping/metadata_reannotate_from_LLmat_dynamicbins.k4me1.2022-02-03.txt"
    ...
    # inf.ldaout <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_repressive_cleaned_from_jupyter/ldaAnalysis_fripfilt_varfilt_binfilt/lda_outputs.count_mat_cleaned_dynbins.", jmark, ".2022-02-16/ldaOut.count_mat_cleaned_dynbins.", jmark, ".2022-02-16.Robj")
    indir.meta <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/celltyping/repressive_cleaned_clean_eryths"
    inf.meta <- file.path(indir.meta, paste0("metadata_celltyping_", jmark, ".dynamicbins.2022-04-15.txt"))
    # inf.ldaout <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_repressive_cleaned_from_jupyter/ldaAnalysis_fripfilt_varfilt_binfilt/lda_outputs.count_mat_cleaned_dynbins.", jmark, ".2022-02-16/ldaOut.count_mat_cleaned_dynbins.", jmark, ".2022-02-16.Robj")
    inf.ldaout <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_k27me3_clean_eryths/lda_outputs.count_mat_merged_with_old_dynbins.k27me3.2022-04-12/ldaOut.count_mat_merged_with_old_dynbins.", jmark, ".2022-04-12.Robj")
  } else if (jmark == "k9me3"){
  ...
  if ("ctype.from.LL" %in% colnames(dat.meta)){
    dat.meta <- dat.meta %>%
  ...
  assertthat::assert_that(nrow(dat.LL.best) == nrow(dat.meta))
  
  dat.meta.reannotate <- dat.meta %>%
    left_join(., dat.LL.best)
  
  table(dat.meta.reannotate$ctype)
  unique(dat.meta.reannotate$ctype.from.LL)
...
# Adjusts -----------------------------------------------------------------
  ...
  dat.meta.reannotate <- dat.meta.reannotate %>%
    ...
    mutate(ctype.from.LL = ifelse(ctype == "MEP", ctype, ctype.from.LL),
           ...
           ctype.from.LL = ifelse(ctype == "ST", ctype, ctype.from.LL), 
           ctype.from.LL = ifelse(ctype == "LT", ctype, ctype.from.LL))
...
# End adjusts -------------------------------------------------------------
  ...
  jsub <- dat.meta.reannotate %>%
    mutate(highlight = umap2 < -20 & umap1 < -5 & ctype.from.LL != "Monocytes" & ctype.from.LL != "NKs")
  
  m1.checks <- ggplot(jsub
