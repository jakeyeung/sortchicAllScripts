...
jsuffix <- "dynamicbins"
outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_update_ctypes_from_LDA_k4me3_cleaned_k27me3_eryths2_with_refmat"
dir.create(outdir)
# for (jmark in jmarks){
...
LL.all.lst <- mclapply(jmarks, function(jmark){
  ...
  outref <- file.path(outdir, paste0("refmat_by_batch_", jsuffix, "_", jmark, ".", Sys.Date(), ".rds"))
  ...
  saveRDS(ref.probs.mat, file = outref)
