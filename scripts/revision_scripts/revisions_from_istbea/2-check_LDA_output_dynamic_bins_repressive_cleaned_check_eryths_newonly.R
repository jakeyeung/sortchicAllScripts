...
# File: ~/projects/scchic/scripts/revision_scripts/revisions_from_istbea/2-check_LDA_output_dynamic_bins_repressive_cleaned_check_eryths.R
# 
...
# jsuffix1 <- "cleaned_check_eryths"
jsuffix1 <- "cleaned_check_eryths_newonly"
jsuffixbins <- "dynamicbins"
# jsuffixbins <- "allbins"

outdir <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/metadata/repressive_cleaned_check_eryths"
dir.create(outdir)

jmarks <- c("k27me3", "k9me3"); names(jmarks) <- jmarks
jmarks.old <- c("H3K27me3", "H3K9me3"); names(jmarks.old) <- jmarks
...
# indir.meta.colors <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/multinom_celltyping_update_ctypes"
# indir.meta.colors <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots/metadata_cleaned_up"

indir.meta <- "/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/from_jupyterhub/primetime_plots"

dat.meta.var.lst <- parallel::mclapply(jmarks, function(jmark){
  print(jmark)
  jmark.old <- jmarks.old[[jmark]]
  ...
  # inf.meta <- file.path(indir.meta, paste0("metadata_with_colors.", jmark, ".2022-02-13.txt"))
  inf.meta.old <- paste0("/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/first_submission_data/metadata/metadata_batch_corrected.arranged_by_lineage.", jmark.old, ".2021-01-02.txt")
  
  if (jmark.old == "H3K27me3"){
    dat.meta.old <- fread(inf.meta.old) %>%
      dplyr::select(cell, ctype)
  } else if (jmark.old == "H3K9me3"){
    dat.meta.old <- fread(inf.meta.old) %>%
      dplyr::select(cell, cluster) %>%
      dplyr::rename(ctype = cluster)
  } else {
    print("error")
  }
  ...
  dname <- paste0("count_mat_cleaned_dynbins.newonly.", jmark, ".2022-04-05")
  ...
  dat.umap.init <- DoUmapAndLouvain(tm$topics, jsettings = jsettings)
  dat.umap <- dat.umap.init %>%
    left_join(., dat.meta.old) %>%
    rowwise() %>%
  ...
  m <- ggplot(dat.umap, aes(x = umap1, y = umap2, color = ctype)) + 
    geom_point() + 
    theme_bw() + 
    ggtitle(jmark) + 
    theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  ...
  m.old <- ggplot(dat.umap %>% filter(batch == "Old"), aes(x = umap1, y = umap2, color = ctype)) + 
    geom_point() + 
    theme_bw() + 
  ...
  m.new <- ggplot(dat.umap %>% filter(batch == "New"), aes(x = umap1, y = umap2, color = ctype)) + 
    geom_point() + 
    theme_bw() + 
  ...
  m <- ggplot(dat.umap, aes(x = umap1, y = umap2, color = batch)) + 
    geom_point() + 
    facet_wrap(~ctype) + 
    theme_bw() + 
    ggtitle(jmark) + 
  ...
  fwrite(dat.meta.new.var, file = outf.meta, sep = "\t")
