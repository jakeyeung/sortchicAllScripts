...
for (jmark in jmarks){
  ...
  infmain <- file.path(hubprefix, paste0("jyeung/data/scChiC/cut_distances_all_clusters2.rerun.bedfile_Heterochromatin_winsize_", jwinsize, ".", jtopn, ".withraw.fourCtypes"))
  assertthat::assert_that(dir.exists(infmain))
  outdir <- file.path(hubprefix, paste0("jyeung/data/scChiC/from_rstudioserver/pdfs_all/cut_distances.Heterochromoatin.winsize_", jwinsize, ".", jtopn, ".withraw.fourCtypes"))
  dir.create(outdir)
  ...
  indirs.base <- list.files(infmain, pattern = paste0(jmark))
  ...
  #   }
    ...
    outpdf <- file.path(outdir, paste0("distances_summary_BM.", jmark, ".pdf"))
    ...
    inf.counts.dirs <- list.files(infmain, pattern = paste0(jmark, "-BM_AllMerged."), full.names = TRUE)
    names(inf.counts.dirs) <- sapply(inf.counts.dirs, function(x) strsplit(basename(x), split = "\\.")[[1]][[2]])
