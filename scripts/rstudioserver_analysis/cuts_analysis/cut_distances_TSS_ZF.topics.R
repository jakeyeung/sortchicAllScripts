...
# gsets <- c("Neutrophil", "HSPCs", "Bcell", "Erythro")
gsets <- c("Granus", "HSPCs", "Bcells", "Eryths")
jpairs <- list(c("eryth", "Eryths"), c("granu", "Granus"), c("HSPCs", "HSPCs"), c("lymph", "Bcells"))
...
# outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/pdfs_from_WKM_BM_merged/cut_distances_topics_ZF"
ntopics <- 1000
outdir <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/pdfs_from_WKM_BM_merged/cut_distances_topic_top_", ntopics, "_ZF")
dir.create(outdir)

# indir.geneset <- "/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/from_rstudioserver/bedannotations/MouseBMFromRNAseq"
indir.geneset <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/from_rstudioserver/bedannotations/ZebrafishWKMFromTopics.", ntopics)
for (jmark in jmarks){
  ...
  for (jpair in jpairs){
    ...
    cutsout <- lapply(jstrands, function(jstrand){
      # inname <- paste0(jmark, "-BM_AllMerged.", jctype, ".sorted.cleaned.TSS_cuts.", jstrand, ".mat.gz")
      inname <- paste0("PZ-ChIC-ZF_", jmark, "_2020-04-07.", jctype, ".sorted.TSS_cuts.radius_", jrad, ".", jstrand, ".mat.gz")
      ...
      indir <- file.path(hubprefix, paste0("jyeung/data/zebrafish_scchic/bams_tagged_merged_by_marks.split_by_clusters.imputevarfilt.lessstringent.mapq_40.FewerClusters.2020-06-17/cut_positions_TSS_rad_", jrad, "_", jstrand))
    ...
    dat.smooth.long <- lapply(gsets, function(jgset){
      ...
      # inf.geneset <- paste0("MouseBM_TSS.", jgset, ".bsize_2.bed")
      inf.geneset <- file.path(indir.geneset, paste0("ZebrafishWKM_TSS_FromTopics.", jgset, ".bsize_2.bed"))
