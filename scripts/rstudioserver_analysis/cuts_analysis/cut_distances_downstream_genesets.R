...
for (jmark in jmarks){
  ...
  infmain <- file.path(hubprefix, paste0("jyeung/data/scChiC/cut_distances_all_clusters2.rerun.bedfile_Topics_winsize_50000.withraw"))
  assertthat::assert_that(dir.exists(infmain))
  outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/cut_distances.genesets"
  dir.create(outdir)
  ...
  for (ctype in ctypes){
    ...
    inf.counts.dirs <- list.files(infmain, pattern = paste0(jmark, "-BM_AllMerged.", ctype), full.names = TRUE)
