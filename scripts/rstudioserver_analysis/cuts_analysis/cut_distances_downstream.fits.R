...
ClipSeries <- function(y, mindist = 20, prob = 0.95, replace.with.NA = TRUE){
  # set ymax
  ymax <- max(y[mindist:length(y)])
  # get x cutoff
  xcutoff <- min(which(y[1:mindist] < ymax))  # index 
  if (is.infinite(xcutoff)){
    return(y)
  }
  # print(xcutoff)
  # print(length(y))
  yfilt <- y
  if (is.na(replace.with.NA)){
    yfilt[1:xcutoff] <- NA
  } else {
    yfilt[1:xcutoff] <- ymax
  }
  return(yfilt)
}
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/cut_distances.withfreq2"
dir.create(outdir)
...
jout2 <- lapply(jmarks, function(jmark){
  ...
  # jdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/cut_distances_all_clusters2.rerun.withraw/H3K27me3-BM_AllMerged.HSCs-Tead1-_topic9.sorted"
  # jmark <- "H3K27me3"
  ...
  jout <- lapply(inf.counts.dirs, function(jdir){
    ...
    # remove cells with nearly all zeros 
    frac.zeros <- apply(jcounts, MARGIN = 2, function(jcol) Matrix::nnzero(jcol) / nrow(jcounts))
    
    jcutoff <- 0.5
    cells.keep <- which(frac.zeros < jcutoff)
    jcounts <- jcounts[, cells.keep]
    ...
    jcounts.filt2 <- apply(jcounts.filt, MARGIN = 2, FUN = ClipSeries)
    jcounts.filt2 <- DescTools::Winsorize(jcounts.filt2, probs = c(0, 0.95))
    
    jcounts.filt3 <- jcounts.filt2[1:1500, ]
    
    
    # Fourier transform each cell, maybe we can determine good and bad cells
    
    # do one: pick one with lots of counts
    
    # jcells.tmp <- colnames(jcounts.filt)
    # dat.var.filt.tmp <- subset(dat.var.lst[[jmark]], cell %in% jcells.tmp) %>%
    #   arrange(desc(totalcounts))
    # jcell.tmp <- dat.var.filt.tmp$cell[[1]]
    # plot(jcounts.filt2[, jcell.tmp])
    ...
    heatmap3::heatmap3(t(jcounts.filt2), Rowv = NULL, Colv = NA, scale = "none", RowSideColors = colvec, RowSideLabs = "IntrachromVar", margin = c(5, 20), col = viridis::viridis(50), method = 'ward.D', main = paste("Spatial domain", jtitle))
    heatmap3::heatmap3(t(jcounts.filt2), Rowv = NULL, Colv = NA, scale = "none", RowSideColors = colvec.totalcuts, RowSideLabs = "TotalCuts", margin = c(5, 20), col = viridis::viridis(50), method = 'ward.D', main = paste("Spatial domain", jtitle))
    ...
    # do fourier cell by cell 
    fout.all.lst <- apply(jcounts.filt3, 2, function(jcol){
      DoFFT(jcol)
    })
    fout.mat <- lapply(fout.all.lst, function(jdat){
      jdat$amp
    })
    fout.mat <- do.call(cbind, fout.mat)
    rownames(fout.mat) <- signif(fout.all.lst[[1]]$period, 4)
    fout.mat <- fout.mat[, which(apply(fout.mat, 2, var) > 0)]
    
    fout.mat <- sweep(fout.mat, MARGIN = 2, STATS = colSums(fout.mat), FUN = "/")
    
    
    jcells3 <- colnames(fout.mat) 
    dat.var.sub3 <- as.data.frame(subset(dat.var.lst[[jmark]], cell %in% jcells3))
    rownames(dat.var.sub3) <- dat.var.sub3$cell
    dat.var.sub3 <- dat.var.sub3[jcells3, ]
    colvec <- viridis::viridis(100, direction = 1)[as.numeric(cut(dat.var.sub3$cell.var.within.sum.norm, breaks = 100))]
    colvec.totalcuts <- viridis::viridis(100, direction = 1)[as.numeric(cut(log(dat.var.sub3$totalcounts), breaks = 100))]
    
    heatmap3::heatmap3(t(fout.mat[2:25, ]), Rowv = NULL, Colv = NA, RowSideColors = colvec.totalcuts, RowSideLabs = "TotalCuts", scale = "none", margin = c(5, 20), col = viridis::viridis(50), method = 'ward.D', main = paste("Frequency domain", jtitle))
    
    # plot averaged signal add sinewave
    
    # https://mathematica.stackexchange.com/questions/123884/why-fourier-transform-is-symmetric-no-matter-what-interval-i-choose
    y <- rowMeans(jcounts.filt3)
    yt <- fft(y)[1:(length(y) / 2)]
    xt <- (seq(yt) - 1) / length(y)
    # plot(xt[2:100], Mod(yt[2:100]), type = "o", )
    
    fout <- data.frame(k = seq(yt) - 1, f = xt, y = yt, amp = Mod(yt), period = 1 / xt, stringsAsFactors = FALSE)
    
    # get peak 
    fout.sub <- fout %>% filter(k > 0 & k < 100) %>%
      arrange(desc(Mod(y)))
    jtitle0 <- c("period", "freq", "k")
    jtitle1 <- c(signif(fout.sub$period[[1]], 4), signif(fout.sub$f[[1]], 1), fout.sub$k[[1]])
    jtitle.fft <- paste(jtitle, paste(jtitle0, jtitle1, sep = ":"), collapse = ", ")
    
    # fit a sine wave to show period
    jperiod <- fout.sub$period[[1]]
    w <- 2 * pi / jperiod
    jfit <- lm(formula = counts.norm.mean ~ 1 + cos(w * distance) + sin(w * distance), data = jcounts.sum %>% filter(distance > 20 & distance < 1000))
    
    jint <- coefficients(jfit)[[1]]
    cospart <- coefficients(jfit)[[2]]
    sinpart <- coefficients(jfit)[[3]]
    
    jphase <- atan2(sinpart, cospart)
    jamp <- sqrt(cospart ^ 2 + sinpart ^ 2)
    
    jtitle0.fit <- c("amp", "phase", "period")
    jtitle1.fit <- c(signif(jamp, 2), signif(jphase, 2), signif(jperiod, 4))
    jtitle.fit <- paste(jtitle, paste(jtitle0.fit, jtitle1.fit, sep = ":"), collapse = ", ")
      
    jcounts.sum$pred <- predict(object = jfit, newdata = jcounts.sum)
    
    m.freq <- ggplot(fout %>% filter(k > 0 & k < 100), aes(x = f, y = Mod(y))) + 
      geom_point() + geom_line()  + 
      theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +  
      ggtitle(jtitle.fft) + xlab("Frequency [k/N])") + ylab("Amplitude")
    print(m.freq)
    
    m.pred <- ggplot(jcounts.sum %>% filter(distance > 20), aes(x = distance, y = counts.norm.wins.mean, 
                                                                ymin = counts.norm.wins.mean - 1 * counts.norm.wins.se, ymax = counts.norm.wins.mean + 1 * counts.norm.wins.se)) + 
      geom_line() + 
      geom_ribbon(alpha = 0.25, fill = 'green') + 
      theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
      ...
      ylab("Normalized counts") + 
      ggtitle(jtitle.fit) + 
      geom_line(mapping = aes(x = distance, y = pred), linetype = "dotted", color = 'blue') 
    print(m.pred)
    return(jcounts.filt3)
  })
  dev.off()
})
