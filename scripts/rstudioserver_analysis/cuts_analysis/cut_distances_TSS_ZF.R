...
jpairs <- list(c("eryth", "Erythro"), c("granu", "Neutrophil"), c("HSPCs", "HSPCs"), c("lymph", "Bcell"))
...
outdir <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/pdfs_from_WKM_BM_merged/cut_distances_gsets.radius_", jrad, ".ZF")
dir.create(outdir)

# indir.geneset <- "/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/from_rstudioserver/bedannotations/MouseBMFromRNAseq"
indir.geneset <- "/home/jyeung/hub_oudenaarden/jyeung/data/WKM_BM_merged/from_rstudioserver/bedannotations/ZebrafishWKM"
for (jmark in jmarks){
  ...
  for (jpair in jpairs){
    ...
    dat.smooth.long <- lapply(gsets, function(jgset){
      ...
      inf.geneset <- file.path(indir.geneset, paste0("ZebrafishWKM_TSS.", jgset, ".bsize_2.bed"))
