...
for (jdist in jdists){
  ...
  for (jnorm in jnorms){
    ...
    dir.create(outdir)
    outfits <- file.path(outdir, paste0("fit_poisson_model_on_TSS.MouseBM.NormMeth_", jnorm, ".bsize_", jdist, ".CleanUpErythss.RData"))
    ...
    dat.annots.filt.forfit <- lapply(jmarks, function(jmark){
      jdat <- dat.annots.filt[[jmark]]
      jdat <- subset(jdat, select = c(cell, cluster.new)) %>%
        rowwise() %>%
        mutate(cond = GetCondFromSamp(samp = cell, mark = jmark)) %>%
        mutate(Cluster = ifelse(cluster.new == "HSPCs", "aHSPCs", as.character(cluster.new)))  # set HSPC as intercept 
      # remove Erythroblasts that are in StemCell condition
      jdat <- subset(jdat, !(Cluster == "Erythroblasts" & cond == "StemCell"))
      return(jdat)
    })
