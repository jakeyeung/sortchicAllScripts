...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/BM_all_merged_from_hiddendomains/2-downstream_GLMPCA_make_exprs_mat_for_MARA_uncentered_LDA_outputs.R
# description
...
outdir.suffix <- "count_mats_peaks_unnorm_LDA"
...
for (jmark in jmarks){
  ...
  outname <- paste0(outdir.suffix, "_", jbase, ".txt")
  outnamepdf <- paste0("countmat_", jbase, ".pdf")
  outdir <- paste0("/home/jyeung/hpc/scChiC/mara_analysis_BM-AllMerged_Peaks_1000/", jmark, "/mara_input/", outdir.suffix)
  ...
  # dat.impute.log
  rnames <- rownames(dat.impute.log)
  rnames.strip <- sapply(rnames, function(x) strsplit(x, ";")[[1]][[1]])
  dat.impute.log.output <- data.frame(Gene.ID = rnames.strip, dat.impute.log, stringsAsFactors = FALSE)
  
  
  # fwrite(dat.impute.glm.output, file = outf, row.names = FALSE, sep = "\t")
  fwrite(dat.impute.log.output, file = outf, row.names = FALSE, sep = "\t")
