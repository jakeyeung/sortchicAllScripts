...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/BM_all_merged_from_hiddendomains/2-downstream_GLMPCA_make_pdfs.R
# 
...
for (jmark in jmarks){
  ...
  outdir <- paste0("/home/jyeung/hpc/scChiC/from_rstudioserver/glmpca_analyses/GLMPCA_outputs.fromHiddenDomains")
...
# jgene <- "S100a7a"
# jgene <- "Sox6"
# x <- glm.out$loadings[grepl(jgene, rownames(glm.out$loadings)), ]
# 
# # jterm <- "chr3:90644000-90717000;S100a7a"
# (jterm <- rownames(x)[[1]])
# 
# # plot gene expression on umap , raw
# # exprs.vec <- data.frame(cell = colnames(count.mat.hd), count.norm = log2(count.mat.hd[jterm, ] / colSums(count.mat.hd) + 1), stringsAsFactors = FALSE)
# exprs.vec <- data.frame(cell = colnames(count.mat.hd), count.norm = count.mat.hd[jterm, ], stringsAsFactors = FALSE)
# 
# jmerge <- left_join(dat.umap.hd.lda.var2, exprs.vec)
# 
# ggplot(jmerge, aes(x = umap1, y = umap2, color = count.norm)) + 
#   geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
#   scale_color_viridis_c(direction = 1) + 
#   facet_wrap(~plate) + ggtitle('LDA hd, var from bins')
# 
