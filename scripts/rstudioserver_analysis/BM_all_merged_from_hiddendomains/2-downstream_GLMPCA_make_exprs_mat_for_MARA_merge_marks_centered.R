...
dat.impute.merged <- Reduce(function(...) merge(..., all = TRUE), glmpca.imputes)
...
mat.merged.centered <- sweep(mat.merged, MARGIN = 1, STATS = rowMeans(mat.merged), FUN = "-")

geneids <- as.character(dat.impute.merged[, 1])
geneids <- sapply(geneids, function(x) strsplit(x, ";")[[1]][[1]])
mat.merged.centered.output <- data.frame(Gene.ID = geneids, mat.merged.centered, stringsAsFactors = FALSE)

# write to output
fwrite(mat.merged.centered.output, file = outf, row.names = FALSE, sep = "\t")
