...
outpdf <- file.path(outdir, paste0(jmark, "_top_DE_genes_TSS_signal_across_pseudobulks.pdf"))
...
for (jclust in jclusts){
  ...
  m <- ggplot(exprs.sub.long, aes(x = value, group = key, fill = key)) + geom_density(alpha = 0.5) + 
    theme_bw() + theme(aspect.ratio=0.2, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "none") + 
    scale_color_manual(values = cbPalette) + facet_wrap(~key, ncol = 2) + ggtitle(paste("Top DE genes (n=", length(genes.keep), ") for ", jclust))
  print(m)
}
dev.off()
