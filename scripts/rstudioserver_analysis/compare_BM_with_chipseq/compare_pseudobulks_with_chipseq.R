...
# jsuffix <- ""
indir <- paste0("/home/jyeung/hpc/scChiC/raw_data/ZellerRawData_B6_All_MergedByMarks_final.bigwigs_by_cluster.compare_with_chipseq.", jmark, jsuffix)
...
pdf(paste0("/home/jyeung/hpc/scChiC/from_rstudioserver/pdfs_all/correlations/correlate_", jmark, "_with_Lara-Astiaso", jsuffix, ".pdf"), useDingbats = FALSE)
...
for (jctype in jctypes){
  ...
  m <- ggplot(jsub, aes(x = forcats::fct_reorder(.f = ctype.compare, .x = pcorr, .desc = TRUE), y = pcorr, fill = ctype.compare)) + 
    ...
    ggtitle(paste(jmark, "reference ChIP-seq:", jctype))
  print(m)
}
dev.off()  
