...
inf <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/bigwig_outputs/merged_bams.deeptools_outputs.tss.MAPQ_40.dist_10000.allctypes_from_seurat.H3K27me3.bsize_100/computeMatrix.MAPQ_40.H3K27me3.gene_tss_winsize.2.diff_exprs_Giladi_seurat.tab.gz"
meta <- inf2dic(inf)
...
# genes.keep <- subset(dat.annot, cluster == "Ltf" & p_val_adj < 0.01 & avg_logFC < 0.5)$gene
# genes.keep <- subset(dat.annot, cluster == "Ltf" & p_val_adj < 0.01 & avg_logFC > 0.5)$gene
genes.keep <- subset(dat.annot, cluster == "core" & p_val_adj < 0.01 & avg_logFC > 0)$gene
# genes.keep <- subset(dat.annot, cluster == "Cd74" & p_val_adj < 0.01 & avg_logFC > 0)$gene
# genes.keep <- subset(dat.annot, cluster == "Fcrla" & p_val_adj < 0.01 & avg_logFC < 0.1)$gene
# genes.keep <- subset(dat.annot, cluster == "Car1" & p_val_adj < 0.01 & avg_logFC > 0.1)$gene
...
exprs.sub.long <- tidyr::gather(as.data.frame(exprs.sub)) %>%
  group_by(key)
  # mutate(value = scale(value, center = TRUE, scale = TRUE))

cbPalette <- c("#696969", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#006400", "#FFB6C1", "#32CD32", "#0b1b7f", "#ff9f7d", "#eb9d01", "#7fbedf")
# ggplot(exprs.sub.long, aes(x = value, group = key, fill = key)) + geom_density(alpha = 0.5) + 
...
ggplot(exprs.sub.long, aes(x = value, group = key, fill = key)) + geom_density(alpha = 0.5) + 
  ...
  scale_color_manual(values = cbPalette) + facet_wrap(~key, nrow = 1)
...
ggplot(exprs.sub.long %>% filter(!grepl("HSC|Linneg", key)), aes(x = value, group = key, fill = key)) + geom_density(alpha = 0.5) + 
  ...
  scale_color_manual(values = cbPalette)  + facet_wrap(~key)
