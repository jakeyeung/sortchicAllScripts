...
# jsuffix <- ".TSS"
jsuffix <- ".genebody"
indir <- file.path(jprefix, paste0("data/zebrafish_scchic/LDA_outputs/ldaAnalysisBins_ZF_AllMerged", jsuffix))
...
infs <- lapply(jmarks, function(jmark){
  if (jsuffix == ""){
    ...
    # fname <- paste0("lda_outputs.counts_table_var_filt.", jmark, ".K-30.binarize.FALSE/ldaOut.counts_table_var_filt.", jmark, ".K-30.Robj")
    # fname <- paste0("lda_outputs.count_mat.", jmark, ".countcutoffmin_1000-500-1000-1000.TAcutoff_0.5.chrfilt.bfilt.K-30.binarize.FALSE/ldaOut.count_mat.", jmark, ".countcutoffmin_1000-500-1000-1000.TAcutoff_0.5.chrfilt.bfilt.K-30.Robj")
    fname <- paste0("lda_outputs.PZ-ChIC-ZF_", jmark, "_2020-04-07.countTable", jsuffix, ".cellfilt.K-30.binarize.FALSE/ldaOut.PZ-ChIC-ZF_", jmark, "_2020-04-07.countTable", jsuffix, ".cellfilt.K-30.Robj")
    # fname <- paste0()
  }
...
# Load and plot  ----------------------------------------------------------
 ...
 for (keepn in keepnvec){
   for (jmark in jmarks){
     ...
     # annot.out <- list()
     # annot.oust$terms.annot <- melt(terms.mat.tmp)
