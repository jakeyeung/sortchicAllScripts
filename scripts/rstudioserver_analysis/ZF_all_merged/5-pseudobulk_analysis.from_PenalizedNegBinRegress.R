...
for (jmark in jmarks){
  if (bad.ctype.grep == ""){
    pdfout <- file.path(outdir, paste0("pseudobulks.", btypes.keep.str, ".AllCtypes.", jmark, ".", Sys.Date(), ".pdf"))
  }  else {
    pdfout <- file.path(outdir, paste0("pseudobulks.", btypes.keep.str, ".filter_", bad.ctype.grep.str, ".", jmark, ".", Sys.Date(), ".pdf"))
  }
  ...
  m <- ggplot(dat.sum.long.gsets, aes(x = logexprs, group = btype, fill = btype)) + geom_density(alpha = 0.25) + 
    facet_grid(ctype ~ geneset, switch = "y") + 
