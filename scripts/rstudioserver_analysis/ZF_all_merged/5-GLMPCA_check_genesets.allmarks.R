...
eryth.louv.lst <- list(H3K4me1 = c("5"), H3K4me3 = c("5"), H3K27me3 = c("1", "4"), H3K9me3 = c("4"))
jtopics.lst.lst <- list(H3K4me1 = list("monocytes" = c("topic16"),
                                       "eryth" = c("topic14"),
                                       "lymph" = c("topic11", "topic29"),
                                       "HSCs" = c("topic30"),
                                       "thrombocytes" = c("topic8")),
                        H3K4me3 = list("monocytes" = c("topic10"),
                                       "eryth" = c("topic14"),
                                       "lymph" = c("topic4", "topic18"),
                                       "HSCs" = c("topic29")),
                        H3K27me3 = list("monocytes" = c("topic1"),
                                        "eryth" = c("topic22"),
                                        "lymph" = c("topic23", "topic14"),
                                        "HSCs" = c("topic6"),
                                        "thrombocytes" = c("topic8")),
                        H3K9me3 = list("monocytes" = c("topic1"),
                                       "eryth" = c("topic22"),
                                       "lymph" = c("topic23", "topic14"),
                                       "HSCs" = c("topic6"),
                                       "thrombocytes" = c("topic8")))
louv2ctype.lst <- list(H3K4me1 = hash("louvain1" = "lymph1",
                                      "louvain2" = "lymph2",
                                      "louvain3" = "monocyte",
                                      "louvain4" = "Unknown",
                                      "louvain5" = "eryth",
                                      "louvain6" = "thrombo",
                                      "louvain7" = "HSC"),
                       H3K4me3 = hash("louvain1" = "monocyte",
                                      "louvain2" = "Unknown",
                                      "louvain3" = "lymph",
                                      "louvain4" = "HSC",
                                      "louvain5" = "eryth"),
                       H3K27me3 = hash("louvain1" = "eryth1",
                                       "louvain2" = "HSC1",
                                       "louvain3" = "HSC2",
                                       "louvain4" = "eryth2",
                                       "louvain5" = "lymph",
                                       "louvain6" = "monocyte"),
                       H3K9me3 = hash("louvain1" = "louvain1",
                                      "louvain2" = "louvain2",
                                      "louvain3" = "louvain3_HSCs",
                                      "louvain4" = "louvain4_eryth",
                                      "louvain5" = "louvain5",
                                      "louvain6" = "louvain6"))

for (jmark in jmarks){
  print(jmark)
  eryth.louv <- eryth.louv.lst[[jmark]]
  jtopics.lst <- jtopics.lst.lst[[jmark]]
  louv2ctype <- louv2ctype.lst[[jmark]]
  ...
  if (file.exists(outf) & file.exists(outpdf)){
    print(paste("Output files already exist, skipping"))
    print(outf)
    print(outpdf)
    next
  }
  ...
  m.after.tss.filt.conds <- ggplot(dat.umap %>% filter(!louvain %in% eryth.louv), aes(x = umap1, y = umap2, color = tss.cuts / total.cuts)) + geom_point()  + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    scale_color_viridis_c() + facet_wrap(~cond)
  
  m.after.tss.filt.plates <- ggplot(dat.umap %>% filter(!louvain %in% eryth.louv), aes(x = umap1, y = umap2, color = tss.cuts / total.cuts)) + geom_point()  + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    scale_color_viridis_c() + facet_wrap(~plate)
