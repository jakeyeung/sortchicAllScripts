...
for (jmark in jmarks){
  ...
  for (jctype in jctypes){
    ...
    peak.pbulk.ctypefilt <- as.data.frame(peak.pbulk[coord.keep, ])
    ...
    dat.pbulk.ctype <-  data.frame(coord = rownames(peak.pbulk.ctypefilt), ctype.count = peak.pbulk.ctypefilt[[jctype]], stringsAsFactors = TRUE) %>%
      ...
      left_join(., dat.annot.lst[[jmark]]) %>%
      ungroup() %>%
      mutate(gene.label = ifelse(rank(-ctype.count) <= 50, gene, NA))
    ...
    pdf(paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/peaks_analysis/peaks_counts_by_distance.", jctype, ".", jmark, ".pdf"), useDingbats = FALSE)
    print(m1)
    print(m2)
    dev.off()
  }
