...
outprefix <- file.path(outdir, paste0("count_pseudobulks_enhs_proms_genebody.dist_", jdist, ".", jsuffix, ".zscores.2020-04-02.debug"))
...
if (!overwrite){
  assertthat::assert_that(!file.exists(outrdata))
    ...
    counts.pbulk.long.lst <- lapply(jmarks, function(jmark){
      jsub <- CalculateZscoresAndIsCtype(counts.pbulk.long.lst[[jmark]])
      return(jsub)
    })
    
    counts.pbulk.lst.allpseudos <- lapply(jmarks, function(jmark){
      jsub <- CalculateZscoresAndIsCtype(counts.pbulk.lst.allpseudos[[jmark]])
      return(jsub)
    })
