...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/stemcell_analysis/multiomics_integration_TSS_and_enhancers_merge_clusters_with_genebody_cleaner_SaveObjs.R
# description
# description
...
AddGeneSetAndZscores <- function(jdat, gene2ctype, m2c){
  ...
  jdat <- jdat %>%
    ...
    group_by(region_coord_full, mark, de.ctype.choose, biotype) %>%  # add zscore and logFC here
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/rdata_robjs"
outprefix <- file.path(outdir, paste0("count_pseudobulks_enhs_proms_genebody.zscores.2020-04-02"))
    ...
    if (!overwrite){
      if (file.exists(outrdata)){
        print(paste("outf exists, skipping:", outrdata))
        next
      }
    }
    ...
    counts.pbulk.long <- data.table::melt(data.frame(region_coord_full = rownames(count.mat.pbulk.all.norm.wide), 
      ...
      left_join(., subset(bins.annot$out2.df.closest, select = c(region_coord, gene, dist.to.tss))) %>%
      mutate(abs.dist.to.tss = abs(dist.to.tss)) %>%
      filter(!is.na(gene))
    counts.pbulk.long <- AddGeneSetAndZscores(counts.pbulk.long, gene2ctype, m2c)
    ...
    counts.pbulk.long.lst <- lapply(jmarks, function(jmark){
      ...
      jlong <- data.table::melt(data.frame(region_coord_full = rownames(jmat), 
        ...
        mutate(abs.dist.to.tss = abs(dist.to.tss)) %>%
        filter(!is.na(gene))
    jlong <- AddGeneSetAndZscores(jlong, gene2ctype, m2c)
    return(jlong)
    })
    ...
    save(biotype.dat, bins.annot, count.mat.pbulk.all.norm.wide, counts.pbulk.long.lst, counts.pbulk.lst.allpseudos, ncuts.dat.lst, file = outrdata)
#   }
# }
