...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/stemcell_analysis/redo_multiomics_integration_TSS_pseudobulk.R
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/rdata_robjs/integrated_analysis_3_marks"
dir.create(outdir, showWarnings = TRUE)

outprefix <- file.path(outdir, paste0("integrated_analysis_3_marks_pseudobulk.fewerk27me3_", fewer.k27me3, ".downsamplereads_", downsample.reads, ".", Sys.Date()))
...
dat.de$cluster <- sapply(dat.de$cluster, function(x) m2c[[x]])

# keep just a few for now
jclsts.keep <- c("Bcell", "HSCs", "Monocyte", "Neutrophil", "Erythroblast")

dat.de <- subset(dat.de, cluster %in% jclsts.keep)

jclsts <- as.character(unique(dat.de$cluster))
names(jclsts) <- jclsts

de.genes.lst <- lapply(jclsts, function(jclst){
  de.genes <- subset(dat.de, p_val_adj < pval.min & cluster == jclst & avg_logFC > fc.min)$gene
  # if there is a semi colon, take the first?
  de.genes <- sapply(de.genes, function(x){
    ifelse(grepl(";", x), strsplit(x, ";")[[1]][[1]], x)
  })
})

# add background genes 
de.genes.lst$zOther <- sample(dat.de$gene, size = 500)
jclsts.after <- names(de.genes.lst)
names(jclsts.after) <- jclsts.after

g2e <- hash::hash(genes.common, ens.common)

de.ens.lst <- lapply(de.genes.lst, function(de.genes){
  sapply(de.genes, function(x) AssignHash(x, g2e, null.fill = x))
})
...
# write objects
save(clstrs.filt.dat, dat.annots.filt, tss.mats.filt, tss.mats.pbulk.ds, jlong.lst, jmerged, jlong.diff.genesets, de.genes.lst, de.ens.lst, file = outrdata)
