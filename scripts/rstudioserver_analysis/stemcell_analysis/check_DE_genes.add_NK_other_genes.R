...
outf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/rdata_robjs/de_genes_stringent_objects/de_genes_sorted_and_giladi.WithHouseKeepAndNotExpressed.FixExprsOther.add_NK_other_cells.", Sys.Date(), ".RData")
outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/rdata_robjs/de_genes_stringent_objects/de_genes_sorted_and_giladi.WithHouseKeepAndNotExpressed.FixExprsOther.add_NK_other_cells.", Sys.Date(), ".pdf")
...
markers.keep.again <- c("Erythroblast", "HSCs", "Bcell", "Neutrophil", "NKcell", "Basophil", "pDendritic", "cDendritic")
names(markers.keep.again) <- markers.keep.again
markers.keep.again.fewer <- c("Erythroblast", "HSCs", "Bcell", "Neutrophil", "NKcell")
names(markers.keep.again.fewer) <- markers.keep.again.fewer
...
inf <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/public_data/Giladi_et_al_2018/diff_exprs_Giladi_seurat.celltypes_filt.meth_poisson.Downsampled.EightCtypes.WithVpreb_FALSE.2020-12-04.rds"
...
cnames.keep <- c("Gene_ID", "Gene_Name", 
                 ...
                 "lymphocyte_of_B_lineage", "nucleate_erythrocyte", "natural_killer_cell")
cnames.keep.rename <- c("Gene_ID", "Gene_Name",
                        "HSCs", "Neutrophil", "Bcell", "Erythroblast", "NKcell")
dat <- dat[, ..cnames.keep]
...
de.ens.sorted.stringent <- lapply(markers.keep.again.fewer, function(jclst){
  print(jclst)
  jsub <- dat.norm.long %>% 
    rowwise() %>%
...
m.stringent <- lapply(markers.keep.again.fewer, function(jclst){
