...
ggplot(dat.merge, aes(x = umap1, y = umap2, color = cell.var.within.sum)) + geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_viridis_c(direction = -1)

# get MT reads
inf.mito <- "/home/jyeung/hpc/scChiC/raw_data/ZellerRawDataB6_redo_2019-12-13.tagged_bams_mergedbymarks/H3K4me1-BM_SC-merged.tagged.mitotable.csv"
dat.mito.raw <- fread(inf.mito)
dat.mito <- data.frame(cell = colnames(dat.mito.raw)[-1], mtcount = unlist(dat.mito.raw[2, -1]), stringsAsFactors = FALSE)
ncuts <- data.frame(cell = colnames(count.mat), ncuts = colSums(count.mat), stringsAsFactors = FALSE)

dat.merge2 <- left_join(dat.merge, dat.mito)
dat.merge2 <- left_join(dat.merge2, ncuts)

ggplot(dat.merge2 %>% filter(mtcount < 20), aes(x = mtcount, y = cell.var.within.sum.norm)) + geom_jitter(width = 1, alpha = 0.5) 
ggplot(dat.merge2, aes(x = mtcount, y = cell.var.within.sum.norm)) + geom_jitter(width = 1, alpha = 0.5) 

ggplot(dat.merge2, aes(x = mtcount, y = cell.var.within.sum.norm)) + geom_jitter(width = 1, alpha = 0.5)  + scale_x_log10()

ggplot(dat.merge2, aes(x = mtcount / ncuts, y = cell.var.within.sum.norm)) + geom_point()  + scale_x_log10()

ggplot(dat.merge2, aes(x = mtcount / ncuts, y = cell.var.within.sum.norm))  + geom_point()

ggplot(dat.merge2, aes(x = mtcount / ncuts)) + geom_density()

ggplot(dat.merge2, aes(x = umap1, y = umap2, color = log10(mtcount/ncuts) + 1)) + geom_point() + scale_color_viridis_c(na.value = "grey")

# correlation total counts?
dat.pca <- prcomp(topics.mat, center = TRUE)

dat.merge3 <- left_join(data.frame(cell = rownames(dat.pca$x), pc1 = dat.pca$x[, 1]), dat.merge)

ggplot(dat.merge3, aes(x = pc1, y = cell.var.within.sum.norm)) + geom_point()

# is there something in raw counts that can determine this?
terms.mat.input <- scale(t(terms.mat), center = TRUE, scale = TRUE)

terms.mat.rotate <- terms.mat.input %*% dat.pca$rotation[, 1]

terms.i <- order(abs(terms.mat.rotate), decreasing = TRUE)[1:10]

terms <- rownames(terms.mat.input)[terms.i]

# terms <- sample(x = rownames(terms.mat.input), size = 1000, replace = FALSE)

count.frac <- count.mat / colSums(count.mat)

bg.signature <- colSums(count.frac[terms, ])
bg.signature.raw <- colSums(count.mat[terms, ])
total.cuts <- colSums(count.mat)

dat.merge4 <- left_join(data.frame(cell = names(bg.signature), frac.umi = bg.signature, stringsAsFactors = FALSE), dat.merge)

ggplot(dat.merge4, aes(x = frac.umi, y = cell.var.within.sum.norm)) + geom_point() + scale_x_log10()
ggplot(dat.merge4, aes(x = frac.umi, y = cell.var.within.sum.norm)) + geom_point()
...
# bd <- mapply(FUN = function(x, p, n) binomial_deviance(x, p, n), bg.signature.raw, bg.signature, total.cuts)
(bd <- binomial_deviance(bg.signature.raw, bg.signature, total.cuts))



bd.dat <- data.frame(cell = names(bd), bd = bd, stringsAsFactors = FALSE)

dat.merge5 <- left_join(bd.dat, dat.merge)
dat.merge5 <- left_join(dat.merge5, subset(dat.merge4, select = c(cell, frac.umi)))

ggplot(dat.merge5, aes(x = bd, y = cell.var.within.sum.norm)) + geom_point() + scale_x_log10()
ggplot(dat.merge5, aes(x = bd, y = cell.var.within.sum.norm)) + geom_point()

ggplot(dat.merge5, aes(x = bd, y = frac.umi)) + geom_point()
