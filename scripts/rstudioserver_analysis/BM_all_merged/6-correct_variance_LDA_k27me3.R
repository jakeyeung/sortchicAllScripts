...
topics.mat.input <- scale(topics.mat, center = TRUE, scale = FALSE)
...
m.pc.vs.var <- ggplot(dat.pca, mapping = aes(x = PC4, y = cell.var.within.sum.norm, color = cell.var.within.sum.norm)) + geom_point() + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_viridis_c(direction = -1) + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
rot.mat <- pca.out$rotation[, c(1, 3:15)]
