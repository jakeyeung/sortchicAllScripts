...
outmain <- "/home/jyeung/hpc/scChiC/from_rstudioserver/pdfs"
infmain <- "/home/jyeung/hpc/scChiC/raw_demultiplexed/LDA_outputs_all/ldaAnalysisBins_B6BM_All_allmarks.2019-12-16"
...
for (jsuff in jsuffs){
  ...
  for (jmark in jmarks){
    ...
    infname <- paste0("ldaOut.B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.K-50.Robj")
    inf <- file.path(infmain, subdir, infname)
    if (!file.exists(inf)){
      print(inf)
