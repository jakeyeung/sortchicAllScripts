...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/BM_all_merged/2-explore_Giladi_et_al_write_gene_tables_from_DE.R
# Write all genes as well as top diffenreitlaly expressed genes 
...
inf <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/public_data/Giladi_et_al_2018/diff_exprs_Giladi_seurat.rds"
...
jclusts <- unique(de.output$cluster)

for (jclust in jclusts){
  print(jclust)
  de.output.filt <- subset(de.output, cluster == jclust)
  
  outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/databases/gene_tss/giladi_filtered.seurat.ctype"
  dir.create(outdir)
  jbase <- basename(inf)
  jbase <- ClipLast(jbase, jsep = "\\.", jsep.out = '.')
  jbase <- paste0(jbase, ".", jclust)
