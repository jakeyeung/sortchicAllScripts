...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/BM_all_merged/3-celltyping_on_LDA_from_gensim.R
# from Gensim 
...
# Functions ---------------------------------------------------------------
# 
# LoadGensim <- function(inf.doc2topics, inf.topic2terms, inf.mm.rnames, inf.mm.cnames, inf.mm){
#   
#   assertthat::assert_that(file.exists(inf.doc2topics))
#   assertthat::assert_that(file.exists(inf.topic2terms))
#   assertthat::assert_that(file.exists(inf.mm.rnames))
#   assertthat::assert_that(file.exists(inf.mm.cnames))
#   assertthat::assert_that(file.exists(inf.mm))
#   
#   cnames <- fread(inf.mm.cnames, header = FALSE, stringsAsFactors = FALSE)$V1
#   rnames <- fread(inf.mm.rnames, header = FALSE, stringsAsFactors = FALSE)$V1
#   
#   topics <- read.table(inf.doc2topics, sep = "\t")
#   terms <- read.table(inf.topic2terms, sep = "\t")
#   
#   colnames(topics) <- gsub("V", "Topic_", colnames(topics))
#   rownames(terms) <- paste("Topic", seq(nrow(topics)))
#   
#   rownames(topics) <- cnames
#   colnames(terms) <- rnames
#   
#   tm.result <- list()
#   tm.result$topics <- topics
#   tm.result$terms <- terms
#   
#   mat <- readMM(inf.mm)
#   
#   rownames(mat) <- rnames
#   rownames(mat) <- cnames
#   
#   return(list(tm.result = tm.result, mat = mat))
# }
...
jmain <- "/home/jyeung/hpc"
assertthat::assert_that(dir.exists(jmain))

outmain <- file.path(jmain, "scChiC/from_rstudioserver/pdfs")
dir.create(outmain)
inf.bulkdat <- file.path(jmain, "scChiC/public_data/E-MTAB-3079-query-results.fpkms.tsv")
assertthat::assert_that(file.exists(inf.bulkdat))

gensim.indir <- file.path(jmain, "scChiC/from_rstudioserver/quality_control_B6.2019-12-16/for_gensim")
gensim.outdir <- file.path(jmain, "scChiC/raw_demultiplexed/LDA_python_outputs/ldaAnalysisBins_B6BM_All_allmarks.2019-12-16.first_try_doc_to_topics_bug")
mat.indir <- file.path(jmain, "/scChiC/from_rstudioserver/quality_control_B6.2019-12-16")

inf.tss <- "/home/jyeung/hpc/databases/gene_tss/gene_tss_winsize.50000.bed"

keeptop <- 150

jsuffs <- c("AllMerged", "Unenriched", "UnenrichedXLinneg", "UnenrichedXStemCells")
# jsuff <- "AllMerged"
...
for (jsuff in jsuffs){
  ...
  for (jmark in jmarks){
    ...
    outpdfname <- paste0(jmark, ".", jsuff, ".bin_", jbin, ".celltype_using_topics_from_gensim.pdf")
    ...
    # subdir <- paste0("lda_outputs.B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.K-50.binarize.FALSE")
    inftopics <- file.path(gensim.outdir, paste0("BM_all.B6BM_", jsuff, "_", jmark, ".eta_auto.alpha_auto.ntopics_50.mapq_40.npasses_100.lda_model.doc_to_topics.zerosbugfixed.csv"))
    infterms <- file.path(gensim.outdir, paste0("BM_all.B6BM_", jsuff, "_", jmark, ".eta_auto.alpha_auto.ntopics_50.mapq_40.npasses_100.lda_model.topic_to_terms.csv"))
    # infmm <- file.path(gensim.indir, paste0("B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.mm"))
    infmat <- file.path(mat.indir, paste0("B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.rds"))
    infrnames <- file.path(gensim.indir, paste0("B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.mm.rownames"))
    infcnames <- file.path(gensim.indir, paste0("B6BM_", jsuff, "_", jmark, ".TAcutoff_0.5.countscutoff_1000.binfilt_cellfilt.2019-12-16.mm.colnames"))
    
    if (!file.exists(infmat)){
      print(paste(infmat, "not found, continuiing"))
      next
    }
    ...
    system.time(
      tm.result <- GetTmResultFromGensim(inf.topics = inftopics, inf.terms = infterms, inf.cellnames = infcnames, inf.binnames = infrnames)
    )
    
    topics.mat <- tm.result$topics
    terms.mat <- tm.result$terms
    
    print("Loading gensim... done") 
    count.mat.orig <- readRDS(infmat)
