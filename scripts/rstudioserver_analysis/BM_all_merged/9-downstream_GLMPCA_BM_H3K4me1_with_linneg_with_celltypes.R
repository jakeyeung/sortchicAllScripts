...
jmark <- jmarks[[1]]
  outname <- paste0("PZ_", jmark, "_withlinneg_GLMPCA.topics_celltypes_Giladi.topn_", topn, ".", Sys.Date(), ".pdf")
  ...
  if (!KeepMorePlates){
    outf <- paste0("/home/jyeung/data/from_rstudioserver/scchic/rdata_robjs/GLMPCA_outputs/PZ_H3K4me1.withLinneg.PlatesRemoved.GLMPCA_var_correction.150.2020-02-02.binskeep_500.devmode.RData")
  } else {
    outf <- paste0("/home/jyeung/data/from_rstudioserver/scchic/rdata_robjs/GLMPCA_outputs/PZ_H3K4me1.KeepMorePlates.GLMPCA_var_correction.150.2020-02-02.binskeep_1000.devmode.RData")
  }
  load(outf, v=T)
  ...
  dat.impute.log <- log2(t(posterior(out.lda)$topics %*% posterior(out.lda)$terms))  #  ashow original 
  ...
  m.var <- ggplot(dat.var.merge, aes(x = umap1, y = umap2, color = cell.var.within.sum.norm)) + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    geom_point() + scale_color_viridis_c(direction = -1, na.value = "grey85")
  ...
  for (jtopic in topics.sum$topic){
    ...
    m.top <- jsub.terms %>%
      ...
      xlab("") + ylab("Bin Weight") +
      ggtitle(paste("Top peak weights for:", jtopic))
    print(m.top)
  }
  
  # plot some genes
  jgenes <- c("S100a8", "Rora", "Inpp4b", "Runx3", "Ets1", "Notch2", "Irf8", "Bach2", "Il4", "Nfia", "Sox6", "Hlf")
  
  jterms.dat <- subset(annots.out$terms.annot, gene %in% jgenes) %>%
    group_by(gene) %>%
    filter(weight == max(weight))
  
  jterms <- jterms.dat$term
  jgenes.align <- jterms.dat$gene
  jterms.cat <- make.names(paste(jterms, jgenes.align, sep = "_"))
  
  dat.sub <- t(dat.impute.glm[jterms, ])
  colnames(dat.sub) <- paste(colnames(dat.sub), jgenes.align, sep = "_")
  dat.sub <- data.frame(cell = rownames(dat.sub), dat.sub)
  
  jsub.merge <- left_join(dat.umap.long.merge, dat.sub)
  
  for (jcname in jterms.cat){
    print(jcname)
    m.impute <- PlotXYWithColor(jsub.merge, xvar = "umap1", yvar = "umap2", cname = jcname)
    print(m.impute)
  }
  dev.off()
