...
inf.dats.rz <- "/home/jyeung/hpc/intestinal_scchic/from_rstudioiserver/quality_controls_intestines.OK/dats.rz.RData"
load(inf.dats.rz, v=T)
...
for (clstr.meth in clstr.meths){
  ...
  parallel::mclapply(jmarks, function(jmark){
    ...
    # which cells will be removed if we apply cutoff? 
    too.high.thres <- 10^5
    dat.umap.long.counts <- left_join(dat.umap.long, dats.rz %>% dplyr::select(samp, Run, experiRun, total.count,  TA.count), by = c("cell"="samp")) %>%
      rowwise() %>%
      mutate(too.high = total.count > too.high.thres)
    
    m.louv.together <- ggplot(dat.umap.long.counts %>% arrange(too.high), aes(x = umap1, y = umap2, color = too.high)) + 
      geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom")  + 
      ggtitle(jmark, paste("TooHigh Thres:", too.high.thres))
    print(m.louv.together)
