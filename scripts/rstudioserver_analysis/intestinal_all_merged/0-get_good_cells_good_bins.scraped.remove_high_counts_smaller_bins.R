# Jake Yeung
# Date of Creation: 2020-01-11
# File: ~/projects/scchic/scripts/rstudioserver_analysis/intestinal_all_merged/0-get_good_cells_good_bins.scraped.remove_high_counts_smaller_bins.R
# Remove high background, use smaller bins 
...
outmain <- paste0("/home/jyeung/hpc/intestinal_scchic/from_rstudioiserver/quality_controls_intestines_RemoveHigh_SmallerBins")
inf.meta <- "/home/jyeung/hpc/intestinal_scchic/metadata_all/OUD_to_filenames.txt"
indir.lhcounts <- "/home/jyeung/hpc/intestinal_scchic/raw_data/HVG-intestines.tagged_bams.all_merged/countTables"

for (jwin in jwins){
  print("Iteration...")
  print(jwin)
  indir <- paste0("/home/jyeung/hpc/intestinal_scchic/raw_data/HVG-intestines.tagged_bams.all_merged/countTables_otherWinSize/winsize_", jwin)
  dir.create(outmain)
  outpdf <- file.path(outmain, paste0("quality_controls_", jname, "_all_marks.", ".winsize_", jwin, "2019-11-01", ".pdf"))
  ...
  (infs.lh <- list.files(path = indir.lhcounts, pattern = jgrp.rz, all.files = TRUE, full.names = TRUE))
  ...
  # for k4me1, how many high counts are stem cells ? 
  cbPalette <- c("#696969", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#006400", "#FFB6C1", "#32CD32", "#0b1b7f", "#ff9f7d", "#eb9d01", "#7fbedf")
  jsub <- subset(dats.rz, mark == "k4me1") %>%
    rowwise() %>%
    mutate(prefix = ClipLast(samp, jsep = "-"))
  ggplot(jsub, mapping = aes(x = log10(total.count), fill = prefix)) + geom_density(alpha = 0.8) + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom") + 
    facet_wrap(~prefix) + scale_color_manual(values = cbPalette) + 
    geom_vline(xintercept = log10(50000))
  ...
  dats.rz <- dats.rz %>%
    rowwise() %>%
    mutate(is.good = total.count > counts.cutoff.lst[[mark]] & TA.frac > TA.cutoff & !is.empty & total.count < counts.topcutoff)
  ...
  outbed <- file.path(outmain, paste0("correlated_bins.", jname, ".bincutoff_", pcutoff.bin, ".winsize_", jwin, ".", jdate.out, ".bed"))
  ...
  # write pdf
  pdf(file = file.path(outmain, paste0("qc_plots_", jname, ".TAcutoff_", TA.cutoff, ".countscutoff_", counts.cutoff.low, "_", counts.cutoff.high, ".highcutoff_", counts.topcutoff, ".winsize_", jwin, ".", jdate.out, ".pdf")))
  print(m.all)
  print(m.dens)
  for (jmark in jmarks){
  ...
  for (jmark in jmarks){
    ...
    outf.base <- file.path(outmain, paste0("mat.", jname, ".AllMerged.", jmark, ".TAcutoff_", TA.cutoff, ".countscutoff_", counts.cutoff.low, "_", counts.cutoff.high, ".highcutoff_", counts.topcutoff, ".winsize_", jwin, ".", jdate.out))
  ...
  for (jmark in jmarks){
    outf.base <- file.path(outmain, paste0("mat.", jname, ".", jgrp.name, ".", jmark, ".TAcutoff_", TA.cutoff, ".countscutoff_", counts.cutoff.low, "_", counts.cutoff.high, ".highcutoff_", counts.topcutoff, ".winsize_", jwin, ".", jdate.out))
  ...
  # write dats.rz to output
  saveRDS(dats.rz, file = file.path(outmain, paste0("datsrz.", jwin, ".rds")))
}
