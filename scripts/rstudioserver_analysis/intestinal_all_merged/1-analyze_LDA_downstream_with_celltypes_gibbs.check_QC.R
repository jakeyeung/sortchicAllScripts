...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/intestinal_all_merged/1-analyze_LDA_downstream_with_celltypes_gibbs.check_QC.R
# 
...
outdir <- paste0("/home/jyeung/hpc/scChiC/from_rstudioserver/quality_control_postLDA_var_", jsystem)
outname <- paste0("BM_", jmark, ".varcutoff_", jcutoff.ncuts.var, ".RemoveImmune.rds")
pdfname <- paste0("BM_", jmark, ".varcutoff_", jcutoff.ncuts.var, ".RemoveImmune.pdf")
...
# inf <- paste0("/home/jyeung/hpc/intestinal_scchic/LDA_outputs/topicmodels/ldaAnalysisBins_intestines.2020-01-13.filter_immune_cells.50000_25000/lda_outputs.mat.Scraped.Unenriched.k4me1.TAcutoff_0.5.countscutoff_500_1000.highcutoff_100000.winsize_50000_25000.2020-01-11.immune_cells_filtered.K-30.binarize.FALSE/ldaOut.mat.Scraped.Unenriched.k4me1.TAcutoff_0.5.countscutoff_500_1000.highcutoff_100000.winsize_50000_25000.2020-01-11.immune_cells_filtered.K-30.Robj")
...
cells.mystery.removed <- rownames(pca.out$x)[which(pca.out$x[, 2] > -0.75)]

topics.mat.filt <- topics.mat.input[cells.keep, ]

rot.mat <- pca.out$rotation[, 2:15]

# topics.mat.rot <- topics.mat.input %*% rot.mat
topics.mat.rot <- scale(topics.mat.filt, center = TRUE, scale = TRUE) %*% rot.mat
...
cells.keep.noimmune <- intersect(cells.keep, cells.mystery.removed)

print(length(cells.keep.noimmune))

print(unique(dat.keep$prefix))

count.mat.keep <- count.mat[, cells.keep.noimmune]
