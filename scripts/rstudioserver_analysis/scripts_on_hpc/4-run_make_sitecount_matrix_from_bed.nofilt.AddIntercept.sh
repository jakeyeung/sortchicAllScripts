...
for jmark in $jmarks; do
    ...
    outdir="/home/jyeung/hpc/scChiC/mara_analysis_${jsuffix}/${jmark}/mara_input/sitecount_mats${outdirsuffix}"
    [[ ! -d $outdir ]] && mkdir -p $outdir

    outf="${outdir}/hiddenDomains_motevo_merged.closest.long.scale_${jscale}.center_${jcenter}.byrow_${byrow}.addint_${addint}.${jmark}.txt"
    ...
    then
        ...
        cd $wd; Rscript $rs $inf $outf --scale $jscale --center $jcenter --AddIntercept $addint
    else
        ...
        cd $wd; Rscript $rs $inf $outf --scale $jscale --center $jcenter --byrow --AddIntercept $addint
    fi
done
wait
echo "Done script"
