...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/k4me1_k9me3/3-setup_objs_from_all_merged_LDA.R
# Set up all merged objs from merged LDA, may have to handle uneven zeros
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/H3K4me1_H3K9me3_analysis/double_staining_input/SetupObjs_AllMerged_UnionRows"
...
dat.louv <- lapply(jmarks, function(jmark){
  dat.louv <- dat.umap.louvs[[jmark]]
  dat.louv$cluster <- dat.louv$louvain
  if (remove.na){
...
dir.create(outdir)
outfobjs <- file.path(outdir, paste0("SetupObjs_AllMerged_UnionRows.clstr_by_louvain_", jmarkdbl, ".removeNA_", remove.na, ".RData"))

print(length(dat.impute.repres.lst))
