...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/k4me1_k9me3/3-setup_objs_from_all_merged_LDA.keep_all_cells.fewer_repressed.fewer_active.R
# 
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/H3K4me1_H3K9me3_analysis/double_staining_input/SetupObjs_AllMerged_UnionRows_KeepAllCells_FewerRepressedClusters"
dir.create(outdir)
...
m.louvs <- lapply(dat.louv, function(jdat){
  m <- ggplot(jdat, mapping = aes(x = umap1, y = umap2, color = cluster)) + 
    geom_point() + 
    scale_color_manual(values = cbPalette) + 
    theme_bw() + 
    ggtitle("After cleaning up clusters") + 
    theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  print(m)
})
