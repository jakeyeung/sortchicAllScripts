...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/k4me1_k9me3/1-run_LDA_downstream_on_each_mark_and_dbl.filt.R
# 
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/H3K4me1_H3K9me3_LDA_downstream.filt.uniquerows"
dir.create(outdir)
...
inmain <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/raw_demultiplexed/LDA_outputs_all_spikeins/ldaAnalysisBins_mouse_spikein_BMround2all.dbl_common_rows.cellfilt_binfilt")
# load(inf.lda, v=T)

# jsuffix <- ".varfilt_1.5.K-30"
jsuffix <- "match_dbl.cellfilt.binfilt.K-30"
...
outs.lst <- lapply(jmarks, function(jmark){
  ...
  dat.umap$mark <- jmark
  outs <- list(tm.result = tm.result, dat.umap = dat.umap, count.mat = count.mat)
  return(outs)
})
...
count.mat.lst <- lapply(outs.lst, function(out){
  return(out$count.mat)
})
...
save(dat.merge, tm.result.lst, count.mat.lst, dat.umaps.lst, file = outrdata)
...
#   unlist()
# 
# bins.keep <- c(jtopics.k9me3.vec, jtopics.k4me1.vec)
# 
# 
