...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/k4me1_k9me3/4-match_UMAPs_K4me1_and_K9me3.KeepAllCellFewerRepressedMergeActiveClusters3.R
# description
...
remove.na <- TRUE
joutsuffix <- "UnionRows_KeepAllCells_FewerRepressedClusters_MergeActiveClusters3"
# joutsuffix <- "UnionRows_KeepAllCells_FewerRepressedClusters_MergeActiveClusters"

outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/H3K4me1_H3K9me3_scChIX_output/check_louvs_umaps.", joutsuffix, ".", Sys.Date(), ".pdf")
...
# load(inf.projs, v=T)
# 
# tm.result.k9me3 <- posterior(out.objs$out.lda)
# tm.result.k9me3 <- AddTopicToTmResult(tm.result.k9me3)
# 
# tm.result.k9me3.pred <- out.lda.predict
# 
# umap.out <- umap(tm.result.k9me3$topics, config = jsettings)
# 
# umap.pred <- predict(umap.out, data = tm.result.k9me3.pred$topics)
# 
# # merge 
# topics.mat.merge <- rbind(tm.result.k9me3$topics, tm.result.k9me3.pred$topics)
# 
# dat.umap.long.merge1 <- data.frame(cell = rownames(umap.out$layout), umap1 = umap.out$layout[, 1], umap2 = umap.out$layout[, 2], stain = "single", stringsAsFactors = FALSE)
# dat.umap.long.merge2 <- data.frame(cell = rownames(umap.pred), umap1 = umap.pred[, 1], umap2 = umap.pred[, 2], stain = "dbl", stringsAsFactors = FALSE)
# dat.umap.long.merge <- rbind(dat.umap.long.merge1, dat.umap.long.merge2)
# dat.umap.long.merge <- DoLouvain(topics.mat.merge, jsettings, dat.umap.long.merge)
# 
# ggplot(dat.umap.long.merge, aes(x = umap1, y = umap2, color = louvain)) + 
#   geom_point() + 
#   facet_wrap(~stain) +
#   scale_color_manual(values = cbPalette) + 
#   theme_bw() + 
#   theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# 
# jsub <- subset(dat.umap.long.merge, louvain == "9" )
...
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# bad.cells <- subset(jsub, umap1 < 3 & umap1 > 0 & umap2 < 0)$cell
# bad.cells <- subset(jsub, umap1 > 4)$cell
# 
# ggplot(jsub %>% mutate(louvain = cell %in% bad.cells), aes(x = umap1, y = umap2, color = louvain)) + 
#   geom_point() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# 
# dat.umap.long.k9me3 <- DoUmapAndLouvain(tm.result.k9me3.pred$topics, jsettings = jsettings)
# 
# dat.umap.long.k9me3$is.bad <- sapply(dat.umap.long.k9me3$cell, function(x) x %in% bad.cells)
# 
# ggplot(dat.umap.long.k9me3, aes(x = umap1, y = umap2, color = louvain)) + 
#   geom_point() + 
#   scale_color_manual(values = cbPalette) + 
#   theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# 
# ggplot(dat.umap.long.k9me3, aes(x = umap1, y = umap2, color = is.bad)) + 
#   geom_point() + 
#   scale_color_manual(values = cbPalette) + 
#   theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
...
# 
# # annotate cells
# dbl.cells <- dat.umap.long.k9me3$cell
# 
# 
# 
# # Load  -------------------------------------------------------------------
...
# jsettings$random_state <- 123
# 
# tm.result.remerge <- posterior(out.lda)
# topics.mat.remerge <- tm.result.remerge$topics
# 
# dat.umap.remerge <- DoUmapAndLouvain(topics.mat.remerge, jsettings)
# 
# dat.umap.remerge$stain <- sapply(dat.umap.remerge$cell, function(x) ifelse(x %in% dbl.cells, "dbl", "single"))
# 
# 
# cbPalette <- c("#696969", "#32CD32", "#56B4E9", "#FFB6C1", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#006400", "#FFB6C1", "#32CD32", "#0b1b7f", "#ff9f7d", "#eb9d01", "#7fbedf")
# ggplot(dat.umap.remerge, aes(x = umap1, y = umap2, color = louvain)) + 
#   geom_point() + 
#   theme_bw() + 
#   scale_color_manual(values = cbPalette) + 
#   theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
