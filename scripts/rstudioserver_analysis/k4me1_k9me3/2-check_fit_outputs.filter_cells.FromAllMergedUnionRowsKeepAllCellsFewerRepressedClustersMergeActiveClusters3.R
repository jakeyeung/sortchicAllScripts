...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/k4me1_k9me3/2-check_fit_outputs.filter_cells.FromAllMergedUnionRowsKeepAllCellsFewerRepressedClustersMergeActiveClusters.R
# description
...
outs.lst <- lapply(jmarks, function(jmark){
  ...
  return(list(dat.umap.out.merge = dat.umap.out.merge, tm.result = tm.result))
})

dat.umaps.lst <- lapply(outs.lst, function(jlst) jlst$dat.umap.out.merge)
tm.result.lst <- lapply(outs.lst, function(jlst) jlst$tm.result)
...
# plot distrubiont of oe topics
x1 <- tm.result.lst$H3K4me1$topics[, 11]
p1 <- log(x1 / (1 - x1))
x2 <- tm.result.lst$H3K9me3$topics[, 27]
p2 <- log(x2 / (1 - x2))
plot(density(p1), main = "H3K4me1, topic11"); abline(v = -2)
plot(density(p2), main = "H3K9me3, topic27"); abline(v = -3)
...
ggplot(dat.umap.dbl, aes(x = umap1, y = umap2, color = louvain)) + 
  geom_point() + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  scale_color_manual(values = cbPalette) + ggtitle("H3K4me1+H3K9me3 UMAP")
