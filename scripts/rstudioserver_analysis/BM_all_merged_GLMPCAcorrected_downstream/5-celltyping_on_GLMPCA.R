...
jcluster <- "Eryth-Sox6-_topic6"
jcluster <- "Bcells_topic16"

jcluster <- "Neutrophils_topic22"
...
# Use topic loadings from K4me1 to plot GLMPCA outputs --------------------
...
# Compare LDA with LDA  ---------------------------------------------------

dat.impute.repress <- t(tm.result.repress$topics %*% tm.result.repress$terms)
dat.impute.active <- t(tm.result.act$topics %*% tm.result.act$terms)
...
rows.i <- which(rownames(dat.impute.repress) %in% coords.keep.act)
# rows.i <- sample(x = seq(nrow(dat.impute.repress)), size = 100, replace = FALSE)

print(rows.i)

jsub <- dat.impute.repress[rows.i, ]

# check genes assigned in jsub
jsub.annot <- subset(annot.out$regions.annotated, region_coord %in% rownames(jsub))

print(sort(jsub.annot$SYMBOL))
...
m <- ggplot(dat.sub, aes(x = umap1, y = umap2, color = log10(exprs / (1 - exprs)))) + geom_point() + scale_color_viridis_c(0) + 
  scale_color_viridis_c(direction = -1) + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  ggtitle(jtop.act)
print(m)
...
# cluster by pseudobulk and find out why HSCs in K27me3 always low when I take top hits in 
  

head(dat.umap.glm.fillNAs)
cnames.keep.lst <- lapply(split(x = subset(dat.umap.glm.fillNAs, select = c(cell, cluster)), f = dat.umap.glm.fillNAs$cluster), function(x){
  return(x$cell)
})


# cnames.keep.lst <- 
count.pseudobulk.repress <- SumAcrossClusters(count.mat.repress, cnames.keep.lst) %>%
  as.data.frame()
cols.keep <- !grepl("Eryth.Sox6._topic6", colnames(count.pseudobulk.repress.norm))

count.pseudobulk.repress.filt <- count.pseudobulk.repress[, cols.keep]

count.pseudobulk.repress.norm <- sweep(count.pseudobulk.repress.filt, MARGIN = 2, STATS = colSums(count.pseudobulk.repress.filt), FUN = "/")

boxplot(log2(count.pseudobulk.repress.norm * 10^6 + 1))

# remove bad sample
boxplot(log2(count.pseudobulk.repress.norm[, cols.keep] * 10^6 + 1))

library(DESeq2)

meta <- data.frame(cell = colnames(count.pseudobulk.repress.filt), stringsAsFactors = FALSE)
rownames(meta) <- meta$cell

dds <- DESeqDataSetFromMatrix(countData = count.pseudobulk.repress.filt, colData = meta, design = ~ 1)

count.vst <- assay(vst(dds))

boxplot(count.vst)

# take top hits from topic and plot vst-normalized across celltypes?

count.vst[rownames(jsub), ]

boxplot(count.vst)

count.vst.long <- reshape2::melt(count.vst[coords.keep.act, ]); colnames(count.vst.long) <- c("region_coord", "cluster", "exprs")
count.vst.long <- count.vst.long %>%
  group_by(region_coord) %>%
  mutate(zscore = scale(exprs, center = TRUE, scale = TRUE))

ggplot(count.vst.long %>% filter(region_coord %in% coords.keep.act), aes(x = cluster, y = zscore)) + geom_boxplot() + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
...
dat.lda.act <- DoUmapFromLDA(out.lda.act, custom.settings = jsettings)


jtop.repress <- "topic22"  # neutrophils
...
# for (jtop.act in rownames(tm.result.act$terms)){
repress.loadings <- sort(tm.result.repress$terms[jtop.repress, ], decreasing = TRUE)
# act.loadings <- sort(tm.result.repress$terms["topic9", ], decreasing = TRUE)
(coords.keep.repress <- names(repress.loadings)[1:150])

# plot on K4me1 UMAP?
exprs.dat <- data.frame(cell = colnames(dat.impute.active), exprs = colMeans(dat.impute.active[coords.keep.repress, ]), stringsAsFactors = FALSE) %>%
  left_join(., dat.lda.act)
