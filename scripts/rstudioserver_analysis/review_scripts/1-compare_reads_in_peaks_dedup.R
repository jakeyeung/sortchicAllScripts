...
indir <- file.path(hubprefix, "jyeung/data/scChiC/public_data/K562_Kaya-Okur/H3K27me3/count_before_after_filtering_ENCODE_filt_dedup")
indir.ko.hdfilt <- file.path(hubprefix, "jyeung/data/scChiC/public_data/K562_Kaya-Okur/H3K27me3/count_before_after_filtering_hiddendomains_filt_dedup")

infs <- list.files(indir, pattern = "*.txt", full.names = TRUE)
infs.ko.hdfilt <- list.files(indir.ko.hdfilt, pattern = "*.txt", full.names = TRUE)

assertthat::assert_that(length(infs) > 0)
assertthat::assert_that(length(infs.ko.hdfilt) > 0)
...
outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/post_submission/K562_public_data/K562_KayaOkur_vs_Zeller_H3K27me3_dedup.", Sys.Date(), ".pdf")
...
ggplot(dat.hdfilt.merge, aes(y = cuts_in_chromo, x = frac.reads.in.peaks, color = set)) + 
  geom_point(alpha = 0.5) + 
  theme_bw() +
  theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom") + 
  ylab("unique reads") + 
...
ggplot(dat.hdfilt.merge, aes(y = log2(cuts_in_chromo), x = set, color = set)) + 
  geom_boxplot() + 
  theme_bw() +
  theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom") + 
  ylab("unique reads") + 
...
dat.cellrank <- dat.ko.hdfilt %>%
  ungroup() %>%
  mutate(rnk = rank(-nlines1))

ggplot(dat.cellrank, aes(y = nlines1, x = rnk)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10() + 
  theme_bw() + 
  xlab("Rank") + ylab("Number of unique fragments per cell") + 
  ggtitle("H3K27me3") + 
  theme(aspect.ratio=1)
  
  
# save output
outrds <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/post_submission/K562_public_data/dat_hdfilt_merge.KayaOkur_vs_Zeller.H3K27me3.2021-06-07.rds"
saveRDS(dat.hdfilt.merge, file = outrds)
