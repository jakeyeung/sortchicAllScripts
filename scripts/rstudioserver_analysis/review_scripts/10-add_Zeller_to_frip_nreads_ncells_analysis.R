...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/review_scripts/10-add_Zeller_to_frip_nreads_ncells_analysis.R
# 
...
outname <- paste0("frip_peakcounts_allcounts.", jdate, ".rds")
outrds <- file.path(outdir, outname)
dat.bart <- readRDS(outrds)
...
pdfname <- paste0("bartosovic_reanalysis_with_Zeller.", Sys.Date(), ".pdf")
...
outrdsmerged <- file.path(outdir, paste0("Bartosovic_reanalysis_many_datasets_frip_with_Zeller.", Sys.Date(), ".rds"))
