...
parallel::mclapply(jmarks, 
         function(jmark){
           
           outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/MARA_output_BM/heatmap_MARA_outputs.", jmark, ".again.thres_", jthres, ".pdf")
           ...
           zscores.sub <- subset(mara.out$zscores, zscore > jthres)
           ...
           names(hscs) <- "aHSC"
           ilcs <- list(ctypes[grepl("^InnateLyph|^ILC", ctypes)])
           names(ilcs) <- "zILC"
           bcells <- list(ctypes[grepl("^Bcell", ctypes)])
           names(bcells) <- "zBcells"
           ...
           lcore2 <- list(ctypes[grepl("LinnegCore2", ctypes)])
           names(lcore2) <- c("aLinnegCore2")
           
           clstr.hash <- hash()
           for (lst in list(hscs, ilcs, bcells, granus, dends, eryths, lcore2)){
             # print(lst)
             jname <- names(lst)[[1]]
             # print(jname)
             # print(lst[[1]])
           ...
           hm.out <- heatmap3(jsub, margins = c(5, 8), cexCol = jcex, cexRow = jcex, Colv = TRUE, Rowv = NA, RowSideLabs = "celltype",
           ...
           hm.out <- heatmap3(t(jsub), margins = c(5, 8), cexCol = jcex, cexRow = jcex, ColSideLabs = "celltype", 
                              Colv = NA, Rowv = TRUE, 
                              # ColSideColors = rep("blue", ncol(jsub)), 
