...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/ZF_all_merged_for_MARA/1-correct_LDA_from_peaks_by_variance.poisson.correct.R
# 
...
outdir <- paste0(hubprefix, "/jyeung/data/zebrafish_scchic/GLMPCA_outputs.Faster.", jmeth, ".", jsuffix)
dir.create(outdir)
...
jout <- mclapply(jmarks, function(jmark){
  ...
  outbase <- paste0("ZF_", jmark, ".", jcond, ".", jsuffix, ".GLMPCA_var_correction.", jmeth, ".mergebinsize_", mergesize, ".binskeep_", jbins.keep, ".covar_", jcovar.cname, ".penalty_", jpenalty, ".winsorize_", winsorize, ".", jdate)
  ...
  dat.merge2$cell.var.within.sum.norm.log2 <- log2(dat.merge2$cell.var.within.sum.norm)
  ...
  system.time(
    glm.out <- glmpca(Y = glm.inits$Y.filt, L = glm.inits$ntopics, fam = jmeth, 
                      ctl = list(maxIters = niter, tol = 1e-4), 
