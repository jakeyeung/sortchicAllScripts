...
outdir <- paste0("/home/jyeung/data/from_rstudioserver/scchic/rdata_robjs/GLMPCA_outputs.Faster.pois.RemoveLargePeaks", jsuffix)
...
# for (jcondmark in jcondsmarks){
  ...
  peaks.keep <- subset(peaks.dist, jdist < 10000)$rname
  ...
  system.time(
    glm.out <- glmpca(Y = glm.inits$Y.filt, L = glm.inits$ntopics, 
                      fam = "poi", 
                      ctl = list(maxIters = niter, eps = 1e-4, penalty = jpenalty, verbose=TRUE), 
