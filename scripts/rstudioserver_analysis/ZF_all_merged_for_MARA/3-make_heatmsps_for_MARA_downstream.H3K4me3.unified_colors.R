...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/ZF_all_merged_for_MARA/3-make_heatmsps_for_MARA_downstream.R
# 
...
outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/MARA_output_BM/heatmap_MARA_outputs.", jmark, ".unified_colors.pdf")
...
dends <- list(ctypes[grepl("Eryth", ctypes)])
names(dends) <- c("Erythroblasts")
...
inf.newannot <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/GLMPCA_peaks_primetime/H3K4me1_H3K4me3_H3K27me3_glmpca_peaks_primetime.2020-09-29.", jmark, ".txt")
assertthat::assert_that(file.exists(inf.newannot))
...
m1 <- ggplot(dat.annots.mergeclst, aes(x = umap1, y = umap2, color = as.character(cluster.renamed))) + 
  geom_point() + 
...
hm.out <- heatmap3(t(jsub), margins = c(5, 8), lasRow = 2, ColSideLabs = "celltype", 
                   cexCol = jcex, cexRow = jcex,
                   Colv = NA, Rowv = TRUE, 
                   # ColSideColors = rep("blue", ncol(jsub)), 
...
# dat.annots.mergeclst$colvec <- colvec
# 
# m1 <- ggplot(dat.annots.mergeclst, aes(x = umap1, y = umap2, color = clst.merged)) + 
#   geom_point() + 
...
#                    distfun = dist, hclustfun = hclust, method = jmeth, main = jmark)
# 
# hm.out <- heatmap3(t(jsub), margins = c(5, 8), cexCol = 0.35, 
#                    Colv = NA, Rowv = TRUE, 
#                    # ColSideColors = rep("blue", ncol(jsub)), 
#                    ColSideColors = colvec, 
#                    # RowSideColors = rep("red", nrow(jsub)), 
...
# save.image(file = outf)"/home/jyeung/hpc/scChiC/from_rstudioserver/rsessions/make_TF_heatmap.2020-03-04.pdf")
