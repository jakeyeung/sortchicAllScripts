...
outrds <- paste0("/home/jyeung/data/from_rstudioserver/spikein_fits/spikein_fits_bins.", Sys.Date(), ".TSS.NormByChromo.rds")
outpdf <- paste0("/home/jyeung/data/from_rstudioserver/spikein_fits/spikein_fits_bins.", Sys.Date(), ".TSS.NormByChromo.pdf")
...
mat.bins.lst.lst <- lapply(jmarks.vec, function(jmark){
  ...
  lapply(prep.vec, function(jconc){
    print(jconc)
    inf <- file.path(hubpath, paste0("jyeung/data/scChiC/spikein/fastqs/tagged_bams.scmo3.contigfixed/countTablesTSS/PZ-K562-", jmark, "-spikein-", jconc, ".scmo3.again_contigfixed.tagged.countTable.csv"))
    assertthat::assert_that(file.exists(inf))
