...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/K562_lsi_with_spikein.R
# 
...
if (make.plots){
  pdfout <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/spikeins_K562_cellcycle/dim_reduction_summaries_with_spikeins.", jmark, ".", Sys.Date(), ".", jsuffix, ".pdf")
...
inf.glmpca <- file.path(hubprefix, paste0("jyeung/data/scChiC/raw_demultiplexed/LDA_outputs_all_spikeins/glmpcaPois_K562_spikein/K562_count_tables_50000.", jmark, ".G1_G2_S", jsuffix))
assertthat::assert_that(file.exists(inf.glmpca))


inf <- file.path(hubprefix, paste0("jyeung/data/scChiC/from_rstudioserver/quality_control_spikeins_K562/K562_count_tables_50000.", jmark, ".G1_G2_S.rds"))
...
lsi.out <- RunLSI2(count.mat = mat, tf.method = "spikeins", idf.method = "inverselog", n.components = 30, .log = FALSE, .center = FALSE, .truncated = TRUE, spikeincounts = spikeincounts.vec)
...
ggplot(dat.umap.lsi, aes(x = umap1, y = umap2, color = cellcycle.str)) + geom_point() + 
...
ggplot(dat.umap.lsi, aes(x = cellcycle.str, y = log2(totalcounts / spikeincounts))) + 
  geom_point() + 
  geom_boxplot() + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  ggtitle("LSI")
...
ggplot(dat.umap.lda, aes(x = umap1, y = umap2, color = cellcycle.str)) + 
  geom_point() +
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  facet_wrap(~cellcycle.str) + 
  ggtitle("LDA")
...
ggplot(dat.umap.glmpca, aes(x = umap1, y = umap2, color = cellcycle.str)) + 
  geom_point() +
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  facet_wrap(~cellcycle.str) + 
  ggtitle("GLMPCA")
