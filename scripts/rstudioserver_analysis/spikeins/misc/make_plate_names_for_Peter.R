...
outdir <- file.path(hubprefix, "jyeung/data/scChiC/from_rstudioserver/cell_cluster_tables.batch_corrected_umaps.2020-12-28/rearranged_by_lineage/plate_names")
for (jmark in jmarks){
  print(jmark)
  fwrite(unique(subset(dat.metas.filt[[jmark]], select = experi) %>% dplyr::select(experi) %>% arrange(experi)), file = file.path(outdir, paste0("plate_names_rep2rep3.", jmark, ".txt")), sep = "\t")
}
