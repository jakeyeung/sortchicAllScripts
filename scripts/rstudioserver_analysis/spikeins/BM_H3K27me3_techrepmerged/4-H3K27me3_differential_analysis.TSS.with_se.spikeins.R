...
for (jmark in jmarks){
  print(jmark)
  outf <- file.path(outdir, paste0("poisson_fit_TSS_", jdist, ".",  jmark, ".", Sys.Date(), ".newannot2.rep2_rep3seq.with_se.spikeins.RData"))
  outpdf <- file.path(outdir, paste0("poisson_fit_TSS_", jdist, ".",  jmark, ".", Sys.Date(), ".newannot2.rep2_rep3seq.with_se.spikeins.pdf"))
  ...
  ncuts.for.fit.mark <- data.frame(cell = dat.annot$cell, ncuts.total = dat.annot$spikein_cuts, stringsAsFactors = FALSE)
  rownames(ncuts.for.fit.mark) <- ncuts.for.fit.mark$cell
  ncuts.for.fit.mark <- ncuts.for.fit.mark[colnames(count.mat), ]
