...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/K562_round2/filter_good_cells_for_bam_clutsering.R
# Filter bams by G1, G2, S. Remove bad cells .
...
indir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_spikeins_K562.round2.cellcyclefilt"

mats <- lapply(jmarks, function(jmark){
  print(jmark)
  fname <- paste0("K562_count_tables_50000.", jmark, ".cellcyclefilt.rds")
  inf <- file.path(indir, fname)
  readRDS(inf)
})

# 
# # infs <- list.files(path = indir, pattern = "*.rds", full.names = TRUE)
# 
...
dats.meta.annot <- lapply(dats.meta, function(dat){
  ...
  dat$cellcycle.str <- make.names(dat$cellcycle.str)
  dat$cellcycle.str <- gsub("\\.", "_", dat$cellcycle.str)
  return(dat)
})
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/cell_cluster_tables.K562_clean_by_cellcycle"

for (jmark in jmarks){
  print(jmark)
  fname <- paste0("K562_cleaned_by_cellcycle.", jmark, ".txt")
