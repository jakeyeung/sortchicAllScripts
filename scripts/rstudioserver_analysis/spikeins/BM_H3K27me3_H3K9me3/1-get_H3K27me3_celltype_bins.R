...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/BM_H3K27me3_H3K9me3/1-get_H3K27me3_celltype_bins.R
# 
...
pdfname <- "H3K27me3_50kb_topics.pdf"

tm.result.lst$H3K27me3$terms[1:5, 1:5]

jtopics <- topics.ordered$topic
jmark <- "H3K27me3"

make.topics.plots <- FALSE
...
for (jtopic in jtopics){
  jvec <- tm.result.lst[[jmark]]$topics[, jtopic]
  jdat <- data.frame(loading = jvec, cell = names(jvec), stringsAsFactors = FALSE)
  dat.metas.tmp <- left_join(dat.metas[[jmark]], jdat)
...
jtopics.lst <- list("topic7" = "NKsLeft",
                    "topic6" = "pDCs",
                    "topic4" = "NKsRight",
                    "topic10" = "Eryths",
                    "topic20" = "DCs",
                    # "topic15" = "HSPCs",
                    "topic28" = "HSPCs",
                    "topic12" = "BcellsSmaller",
                    "topic21" = "BcellsLarger",
                    "topic11" = "Basophils",
                    "topic13" = "Granulocytes"
                    )
...
jbins.lst <- lapply(jtopics2.lst, function(jtopic){
  jvec <- sort(tm.result.lst[[jmark]]$terms[jtopic, ], decreasing = TRUE)
...
# order bins
ctypes.bins <- c("Eryths", "BcellsLarger", "NKsLeft", "pDCs", "Granulocytes", "DCs", "Basophils", "HSPCs"); names(ctypes.bins) <- ctypes.bins
...
outbins <- file.path(outdir, "H3K27me3_cluster_specific_bins_50kb.txt")
...
pdfname.heatmap <- "H3K27me3_50kb_topics.heatmap.pdf"
# pdf(file.path(outdir, pdfname.heatmap), useDingbats = FALSE)

colsidecolors <- dat.metas.reordered[[jmark]]$colorcode
heatmap3::heatmap3(dat.imputed.lst[[jmark]][bins.keep, ], labCol = "", Rowv = NA, Colv = NA, ColSideColors = colsidecolors, scale = "row", revC = TRUE, main = paste0(jmark))
...
mat.pseudogenes <- lapply(ctypes.bins, function(jctype){
  ...
  colMeans(as.matrix(jmat.tmp[jbins.tmp, cells.tmp]))
}) %>%
  as.data.frame() %>%
  bind_rows() %>%
  t()
...
heatmap3::heatmap3(dat.imputed.lst[[jmark.ref]][bins.keep2.reordered, cells.ordered.lst[[jmark.ref]]], labCol = "", Rowv = NA, Colv = NA, ColSideColors = colsidecolors, scale = "row", revC = TRUE, main = paste0(jmark.ref))
