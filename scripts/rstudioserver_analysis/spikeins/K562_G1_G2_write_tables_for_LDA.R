...
ggplot(jsub.merge, aes(x = cellcycle.str, y = log10(chromocounts))) + 
  geom_boxplot() + 
  geom_point()  + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  facet_wrap(~mark)
...
ggplot(jsub.others, aes(x = repl, y = log10(chromocounts))) + 
  geom_boxplot() + 
  geom_point()  + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  facet_wrap(~mark)
...
indir.counts <- file.path(hubprefix, "jyeung/data/scChiC/raw_data_spikeins/VAN6969/K562/tagged_bams/countTablesAndRZr1only.NewFilters")
# infs.counts <- list.files(indir.counts, pattern = paste0("K562-EtOH-", jmark, ".*.countTable.csv"), full.names = TRUE)
...
# wwrite to output
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_spikeins_K562"

for (jmark in jmarks){
  print(jmark)
  outf <- file.path(outdir, paste0("K562_count_tables_50000.", jmark, ".rds"))
