...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/DE_analysis/1-run_DE_analysis_bins.four_marks.HSPC-vs_nonHSPC.spikeins.R
# description
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/poisson_fits_BM_AllMerged3.HSPCs_vs_nonHSPCs"
dir.create(outdir)
...
inf.lst <- lapply(jmarks, function(jmark){
  inf.tmp <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/cell_cluster_tables.batch_corrected_umaps.2020-12-28/metadata_batch_corrected.", jmark, ".2020-12-28.txt")
...
for (jmark in jmarks){
  outf <- file.path(outdir, paste0("poisson_fit_", jtype, ".",  jmark, ".", Sys.Date(), ".newannot2.witherrors.MoreBins.HSPCs_vs_nonHSPCs.total.RData"))
  if (file.exists(outf)){
    next
  }
  ...
  dat.annot.filt2 <- subset(dat.annot.filt, !is.na(spikein_cuts))
  ...
  ncuts.for.fit.mark <- data.frame(cell = dat.annot.filt2$cell, ncuts.total = dat.annot.filt2$cuts_total, stringsAsFactors = FALSE)
