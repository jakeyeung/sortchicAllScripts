...
outpdf <- file.path(outdir, paste0("variance_by_clusters_top_6085_bins.merge_ctypes.", merge.ctypes.by.lineage, ".", Sys.Date(), ".LDA.pdf"))
...
out.lst.lst <- lapply(jsuffix.lst, function(jsuffixmain){
  ...
  return(list(dat.raw.pbulk.lst = dat.raw.pbulk.lst, dat.impute.pbulk.lst = dat.impute.pbulk.lst))
})
...
dat.raw.pbulk.lst <- out.lst.lst$DE_bins_all_marks_top_6085_dists_to_TSS.annot_table$dat.raw.pbulk.lst
dat.impute.pbulk.lst <- out.lst.lst$DE_bins_all_marks_top_6085_dists_to_TSS.annot_table$dat.impute.pbulk.lst
# 
...
dat.vars.lst <- lapply(dat.impute.pbulk.lst, function(jdat){
  jdat.log2 <- log2(jdat)
