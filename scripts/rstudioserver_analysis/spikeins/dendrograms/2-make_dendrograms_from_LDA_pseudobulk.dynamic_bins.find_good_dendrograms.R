...
out.lst <- lapply(jmarks, function(jmark, jscale = 1000000L){
...
outpdf <- file.path(outdir, paste0("heatmaps.", jsuffixmain, ".", jmethod, ".", jscale, ".pdf"))

pdf(outpdf, useDingbats = FALSE)
for (jmark in jmarks){
  print(jmark)
  heatmap3::heatmap3(log2(dat.impute.pbulk.lst[[jmark]]), Rowv = TRUE, Colv = TRUE, scale = "row",  revC = TRUE, main = paste("imputed", jmark, jmethod, jscale), margins = c(5, 8), cexRow = 0.5, method = jmethod)
}
for (jmark in jmarks){
  print(jmark)
  outtxt <- file.path(outdir, paste0("table.", jmark, ".", jsuffixmain, ".", jmethod, ".", jscale, ".", Sys.Date(), ".txt"))
  write.table(x = log2(dat.raw.pbulk.lst[[jmark]]), file = outtxt, row.names = TRUE, col.names = NA)
  heatmap3::heatmap3(log2(dat.raw.pbulk.lst[[jmark]]), Rowv = TRUE, Colv = TRUE, scale = "row",  revC = TRUE, main = paste("raw", jmark, jmethod, jscale), margins = c(5, 8), cexRow = 0.5, method = jmethod)
}
dev.off()
