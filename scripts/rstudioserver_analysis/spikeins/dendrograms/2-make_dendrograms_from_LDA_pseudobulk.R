...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/dendrograms/2-make_dendrograms_from_LDA_pseudobulk.R
# 
...
jsuffixs <- c("celltype_specific_genes.TSS_10kb.txt", "celltype_specific_genes.TSS_TES.txt")
...
for (jsuffix in jsuffixs){
  ...
  outpdf <- file.path(outdir, paste0("heatmaps.", jsuffix, ".pdf"))
  
  pdf(outpdf, useDingbats = FALSE)
  
  for (jmark in jmarks){
    print(jmark)
    heatmap3::heatmap3(log2(dat.impute.pbulk.lst[[jmark]]), Rowv = TRUE, Colv = TRUE, scale = "row",  revC = TRUE, main = paste(jmark, jmethod, jsuffix), margins = c(5, 8), cexRow = 0.5, method = jmethod)
  }
  
  for (jmark in jmarks){
    print(jmark)
    heatmap3::heatmap3(log2(dat.raw.pbulk.lst[[jmark]]), Rowv = TRUE, Colv = TRUE, scale = "row",  revC = TRUE, main = paste("raw", jmark, jmethod, jsuffix), margins = c(5, 8), cexRow = 0.5, method = jmethod)
  }
  dev.off()
}
