...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/K562_spikeins_normalized_counts_NormByRaw.FiltLow.R
# description
...
mincounts <- 800
outrds <- paste0("/home/jyeung/data/from_rstudioserver/spikein_fits/spikein_fits_bins.", Sys.Date(), ".NormByRaw.logncells.LowFilt_", mincounts, ".WithRnames.rds")
outpdf <- paste0("/home/jyeung/data/from_rstudioserver/spikein_fits/spikein_fits_bins.", Sys.Date(), ".NormByRaw.logncells.LowFilt_", mincounts, ".WithRanmes.pdf")
    ...
    lapply(prep.vec, function(jprep){
      ...
      lapply(spikeinconc.vec, function(jspikeinconc){
        ...
        lapply(jrows, function(jrow){
          ...
          fit.dat <- FitGlmRowRaw(input.dat, return.fit.obj = FALSE)
          fit.dat$mark <- jmark
          fit.dat$prep <- jprep
