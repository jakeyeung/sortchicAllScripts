...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/K562_merged/check_rp1_LDA.from_loop.R
# description
rm(list=ls())
...
lapply(jmarks, function(jmark){
  ...
  outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/RP1_vs_K562/RP1_LDA_downstream.", jmark, ".compare_Seurat.from_loop.pdf")
  ...
  inf.lda <- file.path(hubprefix, paste0("jyeung/data/scChiC/raw_demultiplexed/LDA_outputs_all_spikeins/ldaAnalysisBins_RP1_ChICTAPS/lda_outputs.count_mat_from_seurat.", jmark, ".from_loop.K-30.binarize.FALSE/ldaOut.count_mat_from_seurat.", jmark, ".from_loop.K-30.Robj"))
  ...
  m <- ggplot(data = dat.umap, mapping = aes(x = umap1, y = umap2)) + 
    geom_point()  + 
    theme_bw() + 
  ...
  m <- ggplot(dat.pca, aes(x = pc1, y = pc2, color = log2(X.405..460.50.Area))) + 
    geom_point() + 
    theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    theme_bw() 
  ...
  m <- ggplot(dat.umap.frompca, aes(x = umap1, y = umap2, color = log2(X.405..460.50.Area))) + 
    geom_point() + 
    scale_color_viridis_c() + 
    theme_bw()  + 
    theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())  
