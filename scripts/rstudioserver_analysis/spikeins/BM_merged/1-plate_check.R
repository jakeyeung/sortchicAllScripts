...
# AnnotateSortFromLayoutBMall <- function(plate, rowcoord, colcoord, jrep){
...
#       ctype <- AnnotateSortFromLayout(plate = plate, rowcoord = rowcoord, colcoord = colcoord)
#     } else {
#       ctype <- AnnotateSortFromLayout.rep2(plate = plate, rowcoord = rowcoord, colcoord = colcoord)
#     }
#   }
#   return(ctype)
# }
...
# jexperi <- unique(dat.spikeins.mat$experi)[[1]]
jmarks <- unique(dat.spikeins.mat$mark)

for (jmark in jmarks){
  outpdf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/spikeins_mouse.BMround2all/plate_checks.again.", jmark, ".redo.pdf")
  pdf(outpdf, useDingbats = FALSE)
  print(jmark)
  jexperis <- unique(subset(dat.spikeins.mat, mark == jmark)$experi)
  for (jexperi in jexperis){
    print(jexperi)
    m <- ggplot(subset(dat.spikeins.mat, experi == jexperi), aes(y = rowcoord, x = colcoord, color = stype)) + 
...
# 
# ggplot(dat.spikeins.mat, aes(x = rowcoord, y = colcoord)) +
#   geom_point() +
#   theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
# 
# AnnotateSortFromLayoutBMall
# 
# 
# jrow <- subset(dat.spikeins.mat, experi == "PZ-BM-H3K27me3-3")[1, ]
# stype <- AnnotateSortFromLayout.rep2(plate = jrow$plate, rowcoord = jrow$rowcoord, colcoord = jrow$colcoord)
# 
# 
# 
#   rowwise()
#   mutate(stype = AnnotateSortFromLayout.rep2(plate = plate, rowcoord = rowcoord,  colcoord = colcoord))
# 
#   
# 
# m <- ggplot(subset(jrow, experi == jexperi), aes(y = rowcoord, x = colcoord, color = stype)) + 
#   geom_point(alpha = 0.5) + 
#   theme_bw() + 
#   scale_color_manual(values = cbPalette) + 
#   theme(aspect.ratio= 16 / 24, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
