...
dat.umaps.long <- bind_rows(dat.umaps.lst) %>%
  ...
  mutate(experi = ClipLast(cell, jsep = "_"), 
         ...
         stype = AnnotateSortFromLayoutBMall(plate, rowcoord, colcoord, jrep = jrep, jmark = mark))
...
jsub <- subset(dat.spikeins.mat, select = c(samp, chromocounts, spikeincounts, plate, jrep, stype)) %>%
  rowwise() %>%
  mutate(l2r = log2(chromocounts / spikeincounts))

dat.merge2 <- left_join(dat.merge, jsub, by = c("cell" = "samp"))

for (jmark in jmarks){
  m <- ggplot(dat.merge2 %>% filter(mark == jmark), aes(x = umap1, y = umap2, color = log2(chromocounts / spikeincounts))) + 
    geom_point() + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
    scale_color_viridis_c(direction = -1) + 
    ggtitle(jmark)
