...
indir <- file.path(hubprefix, "jyeung/data/scChiC/raw_data_spikeins/BMround2all_VAN5046_VAN5109_VAN5230_BM_VAN5232_VAN5233_VAN5234_VAN5235_VAN5236/tagged_bams_links/countTablesAndRZr1only_ByChromo.NewFilters.blfix")
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_BM_round2_all"
...
names.k4me1 <- grep(pattern = "K4me1", names(mats), value = TRUE)
names(names.k4me1) <- names.k4me1
names.k4me3 <- grep(pattern = "K4me3", names(mats), value = TRUE)
names(names.k4me3) <- names.k4me3
names.k27me3 <- grep(pattern = "K27me3", names(mats), value = TRUE)
names(names.k27me3) <- names.k27me3
names.k9me3 <- grep(pattern = "K9me3", names(mats), value = TRUE)
names(names.k9me3) <- names.k9me3

rnames.k4me1 <- Reduce(f = intersect, x = lapply(names.k4me1, function(x) rownames(mats[[x]])))
rnames.k4me3 <- Reduce(f = intersect, x = lapply(names.k4me3, function(x) rownames(mats[[x]])))
rnames.k27me3 <- Reduce(f = intersect, x = lapply(names.k27me3, function(x) rownames(mats[[x]])))
rnames.k9me3 <- Reduce(f = intersect, x = lapply(names.k9me3, function(x) rownames(mats[[x]])))



# Merge by mark  ----------------------------------------------------------

names.bymark <- list(names.k4me1, names.k4me3, names.k27me3, names.k9me3)
names(names.bymark) <- jmarks

rnames.bymark <- list(rnames.k4me1, rnames.k4me3, rnames.k27me3, rnames.k9me3)
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_BM_round2_all.blfix"
dir.create(outdir)
...
# Filter out by varfilt good cells  ---------------------------------------

indir.varfilt <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_BM_round2_all/varfilt"

mats.varfilt.nobl <- lapply(jmarks, function(jmark){
  print(jmark)
  fname <- paste0("count_mat.", jmark, ".varfilt_1.5.rds")
  inf.varfilt <- file.path(indir.varfilt, fname)
  readRDS(inf.varfilt)
})

cells.keep.lst <- lapply(mats.varfilt.nobl, colnames)



outdir.varfilt <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/quality_control_BM_round2_all.blfix/varfilt"
dir.create(outdir.varfilt)

for(jmark in jmarks){
  print(jmark)
  outf <- file.path(outdir.varfilt, paste0("count_mat.", jmark, ".filt_0.15_0.95_counts_and_l2r.blfix.varfilt.rds"))
  ...
  print(dim(jmat.filt))
  saveRDS(object = jmat.filt, file = outf)
}
