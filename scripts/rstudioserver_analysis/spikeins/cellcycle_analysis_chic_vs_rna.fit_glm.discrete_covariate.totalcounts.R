...
mclapply(jmarks, function(jmark){
  ...
  outpdf <- file.path(outdir, paste0("fit_cellcycle_pseudotime.", jmark, ".", Sys.Date(), "norm_by_totalcounts.pdf"))
  outrdata <- file.path(outdir, paste0("fit_cellcycle_pseudotime.", jmark, ".", Sys.Date(), ".norm_by_totalcounts.RData"))
  outrdata.simplified <- file.path(outdir, paste0("fit_cellcycle_pseudotime.", jmark, ".", Sys.Date(), ".norm_by_totalcounts.simplified.RData"))
  ...
  jfits <- lapply(rnames, function(rname){
    dat.counts.merged <- data.frame(count = mat[rname, ], cell = colnames(mat), stringsAsFactors = FALSE) %>%
      left_join(., dat.umap)
    jfit <- glm(formula = count ~ cellcycle.str + offset(log(totalcounts)), family = "poisson", data = dat.counts.merged)
  })
  # simiplify
  ...
  save(coefs.dat, dat.umap, file = outrdata.simplified)
