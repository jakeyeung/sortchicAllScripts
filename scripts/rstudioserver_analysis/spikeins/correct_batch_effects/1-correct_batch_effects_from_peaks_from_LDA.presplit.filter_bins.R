...
indir.glmpca <- file.path(hubprefix, paste0("jyeung/data/scChiC/glmpca_outputs/BM_from_projections"))
assertthat::assert_that(dir.exists(indir.glmpca))
binskeep <- "1000"

infs.glmpca <- lapply(jmarks, function(jmark){
  print(jmark)
  if (jmark != "H3K9me3"){
    inf.tmp <- file.path(indir.glmpca, paste0("glmpca.", jmark, ".bincutoff_0.binskeep_", binskeep, ".platename_jrep.szname_none.niter_500.reorder_rownames.dupfilt.suffix_old_to_new.RData"))
  } else {
    inf.tmp <- file.path(indir.glmpca, paste0("glmpca.", jmark, ".bincutoff_0.binskeep_", binskeep, ".platename_jrep.szname_none.niter_500.reorder_rownames.dupfilt.suffix_new_to_old.RData"))
  }
  print(inf.tmp)
...
glmpca.lst <- lapply(infs.glmpca, function(inf){
  load(inf, v=T)
  return(glm.out)
})

rnames.keep.lst <- lapply(glmpca.lst, function(x){
  rownames(x$loadings)
})
...
imputed.long.lst <- lapply(jmarks, function(jmark){
  ...
  rnames.keep <- rnames.keep.lst[[jmark]]
  print(length(rnames.keep))
  jmat.tmp <- imputed.lst[[jmark]][rnames.keep, ]
  print(dim(jmat.tmp))
  jmat.filt <- jmat.tmp %>%
    data.table::melt()
...
system.time(
  for (jmark in jmarks){
    print(jmark)
    outrds <- file.path(outdir, paste0("imputed_long.", jmark, ".from_LDA.binskeep_", binskeep, ".rds"))
