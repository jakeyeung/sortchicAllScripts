...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/BM_H3K4me1-H3K9me3_reseq/16-load_raw_counts_filter_cells_for_LDA_k4_k9_dynamic_regions_from_heatmap.R
# 
...
indir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/BM_H3K4me1-H3K9me3_tech_rep_merged/raw_demultiplexed/count_tables_from_H3K4me1_H3K9me3_dynamic_genes_and_bins_from_heatmap"

(infs.dbl <- list.files(indir, pattern = "PZ-BM-rep3-H3K9me3-H3K4me1-.*.sorted.tagged.bam.count_table_k4_k9_dynamic_bins_from_heatmap.txt", full.names = TRUE))
...
infs.k4 <- list.files(indir, pattern = "BM_round1_round2_merged_H3K4me1_.*..bam.count_table_k4_k9_dynamic_bins_from_heatmap.txt", full.names = TRUE)
assertthat::assert_that(length(infs.k4) > 0)
count.mat.k4 <- GetCountMat(infs.k4)

infs.k9 <- list.files(indir, pattern = "BM_round1_round2_merged_H3K9me3_.*..bam.count_table_k4_k9_dynamic_bins_from_heatmap.txt", full.names = TRUE)
...
outdir <- file.path(hubprefix, "jyeung/data/scChiC/from_rstudioserver/count_tables.BM.k4_k9_dynamic_regions/from_heatmap")
dir.create(outdir)

for (jname in jnames){
  print(jname)
  fname <- paste0("count_name.", jname, ".k4_k9_dynamic_bins_from_heatmap.", Sys.Date(), ".rds")
