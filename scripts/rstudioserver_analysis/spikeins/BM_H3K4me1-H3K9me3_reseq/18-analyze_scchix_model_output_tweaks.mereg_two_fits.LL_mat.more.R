# Jake Yeung
# Date of Creation: 2021-02-07
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/BM_H3K4me1-H3K9me3_reseq/18-analyze_scchix_model_output_tweaks.mereg_two_fits.LL_mat.more.R
# 
...
for (jclst in jclsts){
  outpdf <- file.path(outdir, paste0("scchix_outputs.H3K4me1-H3K9me3.", Sys.Date(), ".LL_mats.top_", jtop, ".", jclst, ".pdf"))
  pdf(outpdf, useDingbats = FALSE)
  ...
  if (jclst == "Basophils"){
    ...
    jsub.tmp <- subset(fits.merge, louv.act == jclst & w < (wmax - 0.01)) %>%
      arrange(lnprob)
  }
  jcells.tmp <- jsub.tmp$cell[1:min(nrow(jsub.tmp), jtop)]
  # jcell.tmp <- jsub.tmp$cell[[1]]
  for (jcell.tmp in jcells.tmp){
    print(jcell.tmp)
