...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/BM_H3K4me1-H3K9me3_reseq/16-load_raw_counts_filter_cells_for_LDA_k4_k9_dynamic_regions.TES_genomewide.R
# 
...
indir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/raw_data_spikeins/BM_H3K4me1-H3K9me3_tech_rep_merged/raw_demultiplexed/count_tables_from_H3K4me1_H3K9me3_TES_genomewide"
assertthat::assert_that(dir.exists(indir))

(infs.dbl <- list.files(indir, pattern = "PZ-BM-rep3-H3K9me3-H3K4me1-.*..sorted.tagged.bam.count_table.TES_genomewide.txt", full.names = TRUE))
...
infs.k4 <- list.files(indir, pattern = "BM_round1_round2_merged_H3K4me1_.*..bam.count_table_TES_genomewide.txt", full.names = TRUE)
count.mat.k4 <- GetCountMat(infs.k4)

infs.k9 <- list.files(indir, pattern = "BM_round1_round2_merged_H3K9me3_.*..bam.count_table_TES_genomewide.txt", full.names = TRUE)
...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/count_tables.BM.k4_k9_TES_genomewide"
dir.create(outdir)

for (jname in jnames){
  print(jname)
  fname <- paste0("count_name.", jname, ".k4_k9_TES_genomewide.", Sys.Date(), ".rds")
