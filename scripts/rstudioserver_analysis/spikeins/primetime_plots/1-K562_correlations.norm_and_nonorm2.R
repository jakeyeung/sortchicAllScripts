...
for (bsize in bsizes){
  ...
  outpdf1 <- file.path(indir, paste0("K562_correlations_with_chic_chip.", Sys.Date(), ".check.again.norm_nonorm8.heatmap.bsize_", bsize, ".pdf"))
  outtxt <- file.path(indir, paste0("K562_correlations_with_chic_chip.", Sys.Date(), ".check.again.norm_nonorm8.heatmap.bsize_", bsize, ".txt"))
  # outpdf2 <- file.path(indir, paste0("K562_correlations_with_chic_chip.", Sys.Date(), ".again.norm_nonorm3.heatmap.bsize_", bsize, ".pdf"))
  ...
  inf <- file.path(hubprefix, paste0("jyeung/data/scChiC/raw_data_spikeins/K562_tech_rep_merged/compare_with_chipseq_K562/multibigwigsummary.newbams.log2input_nonorm_r1only.binsizes/K562_chipseq_vs_chic_comparison.bsize_", bsize, ".txt"))
  dat.out <- fread(inf)
  ...
  cnames.new <- gsub(".merged.sorted.tagged.G1filt.r1only.sorted.bsize.mapq_60.r1only", "_chic", cnames.new)
  ...
  write.csv(xmat.final, file = outtxt, quote = FALSE, sep = "\t")
