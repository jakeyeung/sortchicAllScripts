...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/primetime_plots/3b-make_umaps_with_TSS_cuts.PeterList.higher_spread.batch_corrected.R
# 
...
outdir <- file.path(hubprefix, "jyeung/data/scChiC/from_rstudioserver/pdfs_all/primetime2/umaps_final_glmpca_tweak_umap.H3K27me3_rep2rep3reseq")
pdfout <- file.path(outdir, paste0("famous_genes_on_umap_raw_cuts.ListFromHeatmap.higher_spread.with_mat_adj.", Sys.Date() , ".batch_corrected.zscores.pdf"))
...
indir.mat <- file.path(hubprefix, "jyeung/data/scChiC/from_rstudioserver/robjs_for_heatmaps_and_umaps/heatmap_pdfs_and_ordered_matrices")

out.objs <- lapply(jmarks, function(jmark){
  fname <- paste0("heatmap_ordered_with_labels.", jmark, ".RData")
  inf <- file.path(indir.mat, fname)
  load(inf, v=T)
  return(list(mat.adj = mat.adj.tmp, labrows = labrows, colsidecolors = colsidecolors, rowsidecolors = rowsidecolors))
})


mat.adj.lst <- lapply(out.objs, function(jout){
  jout$mat.adj
})

# change to zscore
mat.adj.lst <- lapply(out.objs, function(jout){
  mat.adj.before <- jout$mat.adj
  rnames <- rownames(jout$mat.adj)
  cnames <- colnames(jout$mat.adj)
  mat.adj.zscore <- t(apply(jout$mat.adj, MARGIN = 1, function(jrow) scale(jrow, center = TRUE, scale = TRUE)))
  rownames(mat.adj.zscore) <- rnames
  colnames(mat.adj.zscore) <- cnames
  assertthat::assert_that(nrow(mat.adj.before) == nrow(mat.adj.zscore))
  return(mat.adj.zscore)
})
...
#   print(jmark)
#   inf.tmp <- file.path(indir.meta, paste0("cell_cluster_table_with_spikeins.", jmark, ".2020-12-23.umap_spread.final.txt"))
...
for (jgene in unlist(famous.genes.lst)){
  ...
  dat.tss.imputed.lst <- lapply(jmarks, function(jmark){
    print(jmark)
    rname <- grep(jgene, rownames(mat.adj.lst[[jmark]]), value = TRUE)
    print(rname)
    if (length(rname) == 0){
      print("Skipping, nothing found")
      print(rname)
    ...
    # print(rname)
    dat.tss.imputed <- data.frame(signal = mat.adj.lst[[jmark]][rname, ], cell = colnames(mat.adj.lst[[jmark]]), stringsAsFactors = FALSE) %>%
      left_join(., dat.metas[[jmark]]) %>%
      arrange(signal) %>%
      ungroup() %>%
      mutate(cuts.imputed = signal,
             gene = jgene,
             tssname = rname)
    return(dat.tss.imputed)
  })
  if (is.null(dat.tss.lst[[1]])){
    print("Skipping, nothing found2")
    next
  }
  if (is.null(dat.tss.imputed.lst[[1]])){
    print("Skipping, nothing found imputed")
    next
  }
  ...
  mlst <- lapply(jmarks, function(jmark){
    jrname <- dat.tss.imputed.lst[[jmark]]$tssname[[1]]
    m <- ggplot(dat.tss.imputed.lst[[jmark]], aes(x = umap1, y = umap2, color = signal)) + 
      geom_point_rast() + 
      scale_color_viridis_c() + 
      theme_minimal() + 
      ggtitle(paste(jmark, jgene, jrname)) + 
