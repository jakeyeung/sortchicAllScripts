...
jsuffix <- paste0("glmpca.H3K4me1.bincutoff_0.binskeep_0.byplate.szname_none.niter_1000.reorder_rownames.dupfilt.RData")
...
outbase <- paste0("motif_activity_H3K4me1.niter_", niter, ".", Sys.Date())
outname <- paste0(outbase, ".pdf")
outtxt <- paste0(outbase, ".txt")
...
indir.mara <- file.path(hubprefix, paste0("jyeung/data/scChiC/mara_analysis_BM-AllMerged3_Peaks/H3K4me1/mara_output.lessmemory/BM_H3K4me1.BM_AllMerged3.", jsuffix2, "-hiddenDomains_motevo_merged.closest.long.scale_0.center_0.byrow_0.H3K4me1/BM_H3K4me1.BM_AllMerged3.", jsuffix2))
print(indir.mara)
...
names(bcells) <- "Bcells"
granus <- list(ctypes[grepl("^Granu|^Basophils", ctypes)])
names(granus) <- "Granulocytes"
dends <- list(ctypes[grepl("DC", ctypes)])
...
dat.annots.mergeclst$clst.merged <- factor(dat.annots.mergeclst$clst.merged, levels = c("Eryths", "Bcells", "NKcells", "Granulocytes", "Dendritic", "HSPCs"))

dat.annots.mergeclst <- dat.annots.mergeclst %>%
  mutate(clst.merged = forcats::fct_relevel(clst.merged, c("Eryths", "Bcells", "NKcells", "Granulocytes", "Dendritic", "HSPCs"))) %>%
  arrange(clst.merged)
