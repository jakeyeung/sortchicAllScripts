...
# File: ~/projects/scchic/scripts/rstudioserver_analysis/spikeins/BM_merged_with_old/16-add_geneset_markers_to_UMAP.R
# Show UMAPs, add gene set markers at 10kb of TSS 
...
outpdf <- file.path(outdir, paste0("geneset_on_umap.binskeep_", binskeep, ".niter_", niter, ".", Sys.Date(), ".pdf"))
...
dat.glmpca.annot.lst <- lapply(jmarks, function(jmark){
  ...
  dat.glmpca.annot <- left_join(dat.glmpca, subset(dat.metas.init[[jmark]], select = c(cell, cluster, plate, batch)))
...
jmark <- "H3K27me3"
jdat <- dat.glmpca.annot.lst[[jmark]]

m1 <- ggplot(jdat, aes(x = umap1, y = umap2, color = cluster)) + 
  geom_point() + 
  theme_bw() + 
...
m2 <- ggplot(jdat, aes(x = umap1, y = umap2, color = batch)) + 
  geom_point() + 
  theme_bw() + 
  scale_color_manual(values = cbPalette) + 
  ggtitle(jmark) + 
...
jsets <- c("Erythroblast", "Bcell", "HSCs", "Neutrophil")
names(jsets) <- jsets
