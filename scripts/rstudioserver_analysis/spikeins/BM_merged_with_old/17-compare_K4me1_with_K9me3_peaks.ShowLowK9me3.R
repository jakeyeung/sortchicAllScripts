...
outpdf <- file.path(hubprefix, paste0("jyeung/data/scChiC/from_rstudioserver/pdfs_all/genesets_on_umap/anticorrelations_", jmark1, "_vs_", jmark2, ".lowK9me3.pdf"))
...
# fix k9me3 annotations to add proper batch 

dat.annot2.part1 <- subset(dat.annot2, batch != "Round2") %>%
    rowwise() %>%
...
dat.annot2.part2 <- subset(dat.annot2, batch == "Round2") %>%
  rowwise() %>%
    mutate(experi = ClipLast(cell, jsep = "_"),
           plate = as.numeric(SplitGetLast(experi, jsplit = "-")),
           rowcoord = AddPlateCoordinates(cell)$rowcoord,
           ...
           batch = AnnotateSortFromLayoutBMall(plate, rowcoord, colcoord, jrep, jmark2))


dat.annot2 <- rbind(dat.annot2.part1, dat.annot2.part2) %>%
  ungroup() %>%
  mutate(batch = gsub("LinNeg", "Linneg", batch), 
         batch = gsub("LSK", "StemCell", batch),
         cluster = gsub("Eryth", "Eryths", cluster),
...
m.batch <- ggplot(dat.annot2, aes(x = umap1, y = umap2, color = batch)) + 
  geom_point(alpha = 0.20) + 
  theme_bw() + 
  scale_color_manual(values = stypecols) + 
  theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), legend.position = "bottom")

# combine and replot to get the right colors
dat.annot.merge <- rbind(subset(dat.annot1, select = c(cell, umap1, umap2, cluster, batch, mark)), subset(dat.annot2, select = c(cell, umap1, umap2, cluster, batch, mark)))

m.batch2 <- ggplot(dat.annot.merge, aes(x = umap1, y = umap2, color = batch)) + 
  geom_point(alpha = 0.20) + 
  theme_bw() + 
...
m.ctype <- ggplot(dat.annot.merge, aes(x = umap1, y = umap2, color = cluster)) + 
  geom_point(alpha = 0.20) + 
  theme_bw() + 
  facet_wrap(~mark, scales = "free") + 
