...
jgenes <- c("Pax5", "Ebf1", "Irf4", "S100a8", "Cebpd", "Hlf", "Meis2", "Tal1", "Hbb-y")
...
for (jmark in jmarks){
  ...
  # jmark <- "H3K4me3"
  # jmark <- "H3K27me3"
  outf <- paste0("/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/pdfs_all/differential_analysis_BM_AllMerged3.gene_examples/DE_downstream_", jtype, ".", jmark, ".", Sys.Date(), ".with_examples.pdf")
  ...
  # Print celltype-specifici distributions ----------------------------------
  jannots <- names(de.ens.sorted.stringent)
  names(jannots) <- jannots
  ...
  for (jname in jnames){
    ...
    for (jannot in jannots){
      ...
      m <- ggplot(out$dat.summary %>% filter(abs(value) < 10) %>% mutate(gene.in.ctype = ens %in% jens), aes(x = value / log(2), fill = gene.in.ctype)) + 
        geom_density(alpha = 0.5) + 
        theme_bw() + 
        facet_wrap(~param) + 
        ggtitle(paste(jmark, jname), paste0("Genes split into two groups by:", jannot, " or not")) + 
        xlab("log2FC relative to HSPCs") + 
        geom_vline(xintercept = 0, linetype = "dotted") + 
        theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
      print(m)
