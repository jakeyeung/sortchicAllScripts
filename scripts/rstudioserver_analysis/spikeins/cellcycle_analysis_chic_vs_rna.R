...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/scChiC/from_rstudioserver/spikein_fits_cellcycle"
dir.create(outdir)
...
mclapply(jmarks, function(jmark){
  ...
  jfits <- lapply(rnames, function(rname){
    dat.counts.merged <- data.frame(count = mat[rname, ], cell = colnames(mat), stringsAsFactors = FALSE) %>%
      left_join(., dat.umap)
    jfit <- glm(formula = count ~ pseudotime + offset(log(spikeincounts)), family = "poisson", data = dat.counts.merged)
  })
