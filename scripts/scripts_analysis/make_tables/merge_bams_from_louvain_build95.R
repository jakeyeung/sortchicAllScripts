...
# File: ~/projects/scchic/scripts/scripts_analysis/make_tables/merge_bams_from_louvain_build95.R
# build95
...
data.table::fwrite(dat.merge, file = file.path(outdir, paste0("cell_to_bam_summary_build95.txt")), sep = "\t", col.names = FALSE)

# write UMAP coordinates 
data.table::fwrite(bind_rows(dat.umap.long.new.lst), file = file.path(outdir, paste0("cell_to_bam_summary_build95_withUmapCoordinates.txt")), sep = "\t", col.names = TRUE)
...
# write to output
for (jmark in jmarks.all){
  dat.sub <- subset(dat.umap.long.new.lst[[jmark]], select = c(cell, louvain))
