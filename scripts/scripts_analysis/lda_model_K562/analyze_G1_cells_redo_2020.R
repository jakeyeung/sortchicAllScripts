...
# for (jmark in jmarks){
  ...
  jcounts <- Matrix::colSums(count.mat)
  jcounts.dat <- data.frame(cell = names(jcounts), count = jcounts)
  ...
  dat.umap.long <- left_join(dat.umap.long, jcounts.dat)
  
  ggplot(dat.umap.long, aes(x = umap1, y = umap2, color = log10(count))) + geom_point() + scale_color_viridis_c()
  
  dat.impute.log <- log2(t(tm.result$topics %*% tm.result$terms))
  jchromos <- paste("chr", c(seq(19), "X", "Y"), sep = "")
  dat.var <- CalculateVarAll(dat.impute.log, jchromos)
  dat.umap.long <- left_join(dat.umap.long, dat.var)
  ggplot(dat.umap.long, aes(x = umap1, y = umap2, color = cell.var.within.sum.norm)) + geom_point() + scale_color_viridis_c()
