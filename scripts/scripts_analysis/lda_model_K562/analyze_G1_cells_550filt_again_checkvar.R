...
colsvec <- list(H3K4me1 = "cyan1", H3K4me3 = "darkblue", H3K9me3 = "darkorange1", H3K27me3 = "red1")
...
# for (jmark in jmarks){
  ...
  # jmark <- "H3K4me1"
  # jmark <- "H3K4me3"
  jbin <- "TRUE"; kstr <- "15_20_25_30"
  # jbin <- "FALSE"; kstr <- "15_20_25_30"
  dirmain <- paste0("/Users/yeung/data/scchic/from_cluster/LDA_out_K562/ldaAnalysisBins_MetaCell/lda_outputs.meanfilt_10.cellmin_", cellfilt, ".cellmax_500000.binarize.", jbin, ".no_filt")
  ...
  dat.umap.long <- data.frame(cell = rownames(dat.umap$layout), umap1 = dat.umap$layout[, 1], umap2 = dat.umap$layout[, 2], stringsAsFactors = FALSE)
  ...
  dat.sizes <- data.frame(cell = names(jcounts), cellsize = jcounts)
  
  dat.umap.long.var <- left_join(dat.umap.long, dat.var)
  dat.umap.long.var <- left_join(dat.umap.long.var, dat.sizes)
  dat.umap.long.var <- dat.umap.long.var %>%
    rowwise() %>%
    mutate(experi = ClipLast(cell, jsep = "_"))
  m1.var <- ggplot(dat.umap.long.var, aes(x = umap1, y = umap2, color = cell.var.within.sum.norm)) + geom_point(alpha = 0.5, size = 4) + scale_color_viridis_c(direction = -1) + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  print(m1.var)
  m1.sizes <- ggplot(dat.umap.long.var, aes(x = umap1, y = umap2, color = cellsize)) + geom_point(alpha = 0.5, size = 4) + scale_color_viridis_c(direction = -1) + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  print(m1.sizes)
  m1.experi <- ggplot(dat.umap.long.var, aes(x = umap1, y = umap2, color = experi)) + geom_point(alpha = 0.5, size = 4) + facet_wrap(~experi) + 
    theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  print(m1.experi)
  ...
  # do PCA remove the effect from variance?
  plot(dat.pca$x[, 1], dat.pca$x[, 2], pch = 20, col = jcol.counts)
  # project data
  dat.proj <-  dat.pca$rotation[, 2:ncol(dat.pca$x)]
  
  dat.pca$rotation[, 1]
  plot(dat.pca$x[, 1], jcounts)
  dat.test <- data.frame(cell = rownames(dat.pca$x), pc1 = dat.pca$x[, 1], stringsAsFactors = FALSE)
  dat.test <- left_join(dat.test, dat.var)
  
  ggplot(dat.test, aes(x = pc1, y = cell.var.within.sum.norm)) + geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  ggplot(dat.test, aes(y = pc1, x = cell.var.within.sum.norm)) + geom_point() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
  ...
  # plotf <- paste0("~/Dropbox/scCHiC_figs/figures/K562_G1_Analysis/K562_G1_Analysis.nn.", nn, ".cellfilt.", cellfilt, ".mark.", jmark, ".", Sys.Date(), ".bin.", jbin, ".pdf")
