...
dat.mat <- preprocessCore::normalize.quantiles(dat.mat, copy = TRUE)  # strong normalization,
colnames(dat.mat) <- cnames.tmp
rownames(dat.mat) <- rnames.tmp

boxplot(dat.mat, main = "After normalization")
```
...
cell.counts <- Matrix::colSums(count.filt)/5  # approximately account for sliding window
cell.counts.downsamp <- Matrix::colSums(count.filt)/5
