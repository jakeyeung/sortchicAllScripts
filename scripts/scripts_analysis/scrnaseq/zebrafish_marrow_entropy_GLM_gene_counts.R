...
# S.vec <- apply(mat.filt, 2, function(jcell) GetGeneCounts(jcell))
S.vec <- apply(zf@assays$RNA@counts, 2, function(jcell) GetGeneCounts(jcell))
...
m <- ggplot(meta.merge, aes(x = S, group = celltype)) + facet_wrap(~celltype, ncol = 1) + geom_density(fill = "lightblue") + 
  theme_bw() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + xlab("# Genes with nonzero Counts")
print(m)
