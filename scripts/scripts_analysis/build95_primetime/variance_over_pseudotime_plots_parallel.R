...
MakeVariabilityPlots <- function(jmark, trajname, tm.result.lst, dat.umap.long.trajs, 
                                 jcol = c("gray80", "gray50", "darkblue"), grep.strs = c("gray80", "gray50", "darkblue"), jalpha = 0.5, pseudo = 0, jscale = 1, mdpt.sd = 1, ms.sd = c(0, 3), mdpt.fc = 0.75, lims.fc = c(0, 3), jsize.facet = 0.2, gw.size.facet = 2, 
                                 chromogstr="chr15:"){
  ...
  acf(log2(jsub.hsc$exprs), main = paste(jmark, gstr, "HSC Cell autocorrelation"))
  acf(log2(jsub.hsc$exprs), lag.max = nrow(jsub.hsc), main = paste(jmark, gstr, "HSC Cell autocorrelation"))
  
  acf(log2(jsub.myeloid$exprs), main = paste(jmark, gstr, "Myeloid cell autocorrelation"))
  acf(log2(jsub.myeloid$exprs), lag.max = nrow(jsub.myeloid), main = paste(jmark, gstr, "Myeloid cell autocorrelation"))
...
MakeVariabilityPlots(jmark, trajname, tm.result.lst, dat.umap.long.trajs, jcol)
