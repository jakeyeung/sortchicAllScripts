...
# File: ~/projects/scchic/scripts/scripts_analysis/build95_primetime/tf_activity_analysis_correlations_pretty_from_peaks.R
# Use peak expression 
...
inf.exprs <- "/Users/yeung/data/scchic/from_cluster/mara_input/exprs_mat.rds"
...
# load exprs data
if (use.peaks.mat){
  dat.exprs <- readRDS(inf.exprs)
  cnames.old <- colnames(dat.exprs)
  cnames.new <- sapply(cnames.old, function(x) SwitchColnames(x, jsplit = ))
  colnames(dat.exprs) <- cnames.new
  # assign rows to gene
  peaks.annot <- AnnotateRegionToGene(rownames(dat.exprs))
  peaks.filt <- subset(peaks.annot, abs(distanceToTSS) < Inf)$region_coord
} else {
...
# for (ref.mark in jmarks){
  ...
  if (make.plots){
    pdfout <- paste0("~/data/scchic/pdfs/TF_analysis_gene_correlations.WithSpringTrajs.refmark.", ref.mark, ".", Sys.Date(), ".Pretty.Peaks.pdf")
  ...
  if (!use.peaks.mat){
    ...
    peaks.filt.intersect <- rownames(counts.mat.sub)
  } else {
    rows.i <- rownames(dat.exprs) %in% peaks.filt
    dat.exprs <- dat.exprs[rows.i, ]
    # pick closest 
    counts.mat.sub.long <- tidyr::gather(data = as.data.frame(dat.exprs) %>% mutate(bin = rownames(dat.exprs)), 
                                         key = "cell", value = "exprs", -bin)
    peaks.filt.intersect <- rownames(dat.exprs)
  }
  
  # Do large-scale correlations in a trajectory-specific way  ---------------
  
  annots.sub <- peaks.annot %>% 
    dplyr::select(SYMBOL, region_coord, geneStart, geneEnd, geneStrand, distanceToTSS) %>%
  ...
  counts.fit.all <- counts.mat.sub.long.inner %>%
    group_by(motif) %>%
    summarise(cor.out = cor(x = exprs, y = activity)) %>%
    arrange(desc(abs(cor.out)))
  
  ggplot(counts.fit.all, aes(x = cor.out)) + geom_histogram() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + ggtitle("Correlation before")
