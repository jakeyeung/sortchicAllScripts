...
# File: ~/projects/scChiC/scripts/scripts_analysis/lda_model/downstream_LDA_peaks_Autosomes_H3K4me1.R
# Peak analysis on autosomes
...
if (jbin){
  inf <- paste0("/tmp/ldaAnalysisHiddenDomains_1000/lda_outputs.AutosomesOnly.meanfilt_1.cellmin_100.cellmax_500000.binarize.", jbin, "/lda_out_meanfilt.PZ-BM-", jchip, ".CountThres0.K-5_10_15_20_25.Robj")
} else {
  inf <- paste0("/tmp/ldaAnalysisHiddenDomains_1000/lda_outputs.AutosomesOnly.meanfilt_1.cellmin_100.cellmax_500000.binarize.", jbin, "/lda_out_meanfilt.PZ-BM-", jchip, ".CountThres0.K-5_10_15_20_25.Robj")
}
...
hit.peaks <- subset(regions.annotated, abs(distanceToTSS) < 50000 & grepl("Hbb|Sox6", SYMBOL))$region_coord
# hit.peaks <- subset(regions.annotated, abs(distanceToTSS) < 50000 & grepl("Sox6", SYMBOL))$region_coord

subset(top.peaks.annotated, term %in% hit.peaks)
