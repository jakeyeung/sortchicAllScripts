...
m.louvain.filt <- ggplot(dat.umap.long, aes(x = umap1, y = umap2, color = annot)) + geom_point(size = 0.5) + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())
print(m.louvain.filt)
...
ggplot(dat.umap.long %>% mutate(louvain = ifelse(louvain %in% clsts, TRUE, FALSE)), aes(x = umap1, y = umap2)) + geom_point(size = 0.5) + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  geom_path(data = traj.granu, aes(color = lambda)) + 
  geom_path(data = traj.eryth, aes(color = lambda)) + 
  geom_path(data = traj.bcell, aes(color = lambda)) + 
  geom_path(data = traj.tcell, aes(color = lambda)) + 
  geom_path(data = traj.mega, aes(color = lambda)) 
...
# plot pseudotime
ggplot(traj.granu2, aes(x = lambda, y = counts)) + geom_smooth(se = FALSE) + geom_point()
ggplot(traj.granu2, aes(x = lambda, y = activity)) + geom_smooth(se = FALSE) + geom_point()
