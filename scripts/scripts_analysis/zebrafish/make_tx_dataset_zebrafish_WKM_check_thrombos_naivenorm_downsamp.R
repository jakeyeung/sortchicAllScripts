# Jake Yeung
# Date of Creation: 2019-12-12
# File: ~/projects/scchic/scripts/scripts_analysis/zebrafish/make_tx_dataset_zebrafish_WKM_check_thrombos_naivenorm_downsamp.R
# Downsamp 
...
csums <- colSums(ctype.sum)
# down sample to lowest
min.counts <- min(csums)
jprops <- min.counts / csums

ctype.sum.down <- downsampleMatrix(x = as.matrix(ctype.sum), prop = jprops)

boxplot(log2(ctype.sum.down))

ctype.sum.vst <- log2(sweep(ctype.sum, MARGIN = 2, STATS = colSums(ctype.sum), FUN = "/") * 10^6 + 1)

dds <- DESeqDataSetFromMatrix(countData = round(ctype.sum.down), colData = ctype.metadata, design = ~1)
...
jgene <- "rnf220a"
jgene <- "sox4a"
jrow <- grep(jgene, rownames(ctype.sum.vst))
...
jgenes <- lymph
ggplot(subset(dat.mat.filt.long, gene %in% hspc) %>% mutate(celltype = forcats::fct_reorder(celltype, dplyr::desc(zscore))), 
       aes(x = celltype, y = zscore)) +
  geom_boxplot() + 
  geom_point() + 
  ggtitle(paste("Summary of ", length(jgenes)), "genes") + 
  theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank())

# Save output  ------------------------------------------------------------

outf <- paste0("/Users/yeung/data/scchic/public_data/Zebrafish_WKM/Baron_et_al_pseudobulk_Zebrafish_WKM_cpmnorm.downsamp.", Sys.Date(), ".rds")
