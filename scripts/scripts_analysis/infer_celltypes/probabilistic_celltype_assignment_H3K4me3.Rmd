...
author: Jake Yeung
path: "~/projects/scchic/scripts/scripts_analysis/revisions/probabilistic_celltype_assignment_H3K4me3.R"
date: 2019-09-14
output:
   html_document:
     toc: true
...
m.likelihoods.norm <- ggplot(LL.dat.merge.cor %>% mutate(ctype.pred = ifelse(is.na(ctype.pred), "znone", ctype.pred)), 
       aes(x = umap1, y = umap2, color = LL.max.norm)) + geom_point() + 
  scale_color_viridis_c() + theme_bw() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank()) + 
  facet_wrap(~ctype.pred)
print(m.likelihoods.norm)


```


Save proportions output so we can reuse it for later

```{r saveoutput}
outobj <- paste0("/Users/yeung/data/scchic/public_data/proportions_vectors_Lara-Astiaso.topicskeep_", ntopics.keep, ".", jmark, ".", Sys.Date(), ".rds")
saveRDS(probs.lst.filt, file = outobj)
