...
m.proj1 <- ggplot(dat.umap.pred.merged %>% dplyr::filter(!is.stem), aes(x = umap1, y = umap2, color = is.stem)) + geom_point(alpha = 0.5) + 
  theme_minimal() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text = element_blank(), legend.position = "bottom") + 
  xlab("") + ylab("") + ggtitle(jmark)
m.proj <- ggplot(dat.umap.pred.merged, aes(x = umap1, y = umap2, color = is.stem)) + geom_point(alpha = 0.5) + 
  theme_minimal() + theme(aspect.ratio=1, panel.grid.major = element_blank(), panel.grid.minor = element_blank(), axis.text = element_blank(), legend.position = "bottom") + 
  xlab("") + ylab("") + ggtitle(jmark)

pdf(paste0("/Users/yeung/data/scchic/pdfs/stemcell_analysis/umap_projs.", jmark, ".pdf"))
  print(m.proj1)
  print(m.proj)
dev.off()
...
pdf(paste0("/Users/yeung/data/scchic/pdfs/stemcell_analysis/stemcell_analysis.", jmark, ".", Sys.Date(), ".pdf"))
...
pdf(paste0("/Users/yeung/data/scchic/pdfs/stemcell_analysis/stemcell_analysis.variance.", jmark, ".", Sys.Date(), ".pdf"))
  print(m.intravar)
  print(m.density1)
  print(m.density2)
dev.off()
