...
outdir <- "/home/jyeung/hub_oudenaarden/jyeung/data/databases/genebodies/mm10_IG_genes"
...
if (withchr){
  data.table::fwrite(dat.win.filt, 
                     file = file.path(outdir, paste0("gene_start_end.IG_genes.", Sys.Date(), ".pfilt_", pfilt, ".up_", upstream, ".down_", downstream, ".bed")),
                     sep = "\t", col.names = FALSE, row.names = FALSE)
} else {
  data.table::fwrite(dat.win.filt %>% mutate(seqnames = gsub("chr", "", seqnames)),
                     file = file.path(outdir, paste0("gene_start_end.IG_genes.", Sys.Date(), ".pfilt_", pfilt, ".up_", upstream, ".down_", downstream, ".nochr.bed")),
                     sep = "\t", col.names = FALSE, row.names = FALSE)
}
