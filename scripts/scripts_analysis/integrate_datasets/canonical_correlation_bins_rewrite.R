...
lapply(bins.to.plot.lst, function(bins.to.plot){
  m.cca <- ggplot(loads.long %>% mutate(bin.lab = ifelse(bingene %in% bins.to.plot1, bingene, NA)), aes(x = CC1, y = CC2, label = bin.lab)) + 
...
# 
# # plot CCA1 vs CCA2, highlight CCA1 bins
# m.cca <- ggplot(loads.long %>% mutate(bin.lab = ifelse(bingene %in% bins.to.plot1, bingene, NA)), aes(x = CC1, y = CC2, label = bin.lab)) + 
#   geom_point() + geom_text_repel() + geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
# print(m.cca)
# m.cca <- ggplot(loads.long %>% mutate(bin.lab = ifelse(bingene %in% bins.to.plot2, bingene, NA)), aes(x = CC1, y = CC2, label = bin.lab)) + 
#   geom_point() + geom_text_repel() + geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
# print(m.cca)
# m.cca <- ggplot(loads.long %>% mutate(bin.lab = ifelse(bingene %in% bins.to.plot2b, bingene, NA)), aes(x = CC1, y = CC2, label = bin.lab)) + 
#   geom_point() + geom_text_repel() + geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
# print(m.cca)
# m.cca <- ggplot(loads.long %>% mutate(bin.lab = ifelse(bingene %in% bins.to.plot3, bingene, NA)), aes(x = CC2, y = CC3, label = bin.lab, alpha = 0.1)) + 
#   geom_point() + geom_text_repel() + geom_vline(xintercept = 0) + geom_hline(yintercept = 0)
# print(m.cca)
# 
# 
...
# # 
# # system.time(
# #   cca.umap <- umap(loads[, 1:5])
# # )
# # 
# # # label CC1 genes
# # genes.keep <- (loads.dat %>% arrange(CC3))$bin[1:10]
# # genes.keep <- (loads.dat %>% arrange(desc(CC1)))$bin[1:10]
# # plot(cca.umap$layout[, 1], cca.umap$layout[, 2], pch = 20)
# # text(cca.umap$layout[, 1], cca.umap$layout[, 2], labels = sapply(rownames(cca.umap$layout), function(x) ifelse(x %in% genes.keep, x, "")))
# # 
# # # Why does CC1 show up some weird genes ?? --------------------------------
# # 
# # 
# # 
# 
