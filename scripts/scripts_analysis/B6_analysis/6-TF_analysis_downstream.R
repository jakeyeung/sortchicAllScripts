...
pdf(paste0("~/data/scchic/pdfs/B6_figures/tf_activities/TF_activities.K_", kchoose, ".pdf"), useDingbats = FALSE)
...
pdf(paste0("~/data/scchic/pdfs/B6_figures/tf_activities/TF_activity_with_gene_exprs.pdf"), useDingbats = FALSE)
...
print(m)
for (jhit in jhits){
  m <- ggplot(exprs.long %>% filter(gene == "Tal1" & !is.na(traj)), aes(x = exprs.log2, y = activity, color = traj)) + 
