# Jake Yeung
# Date of Creation: 2019-05-09
# File: ~/projects/scchic/scripts/scripts_analysis/B6_analysis/4-create_clusters_write_objects.R
# Create clusters and write objects
...
nn <- 60
jmindist.new <- c(jmindist.louv, 0.5, 0.3, 0.3)
nn.new <- c(nn, 60, 45, 27)
names(nn.new) <- jmarks
...
custom.settings <- custom.settings.new.lst[[jmark]]
...
# saveRDS(dat.umap.long, file = file.path(outdir, paste0("dat_umap_long_with_louvain.", jmark, ".rds")))
