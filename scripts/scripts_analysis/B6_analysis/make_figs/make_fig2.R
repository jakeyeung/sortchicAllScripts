...
mlst <- lapply(jmarks, function(jmark) PlotXYNoColor(dat.umap.long[[jmark]], xvar = "umap1", yvar = "umap2", jcol = colsvec[[jmark]], jsize = 1))
...
dat.merge <- left_join(dat.umap.long[[jmark]], topics.mat)
...
xvar <- "umap1"; yvar <- "umap2"; cname.str <- paste0("Topic_", jtopic)
...
# for topics and gene
dat.merge <- left_join(dat.umap.long[[jmark]], topics.mat %>% dplyr::select(c("cell", paste0("topic_", topics.vec))))
