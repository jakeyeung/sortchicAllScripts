...
dat.norm.zscore.mat <- spread(dat.norm.long %>% dplyr::select(-zscore), key = "celltype", value = "exprs") %>%
  as.data.frame() 
...
# plot(log10(probs.lst.filt$granulocyte), log10(probs.lst.filt$monocyte), pch = 20)
# text(log10(probs.lst.filt$granulocyte), log10(probs.lst.filt$monocyte), labels = names(probs.lst.filt[[1]]))
...
par(mfrow=c(1,1), mar=c(5.1, 4.1, 4.1, 2.1), mgp=c(3, 1, 0), las=0)
pdf(paste0("/Users/yeung/data/scchic/pdfs/celltyping_analysis_debugging/h3k4me3_debugging_", jctype, "_jgenes_exprs.pdf"), useDingbats = FALSE)
...
# # abline(a = 0, b = 1)
# 
# plot(probs.lst.filt$lymphocyte_of_B_lineage, sqrt(probs.lst.filt$lymphocyte_of_B_lineage))
# text(probs.lst.filt$lymphocyte_of_B_lineage, sqrt(probs.lst.filt$lymphocyte_of_B_lineage), names(probs.lst.filt[[1]]))
# abline(a = 0, b = 1)
# 
# # get gene that is megakaryocyte
# 
# print(names(probs.lst.filt))
# jctype <- "granulocyte"
# # jctype.ref <- "lymphocyte_of_B_lineage"
# # jctype.ref <- "monocyte"
# jctype.ref <- "granulocyte"
# # (jsub <- (LL.dat.merge %>% dplyr::filter(ctype.pred == jctype) %>% arrange(desc(p.max)))[1, ])
# (jsub <- (LL.dat.merge %>% dplyr::filter(ctype.pred == jctype) %>% arrange(umap2))[1, ])
# jcell <- jsub$cell[[1]]
# cell.vec <- count.filt[genes.filt, jcell]
# jprob <- probs.lst.filt[[jctype.ref]]
# 
# celld.vec.filt <- cell.vec[which(cell.vec > 1)]
# jprob.filt <- jprob[names(cell.vec.filt)]
# 
# # plot correlation?
# assertthat::assert_that(all(names(cell.vec) == names(jprob)))
# plot(cell.vec, sqrt(jprob), pch = 20, main = paste("Query ctype:", jctype, ".Ref ctype:", jctype.ref))
# text(cell.vec, sqrt(jprob), labels = names(cell.vec))
# 
# # plot(cell.vec.filt, jprob.filt, pch = 20, main = paste("Query ctype:", jctype, ".Ref ctype:", jctype.ref))
# # text(cell.vec.filt, jprob.filt, labels = names(cell.vec.filt))
# # plot(cell.vec, sqrt(probs.lst.filt$megakaryocyte) / sum(sqrt(probs.lst.filt$megakaryocyte)))
# 
# # remove zero counts?
# LL <- dmultinom(x = cell.vec, prob = sqrt(jprob), log = TRUE)
# print(LL)
# 
# # LL.filt <- dmultinom(x = cell.vec.filt, prob = sqrt(jprob.filt), log = TRUE)
# 
# print(paste("LL all counts:", LL))
# print(paste("LL remove zero counts:", LL.filt))
# 
# # 
