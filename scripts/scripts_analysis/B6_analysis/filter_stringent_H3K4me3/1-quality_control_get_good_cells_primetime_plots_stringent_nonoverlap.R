# Jake Yeung
# Date of Creation: 2019-06-11
# File: ~/projects/scchic/scripts/scripts_analysis/B6_analysis/filter_stringent_H3K4me3/1-quality_control_get_good_cells_primetime_plots_stringent_nonoverlap.R
# Nonoverlapping 
...
# outtxt <- paste0("~/data/scchic/quality_control/good_cells.pcounts.", (1-p), ".pfrac.", (1-pfrac), ".txt")
# if (!file.exists(outtxt)){
#   fwrite(good.cells, file = outtxt, col.names = TRUE)
# }
...
dat.summary <- dat.merge %>%
  group_by(mark) %>%
  summarise(cellsum.mean = mean(cellsum),
            cellsum.median = median(cellsum))

outtxt.summary <- paste0("/Users/yeung/Dropbox/scCHiC_figs/tables/median_mean_unique_counts.txt")
data.table::fwrite(dat.summary, file = outtxt.summary, sep="\t")


pdf(paste0("/Users/yeung/data/scchic/pdfs/B6_figures/quality_controls_B6/quality_controls_all_marks_stringent.pcounts.", (1-p), ".pfrac.", (1-pfrac), ".", Sys.Date(), ".nonoverlapping.pdf"), useDingbats = FALSE)
  print(m.final)
  print(m.hist)
  print(m.hist.all)
dev.off()

# generate mean, median counts across histone modifications
