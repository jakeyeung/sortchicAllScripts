...
# File: ~/projects/scchic/scripts/scripts_analysis/B6_analysis/filter_stringent_H3K4me3/transcriptome_versus_H3K4me1.R
...
imput.merged.lst <- lapply(louvains, function(l){
  ...
  cells.keep.i <- which(colnames(imput.mat) %in% cells.keep)
  assertthat::assert_that(length(cells.keep.i) > 0)
...
nclustrs <- max(as.numeric(cor.dat$cluster))
clstrhash <- hash(sort(as.character(seq(length(cbPalette[1:nclustrs])))), cbPalette[1:nclustrs])
cor.dat$jcol <- sapply(cor.dat$cluster, function(x) clstrhash[[x]])
# cor.dat$jcol <- cbPalette[as.numeric(cor.dat$cluster)]

# plot individually and order properly 


# set scale on y axis manually 
print(cor.dat$jcor)
...
m1 <- ggplot(cor.dat, aes(x = CellType, y = jcor, fill = jcol)) + geom_bar(stat = "identity") + facet_wrap(~cluster) + theme_bw() +
  ...
  ylab(paste0(jmethod)) + xlab("") + ylim(jylim)
print(m1)
