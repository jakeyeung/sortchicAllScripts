...
outmain0="/nfs/scistore12/hpcgrp/jyeung/data_from_Hubrecht/hpc_hub_oudenaarden/scChiC/new_experiments/LDA_outputs_bugfixed_k27me3_rerun"
...
for inf in `ls -d ${inmain}/*new_only*.rds`; do
    prefix=$(basename $inf)
    prefix=${prefix%.*}
	...
	module load R/4.1.2
    cmd="module load R/4.1.2; Rscript $rs $inf $outdir --topics $topics --projname $bname"
