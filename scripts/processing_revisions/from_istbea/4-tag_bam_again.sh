...
outdir="${inbase}/tagged_bams/BM_${jmark}"
...
for indir in `ls -d $inmain/PZ*${jmark}*`; do
	...
	# assume sorted
    cmd=". /nfs/scistore12/hpcgrp/jyeung/miniconda3/etc/profile.d/conda.sh; conda activate scmo2022; cd $wd; bamtagmultiome.py -method chic --cluster -clusterdir $tmpdir -o $outbamtagged -mem $jmem2 -time $jtime2 --multiprocess $sortedbam"
    echo $cmd
