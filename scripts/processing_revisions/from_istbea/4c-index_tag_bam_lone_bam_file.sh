...
for indir in `ls -d $inmain/PZ-sortChIC-BM-SL4-k27me3-2`; do
    ...
    cmd=". /nfs/scistore12/hpcgrp/jyeung/miniconda3/etc/profile.d/conda.sh; conda activate scmo2022; samtools index $sortedbam; cd $wd; bamtagmultiome.py -method chic --cluster -clusterdir $tmpdir -o $outbamtagged -mem $jmem2 -time $jtime2 --multiprocess $sortedbam"
    echo $cmd
